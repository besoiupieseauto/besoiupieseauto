(function () {
    ...
    var API = p('/api/cont_endpoint.php');
    ...
    function renderOrderItem(order, item) {
        ...
        return '<div class="ac-order-item">' +
            ...
            '<div class="ac-order-item-actions">' +
                ...
                '<a class="ac-order-btn ac-order-btn--view" href="' + escapeHtml(item.product_url || p('/catalog')) + '">' +
    ...
    function cartProductUrl(item) {
        if (item.product_id) {
            return p('/produs?id=' + encodeURIComponent(item.product_id));
        }
        if (item.oem) {
            return p('/catalog?q=' + encodeURIComponent(item.oem));
        }
        return p('/catalog?q=' + encodeURIComponent(item.product_name || ''));
    }
    ...
    function renderLastOrder(order) {
        ...
        if (!order) {
            wrap.innerHTML = '<div class="ac-dash-empty">' +
                ...
                'Nu ai comenzi încă. <a href="' + p('/catalog') + '" class="ac-link">Plasează prima comandă</a>.' +
            '</div>';
            return;
        }
    ...
    function renderActivityCards(orders) {
        ...
        wrap.innerHTML =
            '<article class="ac-activity-card">' +
                ...
                cartPreview +
                '<a href="' + p('/cart') + '" class="ac-activity-link">Vezi coșul <i class="fa-solid fa-arrow-right"></i></a>' +
            '</article>' +
            '<article class="ac-activity-card">' +
                '<div class="ac-activity-top">' +
    ...
    function initFavorites() {
        ...
        function renderFavorites() {
            ...
            listEl.innerHTML = favorites.map(function (productId) {
                var href = p('/produs?id=' + encodeURIComponent(productId));
