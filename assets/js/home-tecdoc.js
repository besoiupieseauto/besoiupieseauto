...
'use strict';
    ...
    const p = typeof window.besoiuPath === 'function' ? window.besoiuPath : function (x) { return x; };
    const BESOIU_ADMIN = window.BESOIU_ADMIN_CTX === true;

    let _homeNotifyTimer = null;

    function besoiuHomeNotify(message, isError) {
        const raw = String(message || '').trim();
        if (!raw) return;
        const text = typeof window.besoiuStorefrontPublicError === 'function'
            ? window.besoiuStorefrontPublicError(raw)
            : raw;
        if (!text) return;
        let node = document.getElementById('besoiu-home-notify');
        if (!node) {
            node = document.createElement('div');
            node.id = 'besoiu-home-notify';
            node.className = 'besoiu-home-notify';
            node.setAttribute('role', 'alert');
            node.setAttribute('aria-live', 'assertive');
            document.body.appendChild(node);
        }
        node.textContent = text;
        node.classList.remove('besoiu-notify--dormant', 'is-error', 'is-ok', 'is-visible');
        node.classList.add(isError ? 'is-error' : 'is-ok', 'is-visible');
        window.clearTimeout(_homeNotifyTimer);
        _homeNotifyTimer = window.setTimeout(function () {
            node.classList.remove('is-visible');
            node.classList.add('besoiu-notify--dormant');
        }, isError ? 6000 : 4200);
        if (window.DebugPanel && typeof window.DebugPanel.err === 'function' && isError) {
            window.DebugPanel.err('Notificare', { message: text });
        }
    }

    window.besoiuHomeNotify = besoiuHomeNotify;

    let _homeAjaxBusyCount = 0;

    function setHomeAjaxBusy(isBusy) {
        _homeAjaxBusyCount = Math.max(0, _homeAjaxBusyCount + (isBusy ? 1 : -1));
        const busy = _homeAjaxBusyCount > 0;
        ['btnSearch', '_product-apply-filters', '_home-search-btn'].forEach(function (id) {
            const el = document.getElementById(id);
            if (el) {
                el.classList.toggle('is-ajax-busy', busy);
                el.setAttribute('aria-busy', busy ? 'true' : 'false');
            }
        });
    }

    window.besoiuSetHomeAjaxBusy = setHomeAjaxBusy;

    async function besoiuFetchJson(label, url, options) {
        window.DebugPanel.req(`Cerere: ${label}`, { url });
        let response;
        try {
            response = await fetch(url, options || {});
        } catch (error) {
            const msg = 'Conexiune eșuată: ' + label + '. Verifică rețeaua și încearcă din nou.';
            besoiuHomeNotify(msg, true);
            window.DebugPanel.err(`Eroare: ${label}`, error.message || String(error));
            throw error;
        }
        if (!response.ok) {
            const msg = 'Serverul a răspuns cu HTTP ' + response.status + ' (' + label + ').';
            besoiuHomeNotify(msg, true);
            window.DebugPanel.err(`HTTP ${response.status}: ${label}`, { url, status: response.status });
            throw new Error(msg);
        }
        let data;
        try {
            ...
            const msg = 'Răspuns invalid de la server (' + label + ').';
            besoiuHomeNotify(msg, true);
            window.DebugPanel.err(`JSON: ${label}`, parseError.message || String(parseError));
            throw parseError;
        }
        const quotaMsg = typeof tecdocQuotaNotice === 'function' ? tecdocQuotaNotice(data) : '';
        if (quotaMsg) {
    ...
    /* ════════════════════════════════════════════════════════════
       ...
       ════════════════════════════════════════════════════════════ */
    const apiPath = p('/tecdoc_proxy.php');
    ...
    function buildHomeCatalogUrl(overrides) {
        ...
        return p('/catalog') + (qs ? '?' + qs : '');
    }

    function applyHomeFilterOverrides(overrides) {
        if (!overrides || typeof overrides !== 'object') {
            return;
        }
        if (overrides.q !== undefined) {
            ...
            const vinInput = document.getElementById('_filter-vin');
            const oemInput = document.getElementById('_filter-oem');
            const nameInput = document.getElementById('_filter-name');
            const headerInput = document.getElementById('_home-product-name');
            if (headerInput) headerInput.value = query;
            if (!query) {
                return;
            }
            const vinNorm = query.toUpperCase().replace(/[^A-Z0-9]/g, '');
            const looksLikeVin = vinNorm.length === 17 && !/[IOQ]/.test(vinNorm) && /^[A-HJ-NPR-Z0-9]{17}$/.test(vinNorm);
            if (looksLikeVin) {
                if (vinInput) vinInput.value = query;
                if (oemInput) oemInput.value = '';
                if (nameInput) nameInput.value = '';
            } else {
                const oemNorm = query.toUpperCase().trim().replace(/[\s\-\/_.]/g, '').replace(/[^A-Z0-9]/g, '');
                if (vinInput) vinInput.value = query;
                if (oemInput) oemInput.value = oemNorm;
                if (nameInput) nameInput.value = oemNorm || query;
            }
        }
        if (overrides.category) {
            _selectedCategoryLabel = String(overrides.category);
            _selectedCategory = String(overrides.category_slug || overrides.category);
            if (catBadgeText) catBadgeText.textContent = _selectedCategoryLabel;
            setCatBadgeIcon(overrides.icon || '');
            catBadge?.classList.add('visible');
            loadAdminSubcategoriesForCategory(_selectedCategoryLabel);
        }
        if (overrides.car_id) {
            _ultimulCarId = String(overrides.car_id);
            if (selMotor) selMotor.value = String(overrides.car_id);
        }
        updateSearchButtonState();
    }

    function runHomeAjaxFilters(overrides) {
        if (!isHomeVitrinaPage()) {
            return false;
        }
        applyHomeFilterOverrides(overrides);
        if (typeof window.__applyProductFilters === 'function') {
            window.__applyProductFilters().catch(function (error) {
                besoiuHomeNotify(error.message || 'Căutarea nu a putut fi finalizată.', true);
            });
            scrollToProductTop();
        }
        return true;
    ...
    function redirectHomeFiltersToCatalog(overrides) {
        return runHomeAjaxFilters(overrides);
    }
    ...
    async function loadAdminSubcategoriesForCategory(categoryLabel) {
        ...
        if (!label) return;
        try {
            const res = await fetch(p('/api_categorii.php?action=subcategorii&category=' + encodeURIComponent(label)));
        ...
        (async function loadAdminData() {
            try {
                const [marciRes, modeleRes, subcatRes] = await Promise.all([
                    fetch(p('/api_categorii.php?action=marci')),
                    fetch(p('/api_categorii.php?action=modele')),
                    fetch(p('/api_categorii.php?action=subcategorii'))
                ]);
    ...
    async function loadSearchInsights() {
        ...
        try {
            const res = await fetch(p('/api_categorii.php?action=insights&limit=8'));
            const json = await res.json();
    ...
    async function apiCall(label, url) {
        return besoiuFetchJson(label, url);
    }
    ...
    function goToCatalogFromHomeFilters(overrides) {
        ...
        if (!vinQuery && !hasCategory && !hasVehicle) {
            besoiuHomeNotify('Selectează categorie, vehicul sau introdu cod OEM / VIN, apoi apasă CAUTĂ PIESĂ.', true);
            return true;
        }
        runHomeAjaxFilters(overrides);
        return true;
    }
    ...
    (function bindHomeHeaderSearch() {
        ...
        function runHeaderSearch() {
            ...
            if (!query) {
                besoiuHomeNotify('Introdu cod OEM, VIN sau denumire piesă.', true);
                return;
            }
            applyHomeFilterOverrides({ q: query });
            runHomeAjaxFilters({ q: query });
        }
    ...
    function atașeazăEvenimenteCarduri() {
        document.querySelectorAll('._product-card').forEach(card => {
            ...
            const goToProduct = (e) => {
                ...
                if (!productId) {
                    besoiuHomeNotify('Produsul nu este disponibil în magazin.', true);
                    return;
                }
                window.location.href = p(`/produs?id=${encodeURIComponent(productId)}`);
            };
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function articleImage(article) { return article.s3image || article.img || article.image || 'assets/images/products/product-1.jpg'; }
        ...
        function renderStockProductCards(payload, title, options) {
            ...
            products.forEach(product => {
                ...
                const image = product.image || 'assets/images/products/product-1.jpg';
                ...
                if (useHomeGrid && isHomeVitrinaCard && window.besoiuRenderHomeVitrinaCardHtml) {
                    ...
                    <div class="_product-card-image"><img src="${escapeHtml(image)}" alt="${escapeHtml(name)}" width="300" height="225" loading="lazy" decoding="async"></div>
                    ${specsBlock}
                    ${noteBlock}
        ...
        async function searchProductsViaTecDoc(filters, globalSearch = false) {
            ...
            const data = await besoiuFetchJson('search_stock', `${apiPath}?${params.toString()}`);
        ...
        function gridToolbar(title, backAction) {
            return `<div class="_product-grid-toolbar">
                <button type="button" class="_product-btn _product-btn--back" data-grid-back="${backAction}"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i> Înapoi</button>
        ...
        async function loadVitrinaProducts(forceReload) {
            ...
            try {
                const data = await besoiuFetchJson('vitrina', apiPath + '?action=vitrina&limit=' + vitrinaLimit);
                ...
                besoiuHomeNotify(error.message || 'Vitrina indisponibilă.', true);
        ...
        async function applyFilters() {
            ...
            const ctx = getSelectedSubcategoryContext();
            const carId = _ultimulCarId || selMotor?.value || '';
            const hasVehicleFilter = !!(carId && carId !== '0');
            const hasCategory = !!(_selectedCategoryLabel || _selectedCategory);
            const hasSubcat = !!(ctx && ctx.bdSubcategory);

            if (isHomeVitrinaPage()) {
                if (!hasRemoteQuery && !hasCategory && !hasVehicleFilter && !hasSubcat && !tecdocSelectionActive) {
                    loadVitrinaProducts();
                    return;
                }
                tecdocSelectionActive = true;
            } else if (hasRemoteQuery && redirectHomeFiltersToCatalog({ q: hasVinSearch ? vinQuery : (oemQuery || f.name) })) {
                return;
            }
            ...
            if (hasRemoteQuery) {
                ...
                try {
                    ...
                    renderStockProductCards(data, title, { forceReplace: isHomeVitrinaPage(), homeGridLayout: isHomeVitrinaPage() });
                    if (!data.count && data.notice) {
                        setEmptyStateMessage(data.notice, false);
                    }
                    scrollToProductTop();
                } catch (error) {
                    besoiuHomeNotify(error.message || 'Eroare la căutarea TecDoc.', true);
                    showProductDebug(error.message || 'Eroare la căutarea TecDoc.', 'err');
                    showBrowsePrompt(error.message || 'Nu am putut căuta produsele.');
                } finally {
                    if (loaderPiese) hideBlock(loaderPiese);
                    setHomeAjaxBusy(false);
                }
                return;
            }

            if (isHomeVitrinaPage() && (hasCategory || hasVehicleFilter || hasSubcat)) {
                setHomeAjaxBusy(true);
                try {
                    if (typeof window.__autoApplyVehicleFilter === 'function') {
                        await window.__autoApplyVehicleFilter();
                    }
                } finally {
                    setHomeAjaxBusy(false);
                }
        ...
        window.__besoiuHomeQuickSearch = function (query) {
            applyHomeFilterOverrides({ q: String(query || '').trim() });
            tecdocSelectionActive = true;
            return applyFilters();
        };
        ...
        async function loadProductsForSubcategory(nodeId, title, options) {
            ...
            try {
                ...
                besoiuHomeNotify(error.message || 'Nu am putut încărca produsele disponibile.', true);
        ...
        window.__autoApplyVehicleFilter = async function () {
            ...
            if (category) {
                ...
                try {
                    ...
                    besoiuHomeNotify(error.message || 'Eroare la filtrarea produselor.', true);
    ...
    document.addEventListener('DOMContentLoaded', function initHomeCategoryUi() {
        ...
        async function loadDynamicCategories() {
            try {
                const json = await besoiuFetchJson('categorii_popup', p('/api_categorii.php?action=popup'));
                ...
                if (catPopup) {
                    catPopup.innerHTML = cats.map(c => `
                        <div class="cat-popup-item" data-cat="${c.slug}" data-cat-label="${c.label}" data-cat-icon="${c.icon}">
                            <img src="${c.icon}" alt="" role="presentation" width="30" height="30">${c.label}<span class="cat-count">${c.count || ''}</span>
                        </div>
                    `).join('');
                }
                ...
                if (categoryGrid) {
                    categoryGrid.innerHTML = cats.map(c => `
                        <div class="category-card" data-cat="${c.slug}" data-cat-label="${c.label}" data-cat-icon="${c.icon}">
                            <div class="icon"><img src="${c.icon}" alt="" role="presentation" class="category-card-icon" width="44" height="44"></div>
                            <b>${c.label}</b>
                ...
                besoiuHomeNotify('Nu am putut încărca categoriile. Încearcă din nou.', true);
