...
'use strict';
    ...
    function buildMagazinCardPayload(product) {
        ...
        var image = product.image || 'assets/images/products/product-1.jpg';
        var priceNumeric = Number(product.price_numeric || product.price || product.pPrice || product.pBasePrice || 0);
        if (!isFinite(priceNumeric) || priceNumeric < 0) {
            priceNumeric = 0;
        }
        var priceLabel = product.price_label
            || (priceNumeric > 0 ? priceNumeric.toFixed(2).replace(/\.?0+$/, '') + ' RON' : 'La cerere');
        var oldPriceNumeric = Number(product.price_old || product.old_price || product.pPriceOld || product.compare_at_price || 0);
        var oldPriceLabel = '';
        if (isFinite(oldPriceNumeric) && oldPriceNumeric > priceNumeric && priceNumeric > 0) {
            oldPriceLabel = oldPriceNumeric.toFixed(2).replace(/\.?0+$/, '') + ' RON';
        }
        // Prefer public randomn_id for /produs?id= — never fall back to numeric DB id alone.
        var productId = String(product.randomn_id || '').trim();
        ...
        return {
            ...
            stock: String(product.stock || product.pStock || '').trim(),
            stockLabel: String(product.stock || product.pStock || '').trim() || 'La cerere',
    ...
    async function appendTecdocStatusNotice() {
        ...
        try {
            const response = await fetch(p('/tecdoc_proxy.php?action=status'));
    ...
    async function renderRemoteStockSearch(query, mode) {
        ...
        try {
            ...
            var response = await fetch(p('/tecdoc_proxy.php?' + params.toString()));
    ...
    function loadSubcategoriesForCategory(categoryLabel, keepActive) {
        ...
        return fetch(p('/api_categorii.php?action=subcategorii&category=' + encodeURIComponent(categoryLabel)));
    ...
    function rebuildFacetLists() {
        return fetch(p('/api_categorii.php?action=facets'));
            .then(function(res) { return res.json(); })
            .then(function(json) {
    ...
    productGrid.addEventListener('click', function(e) {
        ...
        if (btnDetalii) {
            ...
            window.location.href = productId ? p('/produs?id=' + encodeURIComponent(productId)) : p('/produs');
            return;
        }
        ...
        if (imgClick) {
            ...
            window.location.href = pid ? p('/produs?id=' + encodeURIComponent(pid)) : p('/produs');
        }
    });
