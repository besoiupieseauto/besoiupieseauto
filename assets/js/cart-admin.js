(function () {
    ...
    const ORDERS_ENDPOINT = (window.BESOIU_API && window.BESOIU_API.orders) || '/api/orders.php';
    ...
    function renderMiniCart(message = 'Produs adaugat in cos', isError = false) {
        ...
        const itemsHtml = cart.length
            ? cart.slice(0, 4).map((item, index) => `
                <div style="display:flex;gap:10px;align-items:center;padding:10px 0;border-top:${index === 0 ? '0' : '1px solid #f0f0f0'};">
                    <img src="${escapeHtml(item.product_image || getDefaultProductImage())}" alt="${escapeHtml(item.product_name)}" width="58" height="58" style="width:58px;height:58px;object-fit:cover;border-radius:10px;border:1px solid #eee;background:#f7f7f7;">
                    ...
                    <button type="button" data-mini-cart-remove="${index}" title="Șterge produsul" aria-label="Șterge ${escapeHtml(item.product_name)} din coș" style="width:28px;height:28px;border:0;border-radius:50%;background:#dc3545;color:#fff;font-weight:700;cursor:pointer;">x</button>
                </div>
            `).join('')
        ...
        miniCart.innerHTML = `
            ...
            <div style="display:flex;gap:8px;margin-top:12px;">
                ...
                <a href="${p('/cart')}" style="flex:1;text-align:center;background:#111;color:#fff;border-radius:10px;padding:10px;text-decoration:none;">Mergi la cos</a>
            </div>
        `;
    ...
    function productCardActionsHtml(productId = '') {
        const id = String(productId || '').trim();
        const idAttr = id ? ` data-product-id="${escapeHtml(id)}"` : '';
        const href = productDetailHref(id);
        return `<div class="_product-card-actions">
            <a class="_product-card-btn product_detal" href="${escapeHtml(href)}"${idAttr} title="Detalii produs" aria-label="Vezi detalii produs">
    ...
    function renderCartPage() {
        ...
        if (cart.length === 0) {
            container.innerHTML = `<div class="cart-empty">
                <div class="cart-empty-icon"><i class="fa-solid fa-cart-shopping" aria-hidden="true"></i></div>
                <h3>Coșul tău este gol</h3>
                <p>Adaugă produse din catalog pentru a continua.</p>
                <a href="${p('/catalog')}">Vezi catalogul <i class="fa-solid fa-arrow-right" aria-hidden="true"></i></a>
            </div>`;
        } else {
            container.innerHTML = cart.map((item, index) => {
                ...
                return `<div class="ci" data-cart-index="${index}">
                    <div class="ci-img">
                        <img src="${escapeHtml(item.product_image || getDefaultProductImage())}" alt="${escapeHtml(item.product_name)}" width="90" height="90" loading="lazy">
                    </div>
                    <div class="ci-info">
                        <div class="ci-name">${escapeHtml(item.product_name)}</div>
                        <div class="ci-meta">${item.oem ? 'OEM: ' + escapeHtml(item.oem) + ' · ' : ''}${unitPrice}/buc</div>
                        <span class="ci-stock ${item.in_stock === false ? 'red' : 'green'}"><i class="fa-solid fa-circle-check" aria-hidden="true"></i> ${item.in_stock === false ? 'Stoc epuizat' : 'In stoc'}</span>
                    </div>
                    <div class="ci-qty">
                        <label for="ci-qty-input-${index}" class="visually-hidden">Cantitate ${escapeHtml(item.product_name)}</label>
                        <button type="button" class="ci-qty-btn minus" data-cart-qty-dec="${index}" aria-label="Scade cantitatea pentru ${escapeHtml(item.product_name)}"><i class="fa-solid fa-minus" aria-hidden="true"></i></button>
                        <input class="ci-qty-val" id="ci-qty-input-${index}" data-cart-qty="${index}" type="number" min="1" value="${qty}" aria-label="Cantitate ${escapeHtml(item.product_name)}">
                        <button type="button" class="ci-qty-btn plus" data-cart-qty-inc="${index}" aria-label="Crește cantitatea pentru ${escapeHtml(item.product_name)}"><i class="fa-solid fa-plus" aria-hidden="true"></i></button>
                    </div>
                    <div class="ci-price">
                        ${formatMoney(item.total_amount)}
                        ${qty > 1 ? '<span class="ci-price-unit">' + qty + ' × ' + unitPrice + '</span>' : ''}
                    </div>
                    <button type="button" class="ci-remove" data-cart-remove="${index}" title="Șterge" aria-label="Șterge ${escapeHtml(item.product_name)} din coș"><i class="fa-solid fa-xmark" aria-hidden="true"></i></button>
                </div>`;
            }).join('');
        }
    ...
    function bindMiniCartPreview() {
        ...
        document.querySelectorAll('a.cart, a[href="' + p('/cart') + '"], a[href="cart.php"]').forEach((link) => {
    ...
    function renderHomeGridCardHtml(options) {
        ...
        const image = options?.image || 'assets/images/products/product-1.jpg';
        ...
        return `${badgeHtml}<div class="_product-card-image _product-card-image--clickable">
            <img src="${escapeHtml(image)}" alt="${escapeHtml(name)}" width="300" height="225" loading="lazy" decoding="async">
        </div>
    ...
    function renderHomeVitrinaCardHtml(options) {
        ...
        return `${renderVitrinaDiscountBadgeHtml(pricing.discountPercent)}<div class="_product-card-image _product-card-image--clickable">
            <img src="${escapeHtml(image)}" alt="${escapeHtml(shortName)}" width="300" height="225" loading="lazy" decoding="async">
        </div>
        <div class="_product-card-head">
    ...
    function renderMagazinCardHtml(options) {
        ...
        const image = options?.image || 'assets/images/products/product-1.jpg';
        const priceLabel = options?.priceLabel || 'La cerere';
        const oldPriceLabel = String(options?.oldPriceLabel || '').trim();
        const productId = options?.productId || '';
        const oem = options?.oem || 'N/A';
        const badgeKey = String(options?.badge || '').trim();
        const deliveryTime = String(options?.deliveryTime || '24');
        const stockLabel = String(options?.stockLabel || options?.stock || '').trim() || 'La cerere';
        ...
        const noteBlock = !specsBlock && cardDescription
            ...
            : '';
        const priceHtml = oldPriceLabel
            ? `<div class="_product-price _product-price--sale"><span class="_product-price-old">${escapeHtml(oldPriceLabel)}</span><span class="_product-price-new">${escapeHtml(priceLabel)}</span></div>`
            : `<div class="_product-price">${escapeHtml(priceLabel)}</div>`;

        return `<div class="_product-card-head"><h3 class="_product-card-name">${escapeHtml(name)}</h3></div>
        ...
        <div class="_product-card-info">
            ...
            <div class="_product-stock">Stoc: ${escapeHtml(stockLabel)}</div>
