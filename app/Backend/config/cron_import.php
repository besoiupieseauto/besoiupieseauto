...
{
    $legacyRoot = defined('BESOIU_LEGACY') ? BESOIU_LEGACY : dirname(__DIR__, 2) . '/Legacy';
    $siteLimitPath = $legacyRoot . '/home-vitrina-render.php';
...
/** ePiesa + vitrină la import consumabile din UI (scraping lent). Dezactivat până la repararea scraperului. */
function import_consumable_epiesa_enabled(): bool
{
    $raw = strtolower(trim((string) ($_ENV['CONSUMABLE_CHECK_EPIESA'] ?? getenv('CONSUMABLE_CHECK_EPIESA') ?: '0')));

    return in_array($raw, ['1', 'true', 'yes', 'on'], true);
}

/** @param array<string, mixed> $options */
function import_consumable_wants_epiesa(array $options): bool
{
    return import_consumable_epiesa_enabled() && !empty($options['check_epiesa']);
}

/** Planuri scraper lente omise la import consumabil (evită timeout 524 ~120s). */
function import_consumable_skip_scraper_sources(): array
{
    $base = ['epiesa'];
    if (!import_consumable_epiesa_enabled()) {
        return $base;
    }
