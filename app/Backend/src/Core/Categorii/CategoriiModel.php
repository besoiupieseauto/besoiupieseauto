...
final class CategoriiModel
{
    ...
    public function findPaginated(int $page = 1, int $perPage = 10, array $filters = []): array
    {
        ...
        if (!empty($filters['q'])) {
            $whereParts[] = '(label LIKE :q OR slug LIKE :q OR meta LIKE :q_meta)';
            $params[':q'] = '%' . trim((string) $filters['q']) . '%';
            $params[':q_meta'] = '%' . trim((string) $filters['q']) . '%';
        }
        if (!empty($filters['source'])) {
            $whereParts[] = "JSON_UNQUOTE(JSON_EXTRACT(meta, '$.source')) = :source_meta";
            $params[':source_meta'] = (string) $filters['source'];
        }
        if (($filters['exclude_source'] ?? '') !== '') {
            $whereParts[] = "(meta IS NULL OR JSON_UNQUOTE(JSON_EXTRACT(meta, '$.source')) != :exclude_source_meta OR JSON_EXTRACT(meta, '$.source') IS NULL)";
            $params[':exclude_source_meta'] = (string) $filters['exclude_source'];
        }

        $where = $whereParts ? 'WHERE ' . implode(' AND ', $whereParts) : '';
        $order = !empty($filters['source']) && $filters['source'] === 'besoiu_tree'
            ? 'CAST(JSON_UNQUOTE(JSON_EXTRACT(meta, \'$.tree_id\')) AS UNSIGNED) ASC'
            : 'sort_order ASC, id ASC';

        return AdvancedCRUD::selectPaginated(self::TABLE, '*', $where, $order, $page, $perPage, $params);
    }
