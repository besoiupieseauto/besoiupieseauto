...
final class AdminUrl
{
    ...
    /** Font Awesome — unpkg (permis de CSP admin: style-src + font-src); nu folosi cdnjs.cloudflare.com. */
    public static function fontAwesomeCssUrl(string $version = '6.5.1'): string
    {
        return 'https://unpkg.com/@fortawesome/fontawesome-free@' . $version . '/css/all.min.css';
    }
    ...
    public const PAGE_ALIASES = [
        ...
        'ai-tokens' => 'ai-tokens',
        'ai-agent' => 'ai-agent',
        'product-formation' => 'produse-formare',
    ...
    /** API din pachetul unui modul (proxy /admin/api/module/{id}/{script}). */
    public static function moduleApi(string $moduleId, string $script): string
    {
        $moduleId = strtolower(trim($moduleId));
        $script = basename(trim($script));

        return self::api('module/' . rawurlencode($moduleId) . '/' . rawurlencode($script));
    }
