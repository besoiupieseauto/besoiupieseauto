...
final class AdminPermissionCatalog
{
            ...
            'produse' => [
                ...
                'features' => [
                    'produse.list' => [
                        ...
                        'urls' => ['/admin/product', '/admin/editproduse', '/admin/addproduse', '/admin/product-formation'],
                    ],
                    'produse.vitrina' => [
                    ...
                    'produse.import' => [
                        ...
                        'desc' => 'Upload și procesare fișiere Base + furnizori',
                        'urls' => ['/admin/import', '/admin/import-system', '/admin/importproduse'],
                    ],
                    ...
                    'produse.calculator' => [
                        ...
                        'desc' => 'Modul calculator: Net × TVA × Adaos',
                        'urls' => ['/admin/calculator'],
                    ],
                ],
            ],
            'comenzi' => [
            ...
            'automatizare' => [
                ...
                'features' => [
                    'automatizare.bots' => [
                        ...
                        'urls' => ['/admin/bots', '/admin/bots-pieseauto', '/admin/bots-whatsapp', '/admin/bots-facebook', '/admin/bots-baselinker', '/admin/bots-monitor'],
                    ],
                    'automatizare.ai_agent' => [
                        'label' => 'AI Agent',
                        'desc' => 'Colectare și sinteză context pentru roboți (temperature 0)',
                        'urls' => ['/admin/ai-agent', '/admin/ai-system', '/admin/api/ai_agent_endpoint.php'],
                    ],
                    'automatizare.ai_rag' => [
                        'label' => 'AI & RAG',
                        'desc' => 'Panou control AI local — module, log audit, RAG (AI propune, omul aprobă)',
                        'urls' => ['/admin/ai-rag', '/admin/api/ai_rag_endpoint.php'],
                    ],
                    'automatizare.marketplace' => [
                        'label' => 'Marketplace',
                        'desc' => 'PieseAuto.ro',
                        'urls' => ['/admin/marketplace', '/admin/marketplace-system', '/admin/marketplace-pieseauto', '/admin/marketplace-baselinker'],
                    ],
                    'automatizare.export' => [
                    ...
                    'automatizare.cron' => [
                        'label' => 'Cron Sync',
                        'desc' => 'Sincronizări programate furnizori + import',
            ...
            'comunicare' => [
                ...
                'features' => [
                    ...
                    'comunicare.messages' => [
                        ...
                        'urls' => ['/admin/messages', '/admin/messages-system'],
                    ],
                    'comunicare.templates' => [
                        'label' => 'Template-uri răspuns',
                        'desc' => 'Formulare WhatsApp, email, Facebook',
                        'urls' => ['/admin/reply-templates'],
                    ],
                    'comunicare.chat' => [
                        'label' => 'Chat — antrenare RAG',
                        'desc' => 'Q&A, butoane, reguli pentru chat widget',
                        'urls' => ['/admin/comunicare-chat'],
                    ],
                    'comunicare.channels' => [
            ...
            'analiza' => [
                ...
                'features' => [
                    'analiza.searchlogs' => [
                        ...
                        'urls' => ['/admin/searchlogs', '/admin/search-logs', '/admin/searchlogs-system'],
                    ],
                    'analiza.oem' => [
                    ...
                    'analiza.planner' => [
                        'label' => 'Planner',
                        'desc' => 'Plan eficiență financiară marketing',
                        'urls' => ['/admin/planner'],
                    ],
                    'analiza.marketing' => [
                        'label' => 'Marketing Hub',
                        'desc' => 'Rezultat → Probleme → Plan → Execuție → Dovezi',
                        'urls' => [
                            '/admin/marketing',
                            '/admin/marketing-system',
                            '/admin/marketing-indicators',
                    ...
                    'analiza.indicators' => [
                        'label' => 'Indicatori trafic',
                        'desc' => 'Redirect către Marketing Hub',
                        'urls' => ['/admin/marketing', '/admin/marketing-indicators'],
                    ],
                    'analiza.actions' => [
                        'label' => 'Plan de acțiune',
                        'desc' => 'Redirect către Marketing Hub',
                        'urls' => ['/admin/marketing', '/admin/marketing-actions'],
                    ],
                    'analiza.content' => [
                        'label' => 'Conținut & postări',
                        'desc' => 'Redirect către Marketing Hub',
                        'urls' => ['/admin/marketing', '/admin/marketing-content'],
                    ],
                ],
            ],
            'website' => [
                ...
                'features' => [
                    ...
                    'website.blog' => [
                        ...
                        'urls' => ['/admin/blog', '/admin/blog-system', '/admin/addblog', '/admin/editblog'],
                    ],
                ],
            ],
            'sistem' => [
                ...
                'features' => [
                    ...
                    'sistem.system_errors' => [
                        'label' => 'Jurnal erori sistem',
                        'desc' => 'Erori API, import, coadă — diagnostic',
                        'urls' => ['/admin/system-errors'],
                    ],
                ],
            ],
            'utilizatori' => [
    ...
    public static function allFeatures(): array
    {
        ...
        // Feature-uri din module opționale active (module.json → permissions + paths)
        if (class_exists(\Besoiu\Core\Module\ModulePermissionBridge::class)) {
            foreach (\Besoiu\Core\Module\ModulePermissionBridge::featuresFromModules() as $key => $feat) {
                if (isset($out[$key])) {
                    $out[$key]['urls'] = array_values(array_unique(array_merge(
                        $out[$key]['urls'] ?? [],
                        $feat['urls'] ?? []
                    )));
                } else {
                    $out[$key] = [
                        'label' => (string) ($feat['label'] ?? $key),
                        'desc' => (string) ($feat['desc'] ?? ''),
                        'urls' => array_values($feat['urls'] ?? []),
                    ];
                }
            }
        }
    ...
    public static function urlAllowed(string $path, array $permissions, ?string $role = null): bool
    {
        ...
        foreach (['/admin/login', '/admin/logout', '/admin/settings', '/admin/dashboard', '/admin/alerts', '/admin/system-errors'] as $always) {
    ...
    /**
     * Verifică dacă rolul curent are o permisiune granulară (feature key).
     *
     * @param mixed $permissionsRaw
     */
    public static function featureAllowed(string $featureKey, ?string $role = null, $permissionsRaw = null): bool
    {
        $role = strtolower(trim((string) ($role ?? ($_SESSION['role'] ?? 'guest'))));
        if ($role === 'super_ambassador') {
            return true;
        }

        $perms = self::normalizePermissions(
            $permissionsRaw ?? ($_SESSION['admin_permissions'] ?? null),
            $role
        );
        $expanded = self::expandToFeatureKeys($perms);

        return in_array($featureKey, $expanded, true);
    }
}
