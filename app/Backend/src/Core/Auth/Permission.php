...
/**
 * Permission – control de permisiuni / navigare / vizibilitate pe roluri.
 *
 * Exemplu rapid:
 *   $perm = new Permission($ROLES);
...
class Permission
{
    ...
    public function isRouteAllowedForRole(string $role, string $method, string $path, array $overrides = []): bool
    {
        ...
        // Pagini system (nav, workspace, setări) — acces admin autentificat
        foreach (['/admin/workspace', '/admin/workspace-switch', '/admin/settings'] as $sysPath) {
            if ($path === $sysPath || str_starts_with($path, $sysPath . '/')) {
                return true;
            }
        }

        // Pagini standard din admin_nav_routes.php (inclusiv module noi: nav, mvp-kit…)
        if ($this->isStandardAdminPagePath($path)) {
            return true;
        }
    ...
    private function isStandardAdminPagePath(string $path): bool
    {
        ...
        if ($standardPaths === null) {
            ...
            foreach ($slugs as $slug) {
                ...
                if ($slug === '' || !\Besoiu\Core\Module\ModuleGate::slugAllowed($slug)) {
                    continue;
                }
...
/** Alias compatibilitate pentru numele istoric cu typo („Permision”) */
\class_alias(\Besoiu\Core\Auth\Permission::class, \Besoiu\Core\Auth\Permision::class);
