...
final class ComenziModel
{
    ...
    public function findPaginated(int $page = 1, int $perPage = 10, array $filters = []): array
    {
        ...
        if (!empty($filters['date_from'])) {
            $whereParts[] = 'DATE(created_at) >= :date_from';
            $params[':date_from'] = (string) $filters['date_from'];
        }
        if (!empty($filters['date_to'])) {
            $whereParts[] = 'DATE(created_at) <= :date_to';
            $params[':date_to'] = (string) $filters['date_to'];
        }
        if (!empty($filters['q'])) {
            $like = '%' . trim((string) $filters['q']) . '%';
            $whereParts[] = '(client_name LIKE :q_cn OR phone LIKE :q_ph OR vin LIKE :q_vn OR order_number LIKE :q_on OR name LIKE :q_nm OR notes LIKE :q_nt OR email LIKE :q_em)';
            $params[':q_cn'] = $like;
            $params[':q_ph'] = $like;
            $params[':q_vn'] = $like;
            $params[':q_on'] = $like;
    ...
    public function getDashboardStats(): array
    {
        ...
        return [
            ...
            'new_orders' => $newTotal,
            'by_status' => $byStatus,
            'daily_trend' => $this->getDailyTrend(14),
            'revenue_trend' => $this->getRevenueTrend(14),
        ];
    }

    /**
     * Comenzi pe zi — ultimele N zile (inclusiv azi).
     *
    ...
    public function getDailyTrend(int $days = 14): array
    {
        ...
        $map = [];
        foreach ($rows as $row) {
            $map[(string) ($row['day'] ?? '')] = (int) ($row['cnt'] ?? 0);
        }

        $out = [];
        for ($i = $days - 1; $i >= 0; $i--) {
            $day = date('Y-m-d', strtotime('-' . $i . ' days'));
            $out[] = ['day' => $day, 'count' => $map[$day] ?? 0];
        }
    ...
    /**
     * Venituri pe zi — ultimele N zile.
     *
     * @return list<array{day: string, revenue: float}>
     */
    public function getRevenueTrend(int $days = 14): array
    {
        ...
        $map = [];
        foreach ($rows as $row) {
            $map[(string) ($row['day'] ?? '')] = round((float) ($row['revenue'] ?? 0), 2);
        }

        $out = [];
        for ($i = $days - 1; $i >= 0; $i--) {
            $day = date('Y-m-d', strtotime('-' . $i . ' days'));
            $out[] = ['day' => $day, 'revenue' => $map[$day] ?? 0.0];
        }
