...
if (!function_exists('normalize_amount_text')) {
    $importLib = __DIR__ . '/import_lib.php';
    if (!is_file($importLib)) {
        throw new RuntimeException('import_lib.php not found');
    }
    require_once $importLib;
}

$__productCodeNormalize = import_system_file_path('product-code-normalize.php');
if ($__productCodeNormalize === null) {
    throw new RuntimeException('product-code-normalize.php not found');
}
require_once $__productCodeNormalize;
unset($__productCodeNormalize);
...
function import_price_logic_service(): ?object
{
    if (
        !class_exists(\Besoiu\Core\Module\OptionalModuleBridge::class)
        || !\Besoiu\Core\Module\OptionalModuleBridge::furnizoriAvailable()
    ) {
        return null;
    }
    ...
    $service = \Besoiu\Core\Supplier\SupplierHooks::priceLogic();
    if ($service !== null) {
        $GLOBALS['__import_price_logic_service'] = $service;
    }
...
{
    $priorities = array_merge(import_supplier_priority_map_defaults(), import_supplier_priority_map());

    return [
        'AUTOTOTAL' => [
            'name' => 'Autototal',
            'priority' => $priorities['AUTOTOTAL'] ?? 99,
        ...
        'AUTONET' => [
            'name' => 'Autonet',
            'priority' => $priorities['AUTONET'] ?? 99,
            'vat_rule' => 'net_plus_tva',
        ...
        'MATEROM' => [
            'name' => 'Materom',
            'priority' => $priorities['MATEROM'] ?? 99,
            'vat_rule' => 'net_plus_tva',
            'price_columns' => 'Pret',
            'connection_type' => 'ftp',
        ],
        ...
        'INTERCARS' => [
            'name' => 'Inter Cars',
            'priority' => $priorities['INTERCARS'] ?? 99,
        ...
        'AUTOPARTNER' => [
            'name' => 'Auto Partner',
            'priority' => $priorities['AUTOPARTNER'] ?? 99,
            'vat_rule' => 'net_plus_tva',
...
function import_furnizori_catalog(): array
{
    ...
    try {
        $modelPath = dirname(__DIR__) . '/Core/Supplier/SupplierCatalogRepository.php';
        if (is_file($modelPath)) {
            require_once $modelPath;
            $model = new \Besoiu\Core\Supplier\SupplierCatalogRepository();
...
function import_furnizor_is_active(string $supplierCode): bool
{
...
/** @return array<string, float> Valori brute din DB (fără rezolvare defaults / return10). */
function import_supplier_feed_markup_stored_map(): array
{
    if (is_array($GLOBALS['__import_supplier_feed_markup_stored'] ?? null)) {
        return $GLOBALS['__import_supplier_feed_markup_stored'];
    }
        ...
        try {
            ...
            if ($stmt !== false) {
                while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                    ...
                    if ($code === '' || (string) ($row['price_markup_type'] ?? 'percentage') !== 'percentage') {
                        continue;
                    }
    ...
    $GLOBALS['__import_supplier_feed_markup_stored'] = $cache;
...
function import_supplier_feed_markup_map(): array
{
    ...
    foreach (array_keys($cache) as $code) {
        $cache[$code] = import_supplier_feed_markup_percent($code);
    }
    foreach (array_keys(import_supplier_feed_markup_stored_map()) as $code) {
        $cache[$code] = import_supplier_feed_markup_percent($code);
    }
...
{
    unset($GLOBALS['__import_supplier_feed_markup_map'], $GLOBALS['__import_supplier_feed_markup_stored']);
}

/**
 * Compensator pre-import — aceeași logică ca în FurnizoriStatsService (profil furnizor).
 */
function import_supplier_resolve_feed_markup_percent(string $supplier, ?float $storedValue = null): float
{
    ...
    if ($storedValue === null) {
        $storedMap = import_supplier_feed_markup_stored_map();
        if (array_key_exists($code, $storedMap)) {
            $storedValue = (float) $storedMap[$code];
        } else {
            $storedValue = (float) (import_supplier_feed_markup_defaults()[$code] ?? 0.0);
        }
    }

    $isReturn10Supplier = in_array($code, import_supplier_return10_codes(), true);
    if ($isReturn10Supplier) {
        return $storedValue > 0.0001 ? (float) $storedValue : 0.0;
    }

    if ($storedValue <= 0.0001) {
...
function import_supplier_feed_markup_percent(string $supplier): float
{
    ...
    $storedMap = import_supplier_feed_markup_stored_map();
    $stored = array_key_exists($code, $storedMap) ? (float) $storedMap[$code] : null;

    return import_supplier_resolve_feed_markup_percent($code, $stored);
}
...
function import_supplier_commercial_vat_percent(): float
{
    if (class_exists(\Besoiu\Services\AdaosComercial\AdaosComercialService::class)) {
        try {
            return (new \Besoiu\Services\AdaosComercial\AdaosComercialService())->getCommercialVatPercent();
        } catch (\Throwable $e) {
            // fallback
        }
    }
...
function import_supplier_vat_rule(string $supplier): string
{
    $code = import_supplier_normalize_code($supplier);
    if ($code === '') {
        return 'net_plus_tva';
    ...
    $defs = import_supplier_definitions();

    return (string) ($defs[$code]['vat_rule'] ?? 'net_plus_tva');
}

/**
 * Formare preț inițial Import Pro / profil furnizor: CSV → compensator → achiziție → TVA (dacă regula o cere).
 *
 * @return array{
 *   price_csv: ?float,
 *   price_purchase_net: ?float,
 *   price_purchase_vat: ?float,
 *   feed_markup_percent: float,
 *   feed_markup_type: string,
 *   vat_rule: string,
 *   vat_percent: float,
 *   vat_extra_applied: bool
 * }
 */
function import_supplier_form_initial_purchase_prices(mixed $csvPriceRaw, string $supplier): array
{
    $code = import_supplier_normalize_code($supplier);
    $priceCsv = import_parse_supplier_price($csvPriceRaw);
    $feedPct = $code !== '' ? import_supplier_feed_markup_percent($code) : 0.0;
    $purchaseNet = null;

    if ($priceCsv !== null) {
        $purchaseNet = $code !== ''
            ? import_supplier_apply_feed_markup($priceCsv, $code)
            : round($priceCsv, 4);
    }

    $vatRule = import_supplier_vat_rule($code);
    $vatPct = import_supplier_commercial_vat_percent();
    $purchaseVat = null;
    $vatExtra = false;

    if ($purchaseNet !== null && $purchaseNet > 0) {
        $base = import_supplier_net_to_base($purchaseNet, $code);
        if ($vatRule === 'price_final') {
            $purchaseVat = round($base, 2);
        } else {
            $vatExtra = true;
            $purchaseVat = (float) ceil($base * (1 + ($vatPct / 100)));
        }
        $purchaseNet = $base;
    }

    return [
        'price_csv' => $priceCsv !== null ? round($priceCsv, 2) : null,
        'price_purchase_net' => $purchaseNet !== null ? round($purchaseNet, 2) : null,
        'price_purchase_vat' => $purchaseVat,
        'feed_markup_percent' => $feedPct,
        'feed_markup_type' => 'percentage',
        'vat_rule' => $vatRule,
        'vat_percent' => $vatPct,
        'vat_extra_applied' => $vatExtra,
    ];
}
 ...
 */
function import_supplier_apply_initial_prices_to_card(array $card, string $supplierCode = ''): array
{
    if ($supplierCode === '') {
        foreach (['sourceSupplier', 'supplier'] as $key) {
            if (!empty($card[$key])) {
                $supplierCode = import_supplier_normalize_code((string) $card[$key]);
                if ($supplierCode !== '') {
                    break;
                }
            }
        }
        if ($supplierCode === '' && !empty($card['sourceFile'])) {
            $parts = explode('/', (string) $card['sourceFile'], 2);
            $supplierCode = import_supplier_normalize_code($parts[0] ?? '');
        }
    }

    $csvRaw = null;
    if (isset($card['priceCsv']) && is_numeric($card['priceCsv'])) {
        $csvRaw = $card['priceCsv'];
    } elseif (!empty($card['price_feed_markup_applied']) || !empty($card['priceFeedMarkupApplied'])) {
        $purchase = (float) ($card['pricePurchaseNet'] ?? $card['priceNet'] ?? 0);
        if ($purchase > 0 && $supplierCode !== '') {
            $pct = import_supplier_feed_markup_percent($supplierCode);
            $csvRaw = import_supplier_reverse_feed_csv_price(
                import_supplier_net_to_base($purchase, $supplierCode),
                $pct
            );
        }
    } else {
        $csvRaw = $card['priceNet'] ?? $card['price'] ?? null;
    }

    $formed = import_supplier_form_initial_purchase_prices($csvRaw, $supplierCode);

    if ($formed['price_purchase_vat'] === null) {
        $card['priceCsv'] = $formed['price_csv'];
        $card['pricePurchaseNet'] = null;
        $card['pricePurchaseVat'] = null;
        unset($card['priceFinal'], $card['priceRecommended']);

        return $card;
    }

    $card['priceCsv'] = $formed['price_csv'];
    $card['pricePurchaseNet'] = $formed['price_purchase_net'];
    $card['priceNet'] = $formed['price_purchase_net'];
    $card['pricePurchaseVat'] = fmod((float) $formed['price_purchase_vat'], 1.0) === 0.0
        ? (int) $formed['price_purchase_vat']
        : $formed['price_purchase_vat'];
    $card['feedMarkupPercent'] = $formed['feed_markup_percent'];
    $card['purchaseVatPercent'] = $formed['vat_percent'];
    $card['supplierVatRule'] = $formed['vat_rule'];
    $card['purchaseVatExtraApplied'] = $formed['vat_extra_applied'];
    $card['priceFeedMarkupApplied'] = true;
    unset($card['priceFinal'], $card['priceRecommended']);
...
if (!function_exists('import_strip_bom')) {
...
function import_stream_intercars_txt(
    string $path,
    callable $rowHandler,
    ?bool &$stop = null,
    int $skipRows = 0,
...
function import_stream_supplier_file(
    string $path,
    string $filename,
    string $supplierType,
    callable $rowHandler,
    ...
    if ($supplierType === 'INTERCARS' && str_ends_with(strtolower($filename), '.txt')) {
        return import_stream_intercars_txt($path, $rowHandler, $stop, $skipRows, $maxRows);
    }
    ...
    $processValues = static function (array $values) use (
        &$count,
        &$rowsSeen,
        $rowHandler,
        $supplierType,
    ...
    $processRow = static function (array $row, string $type) use (
        &$count,
        &$rowsSeen,
        $rowHandler,
        $skipRows,
...
function import_apply_supplier_pricing(array $product, array $priceIndex, ?\Besoiu\Services\AdaosComercial\AdaosComercialService $markupService = null): array
{
    if ($priceIndex === []) {
        return $product;
    }
    ...
    // Fără markupService: păstrăm doar achiziția (pBasePrice). pPrice rămâne pentru Adaos comercial.
