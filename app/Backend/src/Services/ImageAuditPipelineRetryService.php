...
final class ImageAuditPipelineRetryService
{
    ...
    public function retryForProductIds(PDO $pdo, array $publicIds, array $options = []): array
    {
        ...
        foreach ($publicIds as $rid) {
            ...
            if (!$force && $audit === null && $this->hasTrustedPipelineImage($row)) {
                ++$skipped;
                $out[] = [
                    'randomn_id' => $rid,
                    'status' => 'skipped',
                    'message' => 'Imagine deja din pipeline validat — fără verdict audit mismatch.',
                    'audit_verdict' => '',
                    'audit_score' => 0,
                ];
                continue;
    ...
    public function resolveProductImage(array $row, bool $force = true): array
    {
        ...
        return ScraperModule::instance()->resolveProductImage($row, [
            'categories' => $cats,
            'force' => $force,
            'test_mode' => true,
            'skip_vision' => true,
    ...
    private function bootstrapPipeline(): void
    {
        ...
        require_once $this->projectRoot . '/lib/Scraper/ScraperModule.php';
        ScraperModule::instance();
    }
    ...
    /** @param array<string, mixed> $row */
    private function hasTrustedPipelineImage(array $row): bool
    {
        $path = $this->projectRoot . '/system/import-image-validate.php';
        if (is_file($path)) {
            require_once $path;
        }

        if (!function_exists('besoiu_import_row_image_url') || !function_exists('besoiu_import_image_url_is_trusted')) {
            return false;
        }
        ...
        $source = strtolower(trim((string) ($row['pImageSource'] ?? '')));
