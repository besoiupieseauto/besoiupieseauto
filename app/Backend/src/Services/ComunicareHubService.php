...
use Besoiu\Core\Comunicare\ReplyTemplateModel;
use Besoiu\Core\Comunicare\ReplyTemplateRenderer;
...
final class ComunicareHubService
{
    ...
    /** @return list<array<string, mixed>> */
    public function listRecentMessages(int $limit = 12): array
    {
        $limit = max(1, min(30, $limit));
        try {
            $pdo = \Config\Database::getDB();
            $stmt = $pdo->query(
                "SELECT randomn_id, name, email, phone, subject, message_body, channel, is_read, direction, created_at
                 FROM messages
                 ORDER BY id DESC
                 LIMIT {$limit}"
            );
            ...
            foreach ($rows as $row) {
                ...
                if (mb_strlen($body, 'UTF-8') > 80) {
                    $body = mb_substr($body, 0, 77, 'UTF-8') . '…';
                }
                ...
                $out[] = [
                    ...
                    'channel' => trim((string) ($row['channel'] ?? '')) ?: 'manual',
                    'subject' => trim((string) ($row['subject'] ?? '')) ?: ($body !== '' ? $body : '-'),
                    'message_preview' => $body !== '' ? $body : '-',
                    'read_label' => $isRead ? 'Citit' : 'Necitit',
                    'direction' => trim((string) ($row['direction'] ?? '')) ?: '-',
                    'created_at' => $created !== '' ? $created : '-',
                    'edit_url' => $rid > 0 ? '/admin/messages?highlight=' . $rid : '/admin/messages',
                ];
            }
    ...
    {
        return [
            ['title' => 'Chat mesagerie', 'desc' => 'Inbox unificat WhatsApp, site, email.', 'url' => '/admin/messages', 'icon' => 'messages-square'],
            ['title' => 'Chat — antrenare RAG', 'desc' => 'Q&A, butoane, reguli — învață widget-ul să răspundă eficient.', 'url' => '/admin/comunicare-chat', 'icon' => 'bot'],
            ['title' => 'Marker DOM', 'desc' => 'Click pe elemente pagină → descriere → RAG chat.', 'url' => '/admin/comunicare-chat?tab=marker', 'icon' => 'crosshair'],
            ...
            ['title' => 'Canale active', 'desc' => 'Statistici per WhatsApp, Facebook, PieseAuto.ro.', 'url' => '/admin/comunicare-canale', 'icon' => 'share-2'],
