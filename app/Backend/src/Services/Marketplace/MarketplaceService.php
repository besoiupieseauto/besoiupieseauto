...
namespace Besoiu\Services\Marketplace;
...
use Besoiu\Services\Marketplace\BaseLinkerImportLimits;
use Besoiu\Services\Marketplace\BaseLinkerProductMapper;
...
final class MarketplaceService
{
    ...
    public function getBaseLinkerStoreImportInfo(): array
    {
        ...
        $report = \Besoiu\Services\Marketplace\BaseLinkerStoreImportInvestigation::report($activeProducts);

        return [
            'shop' => $shopInfo,
            'investigation' => $report,
            'support_ticket' => \Besoiu\Services\Marketplace\BaseLinkerStoreImportInvestigation::buildSupportTicket(
                $shopInfo,
                $activeProducts
            ),
    ...
    private function loadOrderForSync(int $orderRandomId): ?array
    {
        ...
        $comenziModel = new \Besoiu\Core\Comenzi\ComenziModel();
        $order = $comenziModel->findByRandomId($orderRandomId);
        if ($order === null) {
            return null;
        }

        $orderItemsModel = new \Besoiu\Core\Comenzi\OrderItemsModel();
        $dbId = (int) ($order['id'] ?? 0);
