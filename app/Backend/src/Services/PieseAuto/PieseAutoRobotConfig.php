...
final class PieseAutoRobotConfig
{
    ...
    public static function robotReachableFromPhp(bool $allowHttpProbe = true): bool
    {
        if (self::tunnelUrl() !== '') {
            return true;
        }
        ...
        $listener = self::readListenerFile();
        if ($listener !== null && self::listenerLooksFresh($listener)) {
            return true;
        }
    ...
    public static function adminJsConfig(bool $fastPageLoad = true): array
    {
        $port = self::listenerPortFromDisk();
        if ($fastPageLoad) {
