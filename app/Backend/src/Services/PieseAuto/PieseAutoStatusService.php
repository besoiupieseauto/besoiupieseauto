...

{
...

    {
        ...
        $canProbe = $liveProbe && PieseAutoRobotConfig::robotReachableFromPhp(true);

        $ping = $this->resolvePing($listener, $runtime, $port, $serviceUrl, $canProbe);
        ...
        if ($robotMessage === '' && !$serviceOnline) {
            $robotMessage = $liveProbe
                ? PieseAutoRobotConfig::robotAccessHint()
                : (is_file(PieseAutoRobotConfig::runtimeListenerPath())
                    ? 'Verificare status robot…'
                    : 'Pornește robot\\start_pieseauto_visible.bat');
        }
        ...
        return [
            ...
            'access_hint' => $liveProbe ? PieseAutoRobotConfig::robotAccessHint() : '',
    ...
    private function resolvePing(?array $listener, ?array $runtime, int $port, string $serviceUrl, bool $doHttpPing): array
    {
        ...
        /* Încărcare pagină: fără curl pe porturi — status live vine din JS (/api/pieseauto_status). */
        if (!$doHttpPing) {
            $diskOnline = $listener !== null && PieseAutoRobotConfig::listenerLooksFresh($listener);
