...
use Besoiu\Services\Fulfillment\OrderFulfillmentService;
use Besoiu\Services\Products\ProductLegacyBridgeService;
use PDO;
use PDOException;
...
final class ComenziService
{
    ...
    private function resolveWebsiteLines(array $rawItems): array
    {
        ...
        foreach ($rawItems as $rawItem) {
            ...
            $randomId = strtolower(trim((string) ($rawItem['randomn_id'] ?? $rawItem['product_id'] ?? '')));
            ...
            if (!preg_match('/^[a-f0-9]{16}$/', $randomId)) {
                throw new ValidationException(
                    'ID produs invalid pentru magazin. Folosește randomn_id (catalog site), nu id numeric sau idprodus ERP.'
                );
            }

            $product = $this->produseModel->findByStorefrontPublicId($randomId);
    ...
    /**
     * Cotatie checkout — prețuri din BD fără a crea comandă (#42).
     *
     * @param array<int, array<string, mixed>> $rawItems
     * @return array{lines: list<array<string, mixed>>, total_amount: float, quantity: int}
     */
    public function quoteWebsiteCart(array $rawItems): array
    {
        $resolvedLines = $this->resolveWebsiteLines($rawItems);
        if ($resolvedLines === []) {
            throw new ValidationException('Cosul nu conține produse valide.');
        ...
        foreach ($resolvedLines as $line) {
            ...
            $totalQuantity += (int) $line['quantity'];
            $publicLines[] = [
                'randomn_id' => (string) ($line['randomn_id'] ?? ''),
                'product_name' => (string) ($line['product_name'] ?? ''),
                'product_image' => (string) ($line['product_image'] ?? ''),
    ...
    public function listOrders(array $params = []): array
    {
        ...
        foreach ($items as $index => $item) {
            ...
            $items[$index]['order_items'] = $this->attachLegacyBridgeToLines($lines);
            $items[$index]['items_count'] = count($lines);
            $items[$index]['order_universe'] = 'site_besoiu';
        }

        $result['items'] = $items;
        $result['_meta'] = $this->orderListMeta();
    ...
    /** @return array<string, mixed> */
    private function orderListMeta(): array
    {
        $legacyConfigured = false;
        try {
            $cfg = require dirname(__DIR__, 3) . '/config/config.php';
            $legacyConfigured = trim((string) ($cfg['legacy_db_name'] ?? '')) !== '';
        } catch (\Throwable) {
            $legacyConfigured = false;
        }

        return [
            'order_universe' => 'site_besoiu',
            'product_public_id' => 'randomn_id',
            'erp_product_id' => 'idprodus',
            'erp_orders_note' => 'Comenzile TM/UTVIN/externe sunt în LEGACY_DB (Caiet Laravel) — tab separat în admin Comenzi.',
            'legacy_db_configured' => $legacyConfigured,
        ];
    }
     ...
     * @return array<int, array<string, mixed>>
     */
    private function attachLegacyBridgeToLines(array $lines): array
    {
        if ($lines === []) {
            return $lines;
        }
        ...
        try {
            $bridge = new ProductLegacyBridgeService();
            if (!$bridge->isLegacyAvailable()) {
                return $lines;
            }
        } catch (\Throwable) {
            return $lines;
        ...
        foreach ($lines as $index => $line) {
            ...
            $publicId = strtolower(trim((string) ($line['randomn_id'] ?? '')));
            if ($publicId === '' || !preg_match('/^[a-f0-9]{16}$/', $publicId)) {
                continue;
            }
            $product = $this->produseModel->findByStorefrontPublicId($publicId);
            if ($product === null) {
                continue;
            }
            $legacy = $bridge->findLegacyMatch($product);
            if ($legacy === null) {
                continue;
            }
            $lines[$index]['legacy_bridge'] = [
                'idprodus' => (int) ($legacy['idprodus'] ?? 0),
                'code' => trim((string) ($legacy['cod_produs'] ?? $legacy['pCode'] ?? '')),
                'label' => 'ERP legacy (idprodus)',
            ];
        }
