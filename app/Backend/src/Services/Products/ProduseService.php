...
class ProduseService
{
     ...
     */
    public function getProdusesPaginated(int $page = 1, int $perPage = 10, array|string|null $filters = null): array
    {
        // Compatibilitate cu apelurile vechi care trimiteau doar "no_image".
        if (is_string($filters)) {
            $filters = $filters === 'no_image' ? ['image' => 'missing'] : [];
        }
        return $this->model->paginatedAdminList($page, $perPage, $filters ?? []);
    }

    /** @param array<string,string> $filters @return array{all:int,site:int,export:int} */
    public function countAdminListByOrigin(array $filters = []): array
    {
        return $this->model->countAdminListByOrigin($filters);
    }

    /** @param array<string,string> $filters @return array{ids:array<int,string>,last_id:?int} */
    public function listRandomIdsByFilters(array $filters, int $limit = 200, ?int $beforeId = null): array
    {
        return $this->model->listRandomIdsByFilters($filters, $limit, $beforeId);
    }

    /** @param array<string,string> $filters */
    public function countAdminList(array $filters = []): int
    {
        return $this->model->countAdminList($filters);
    }
    ...
    /** Actualizează badge-ul pe toate produsele active din catalog. */
    public function setBadgeOnAllActive(string $badge): int
    {
        return $this->model->setBadgeOnAllActive($badge);
    }
    ...
    public function vitrinaHomepageMax(): int
    {
        ...
        $legacyRoot = defined('BESOIU_LEGACY') ? BESOIU_LEGACY : dirname(__DIR__, 4) . '/app/Legacy';
        $path = $legacyRoot . '/home-vitrina-render.php';
        if (is_file($path)) {
            require_once $path;
        }
    ...
    public function ensureProductForVitrina(string $id): ?string
    {
        ...
        $legacyRoot = defined('BESOIU_LEGACY') ? BESOIU_LEGACY : $siteRoot . '/app/Legacy';
        $scraperHome = $legacyRoot . '/scraper-home.php';
