...
final class SupplierScanDashboardService
{
    ...
    public function buildCronDashboard(array $suppliers = []): array
    {
        ...
        foreach ($suppliers as $row) {
            ...
            $isDue = SupplierHooks::scanScheduleShouldRunAuto($row, $agent, $now);
            if ($isDue) {
                ++$dueCount;
            }

            $nextAt = SupplierHooks::scanScheduleEstimateNextRunAt($row, $agent, $now);
            ...
            $supplierJobs[] = [
                ...
                'schedule_label' => (string) ($row['scan_schedule_label'] ?? SupplierHooks::scanScheduleFormatLabel($row)),
                'progress' => $progress,
                'status' => $status,
                ...
                'next_scan_label' => SupplierHooks::scanScheduleFormatNextRunLabel($row, $agent, $now),
                'is_due' => $isDue,
    ...
    public function log(
        ...
        if (!function_exists('import_live_trace_from_cron')) {
            $traceLib = dirname(__DIR__, 2) . '/Controllers/Produse/import_live_trace_lib.php';
            if (is_file($traceLib)) {
                require_once $traceLib;
            }
        }
        if (function_exists('import_live_trace_from_cron')) {
            import_live_trace_from_cron($entry);
        }
    }
    ...
    public function updateProgress(array $data): void
    {
        ...
        $oldMessage = trim((string) ($current['message'] ?? ''));
        if ($newMessage !== '' && $newMessage !== $oldMessage && !empty($payload['running'])) {
            if (preg_match('/^Import\s+[A-Z0-9]+:\s*produs\s+\d+/i', $newMessage) === 1) {
                if (!function_exists('import_live_trace_parse_import_progress')) {
                    $traceLib = dirname(__DIR__, 2) . '/Controllers/Produse/import_live_trace_lib.php';
                    if (is_file($traceLib)) {
                        require_once $traceLib;
                    }
                }
                if (!function_exists('import_live_trace_pipeline')) {
                    $stepsLib = dirname(__DIR__, 2) . '/Controllers/Produse/import_live_pipeline_steps_lib.php';
                    if (is_file($stepsLib)) {
                        require_once $stepsLib;
                    }
                }
                $parsed = function_exists('import_live_trace_parse_import_progress')
                    ? import_live_trace_parse_import_progress($newMessage)
                    : null;
                if ($parsed !== null && function_exists('import_live_trace_pipeline')) {
                    $brand = trim((string) ($payload['product_brand'] ?? ''));
                    $code = $parsed['code'];
                    $productKey = $brand !== '' && $code !== ''
                        ? $brand . ' · ' . $code
                        : ($code !== '' ? $code : $parsed['product_key']);
                    import_live_trace_pipeline(9, [
                        'supplier' => $parsed['supplier'],
                        'product_key' => $productKey,
                        'product_code' => $code,
                        'query' => $code,
                        'result' => $parsed['idx'] . '/' . $parsed['total']
                            . ($code !== '' ? ' — ' . $code : ''),
                        'level' => 'step',
                    ]);
                }
            ...
            if (!function_exists('import_live_trace_from_progress')) {
                $traceLib = dirname(__DIR__, 2) . '/Controllers/Produse/import_live_trace_lib.php';
                if (is_file($traceLib)) {
                    require_once $traceLib;
                }
            ...
            if (function_exists('import_live_trace_from_progress')) {
                import_live_trace_from_progress($payload);
            }
        }
    }
