...
namespace Besoiu\Services\AdaosComercial;

use Config\Database;
use Besoiu\Core\AdaosComercial\AdaosComercialModel;
use Besoiu\Core\Supplier\SupplierPriceLogicStore;
...
final class AdaosComercialService
{
    ...
    /**
     * Calculează prețul pe baza procesată (pBasePrice = CSV + compensator furnizor).
     * Lanț: preț bază → TVA magazin → adaos comercial (global + prima regulă potrivită) → rotunjire.
     */
    public function applyAutomaticMarkup(
        array $payload,
        ?array $existingProduct = null,
        bool $matchConditionalRules = true,
    ...
    public function reapplyGlobalMarkupToAll(): array
    {
        ...
        $totalDelta = 0.0;
        $rulesById = [];
        foreach ($this->model->findAll() as $rule) {
            $rulesById[(int) ($rule['id'] ?? 0)] = $rule;
        }
        $this->warmPricingConfiguration();
        ...
        try {
            $afterId = 0;
            do {
                $products = $this->produseModel->markupChunk($afterId, 500);
                $updates = [];
                foreach ($products as $product) {
                    $afterId = max($afterId, (int) ($product['id'] ?? 0));
                    $totalCount++;
                    ...
                    $conditionalRule = $ruleId > 0 ? ($rulesById[$ruleId] ?? null) : null;
                    $update = $this->buildMarkupUpdate($product, $basePrice, $conditionalRule, false);
                    if ($update === null) {
                        continue;
                    }
                    $updates[] = $update['row'];
                    $totalDelta += $this->parseAmount($update['row']['pPrice'] ?? '')
                        - $this->parseAmount($product['pPrice'] ?? '');
                }
                $updatedCount += $this->produseModel->updateMarkupRows($updates);
            ...
            foreach ($products as $product) {
                ...
                if (count($preview) < $limit) {
                    $preview[] = [
                        ...
                        'name' => (string)($product['pName'] ?? 'Produs fără nume'),
        ...
        try {
            ...
            do {
                ...
                foreach ($products as $product) {
                    ...
                    $update = $this->buildMarkupUpdate($product, $basePrice, $rule, true);
                    if ($update === null) {
                        continue;
                    }
                    $updates[] = $update['row'];
    ...
    public function applyRuleToProductIds(int $ruleId, array $productIds): array
    {
        ...
        try {
            foreach (array_chunk($productIds, 400) as $idChunk) {
                $products = $this->produseModel->findMany($idChunk, [
                    'id', 'randomn_id', 'pBasePrice', 'pPrice', 'pMarkupRuleId',
                    'pMarkupRuleName', 'pMarkupAppliedAt',
                ]);
                $notFoundCount += max(0, count($idChunk) - count($products));
                $updates = [];
                foreach ($products as $product) {
    ...
    /**
     * Reaplică potrivirea automată pe o selecție, cu citire și update set-based.
     *
     * @param list<string> $productIds
     * @return array{requested_count:int,updated_count:int,not_found_count:int}
     */
    public function reapplyAutomaticMarkupToProductIds(array $productIds): array
    {
        $productIds = array_values(array_unique(array_filter(array_map('strval', $productIds))));
        $pdo = Database::getDB();
        $updated = 0;
        ...
        try {
            foreach (array_chunk($productIds, 400) as $idChunk) {
                $products = $this->produseModel->findMany($idChunk, [
                    'id', 'randomn_id', 'pCategory', 'pSubcategory', 'pBrand', 'pMarca',
                    'pBasePrice', 'pPrice', 'pMarkupRuleId', 'pMarkupRuleName', 'pMarkupAppliedAt',
                ]);
                $found += count($products);
                $updates = [];
                foreach ($products as $product) {
                    $pricing = $this->applyAutomaticMarkup($product, $product, true);
                    $data = $pricing['data'];
                    $updates[] = [
                        'id' => (int) $product['id'],
                ...
                $updated += $this->produseModel->updateMarkupRows($updates);
            }
            $pdo->commit();
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) {
        ...
        return [
            ...
            'not_found_count' => max(0, count($productIds) - $found),
        ];
    }

    /**
     * @return array{row:array<string,mixed>,delta:float,price_changed:bool}|null
     */
    private function buildMarkupUpdate(
        array $product,
        float $basePrice,
        ?array $rule,
        bool $refreshAppliedAt
    ): ?array {
        $finalPrice = $this->computeStorePrice($basePrice, $rule);
        $targetBase = $this->formatAmount($basePrice);
        $targetPrice = $this->formatAmount($finalPrice);
        $targetRuleId = $rule !== null ? ((int) ($rule['id'] ?? 0) ?: null) : null;
        $targetRuleName = $rule !== null ? (string) ($rule['name'] ?? '') : '';
        ...
        $needsRuleUpdate = $currentRuleId !== $targetRuleId
            ...
            || ($refreshAppliedAt && $currentAppliedAt === '');
        ...
        return [
            'row' => [
                ...
                'pMarkupAppliedAt' => $rule !== null && ($refreshAppliedAt || $currentAppliedAt === '')
                    ? date('Y-m-d H:i:s')
                    : ($product['pMarkupAppliedAt'] ?? null),
            ],
            'delta' => $finalPrice - $basePrice,
            'price_changed' => abs($finalPrice - $basePrice) >= 0.00001,
        ];
    }

    /**
     * Încarcă setările înainte de tranzacție. SupplierPriceLogicStore poate inițializa
     * schema la prima utilizare, iar DDL-ul MySQL ar face implicit COMMIT.
     */
    private function warmPricingConfiguration(): void
    {
        $this->getGlobalCommercialMarkupPercent();
        $this->getCommercialVatPercent();
        $this->getGlobalPriceRoundSettings();
    }
    ...
    public function computeCommercialBreakdown(float $basePrice, ?array $rule): array
    {
        ...
        $afterVat = $this->applyCommercialVat($basePrice);
        $vatDelta = max(0.0, $afterVat - $basePrice);
        $globalDelta = $this->computeGlobalCommercialMarkupDelta($afterVat);
        $conditionalDelta = $rule !== null ? $this->computeRuleMarkupDelta($afterVat, $rule) : 0.0;
        $afterMarkup = $afterVat + $globalDelta + $conditionalDelta;
        ...
        $markupDelta = $afterMarkup - $afterVat;
        $final = $this->applyGlobalPriceRound($afterMarkup);
        $roundDelta = max(0.0, $final - $afterMarkup);
    ...
    public function getCommercialVatPercent(): float
    {
        ...
        $model = new SupplierPriceLogicStore();
    ...
    public function computeStorePrice(float $basePrice, ?array $conditionalRule = null): float
    {
        ...
        $conditionalDelta = $conditionalRule !== null
            ? $this->computeRuleMarkupDelta($afterVat, $conditionalRule)
            : 0.0;
        $afterMarkup = $afterVat + $globalDelta + $conditionalDelta;
        ...
        return $this->applyGlobalPriceRound($afterMarkup);
    }
