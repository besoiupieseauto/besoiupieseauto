...
/**
 * Sincronizează modele vehicul (type=model) din TecDoc în categorii.
 *
 * Usage:
 *   php app/Backend/tools/sync_tecdoc_modele.php preview [--db-only]
 *   php app/Backend/tools/sync_tecdoc_modele.php sync [--db-only]
 */
...
if ($action === 'preview') {
    $preview = $service->previewTecdocModele(!$dbOnly);
...
if ($action === 'sync') {
    try {
        $result = $service->syncTecdocModele(!$dbOnly, true);
