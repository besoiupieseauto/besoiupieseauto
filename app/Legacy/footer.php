...
if ($footerPhoneHref === '' && $footerPhone !== '') {
    ...
    $footerPhoneHref = besoiu_store_phone_tel();
}
    ...
    <div class="container">
        <div class="footer-main">
            <div class="footer-block footer-brand">
                <a class="logo" href="/">
                    <img src="img/logo.png" alt="Besoiu Piese Auto" class="logo-img" width="125" height="42" loading="lazy" decoding="async">
                </a>
                ...
                <div class="social" id="footer-social-links">
                    <?php foreach ($footerSocial as $si => $socialItem): ?>
                        ...
                        ?>
                    <a href="<?= site_cms_h(besoiu_normalize_href($socialHref)) ?>" aria-label="<?= site_cms_h($socialLabel) ?>" target="_blank" rel="noopener noreferrer"><?php if (function_exists('site_live_cms_image_tag')): ?><?php site_live_cms_image_tag('global', 'footer.social.' . $si . '.icon', $socialIcon, ['class' => 'footer-social-icon', 'alt' => '', 'role' => 'presentation', 'width' => '16', 'height' => '16', 'loading' => 'lazy', 'data-cms-variant' => 'icon']); ?><?php else: ?><img src="<?= site_cms_h($socialIcon) ?>" alt="" class="footer-social-icon" role="presentation" width="16" height="16" loading="lazy"><?php endif; ?></a>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="footer-block footer-contact">
                <h4>Contact</h4>
                <ul>
                    <li><img src="img/icons/12_telefon.svg" alt="" class="footer-contact-icon" role="presentation" width="14" height="14" loading="lazy"><?php if ($footerPhone !== ''): ?><a href="<?= site_cms_h($footerPhoneHref) ?>"><?= site_cms_h($footerPhone) ?></a><?php endif; ?></li>
                    <li><img src="img/icons/29_email_plic.svg" alt="" class="footer-contact-icon" role="presentation" width="14" height="14" loading="lazy"><?php if ($footerEmail !== '' && $footerEmailHref !== ''): ?><a href="<?= site_cms_h($footerEmailHref) ?>"><?= site_cms_h($footerEmail) ?></a><?php endif; ?></li>
                    <li><img src="img/icons/28_locatie_pin.svg" alt="" class="footer-contact-icon" role="presentation" width="14" height="14" loading="lazy"><?= site_cms_h($footer['address'] ?? '') ?></li>
                </ul>
            </div>
        </div>
        <div class="copy">
...
if (besoiu_admin_storefront_context() || dom_marker_mode_active()) {
    dom_marker_render_assets();
    dom_marker_render_fab();
}
if (dom_marker_mode_active()) {
    require __DIR__ . '/dom-marker-bar.php';
}
if (!function_exists('besoiu_admin_storefront_context')) {
    require_once __DIR__ . '/storefront-context.php';
}
