...
function search_log_write(
    ...
    if ($hasMeta) {
        ...
        search_log_ai_action_event($queryType, $queryValue, $found, $vehicleLabel, $resultCount, $meta);
    ...
    search_log_ai_action_event($queryType, $queryValue, $found, $vehicleLabel, $resultCount, null);
}

function search_log_ai_action_event(
    string $queryType,
    string $queryValue,
    bool $found,
    ?string $vehicleLabel,
    ...
    if (!$loaded) {
        ...
        $learning = __DIR__ . '/ai_learning.php';
        if (is_file($learning)) {
            require_once $learning;
        }
        $loaded = true;
    }

    $meta = is_array($meta) ? $meta : [];
    if (function_exists('ai_learning_record_search')) {
        ai_learning_record_search($queryType, $queryValue, $found, $meta);
    }
    ...
    if ($found && function_exists('ai_learning_is_resolved') && ai_learning_is_resolved($queryType, $queryValue)) {
        return;
    }

    ai_action_event_record('client', 'search', $queryValue, [
        'query_type' => $queryType,
        'query' => $queryValue,
        'found' => $found,
