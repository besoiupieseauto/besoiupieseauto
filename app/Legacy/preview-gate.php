...
/**
 * Mod preview storefront — site vizibil doar cu cheie (?key= / ?keys=) sau cookie valid.
...
if (!function_exists('besoiu_preview_request_key')) {
    /** Cheie din URL: ?key= sau ?keys= (alias). */
    function besoiu_preview_request_key(): string
    {
        foreach (['keys', 'key'] as $param) {
            if (!isset($_GET[$param])) {
                continue;
            }
...
if (!function_exists('besoiu_preview_try_unlock_from_request')) {
    ...
    {
        $key = besoiu_preview_request_key();
...
if (!function_exists('besoiu_preview_admin_authenticated')) {
    function besoiu_preview_admin_authenticated(): bool
    {
...
if (!function_exists('besoiu_preview_admin_public_uri')) {
    /** Rute admin accesibile fără cheie preview (doar login / OAuth / înregistrare). */
    function besoiu_preview_admin_public_uri(string $uri): bool
    {
        if ($uri === '/admin' || $uri === '/admin/') {
            return true;
        }

        if (preg_match('#^/admin/public/?$#', $uri) === 1) {
            return true;
        }

        if (preg_match('#^/admin/(login|logout|addusersadd|reg)(/|$|\.php)(\?.*)?$#', $uri) === 1) {
            return true;
        }

        if (preg_match('#^/admin/public/(login|logout|addusersadd|reg|auth/google|auth/google/callback)(/|$|\.php)(\?.*)?$#', $uri) === 1) {
            return true;
        }
...
if (!function_exists('besoiu_preview_is_allowed')) {
    function besoiu_preview_is_allowed(): bool
    {
        ...
        if (besoiu_preview_lock_all_enabled() && besoiu_preview_admin_authenticated()) {
            return true;
        }
...
if (!function_exists('besoiu_preview_should_skip_request')) {
    function besoiu_preview_should_skip_request(): bool
    {
        ...
        if (besoiu_preview_lock_all_enabled()) {
            ...
            if (besoiu_preview_admin_public_uri($uri)) {
                return true;
            }
        }
...
if (!function_exists('besoiu_preview_render_coming_soon')) {
    function besoiu_preview_render_coming_soon(): void
    {
        ...
        $phone = besoiu_store_phone_display();
        $phoneTel = besoiu_store_phone_tel();
        $address = besoiu_store_address_line();
        $assetJs = '/assets/js/preview-gate.js?v=4';
        $urlKey = besoiu_preview_request_key();
...
<title>' . htmlspecialchars($brand, ENT_QUOTES, 'UTF-8') . ' — Proiect în lucru</title>
<style>
...
.card{max-width:560px;width:100%;background:rgba(255,255,255,.08);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.15);border-radius:20px;padding:40px 32px;text-align:center;box-shadow:0 24px 80px rgba(0,0,0,.35)}
...
.contact{margin-top:28px;padding-top:24px;border-top:1px solid rgba(255,255,255,.12);text-align:left}
.contact-label{margin:0 0 12px;font-size:.78rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5eead4}
.contact-row{display:flex;gap:12px;align-items:flex-start;margin:0 0 14px;color:#e2e8f0;line-height:1.55}
.contact-row:last-child{margin-bottom:0}
.contact-icon{flex:0 0 22px;width:22px;text-align:center;color:#5eead4;margin-top:2px}
.contact-row a{color:#99f6e4;text-decoration:none;font-weight:600}
...
<h1>Proiect în lucru</h1>
<p>Magazinul online Besoiu Piese Auto se pregătește de lansare. Lucrăm la catalog, stocuri și livrare rapidă.</p>
<p>Revenim în curând — mulțumim pentru răbdare!</p>
<span class="badge">Proiect în lucru</span>
<div class="contact">
<p class="contact-label">Contact</p>
<p class="contact-row"><span class="contact-icon" aria-hidden="true">☎</span><span><a href="' . htmlspecialchars($phoneTel, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($phone, ENT_QUOTES, 'UTF-8') . '</a></span></p>
<p class="contact-row"><span class="contact-icon" aria-hidden="true">📍</span><span>' . htmlspecialchars($address, ENT_QUOTES, 'UTF-8') . '</span></p>
</div>
</div>
...
if (!function_exists('besoiu_preview_gate_enforce')) {
    ...
    function besoiu_preview_gate_enforce(string $format = 'html'): void
    {
        ...
        // Development: evită blocarea accidentală a întregului proiect (preview doar cu FORCE=1).
        besoiu_preview_load_env();
        $appEnv = strtolower(trim((string) ($_ENV['APP_ENV'] ?? getenv('APP_ENV') ?: '')));
        if (in_array($appEnv, ['development', 'local', 'dev'], true) && !besoiu_preview_env_bool('SITE_PREVIEW_FORCE', false)) {
            return;
        }
...
if (!function_exists('besoiu_preview_head_script')) {
    function besoiu_preview_head_script(): string
    {
        ...
        $src = '/assets/js/preview-gate.js?v=4';
