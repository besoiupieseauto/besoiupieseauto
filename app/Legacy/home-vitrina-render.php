...
if (!function_exists('besoiu_vitrina_card_pricing')) {
    ...
    function besoiu_vitrina_card_pricing(array $product): array
    {
        ...
        if (function_exists('besoiu_product_card_offer_price')) {
            $newPrice = besoiu_product_card_offer_price($product);
        } else {
            $newPrice = besoiu_catalog_price($product['pPrice'] ?? $product['price'] ?? $product['price_numeric'] ?? $product['pBasePrice'] ?? 0);
        }
...
<section class="_product-section section home-products-panel home-vitrina-under-hero <?= $panelClass ?>" id="home-vitrina-panel" aria-labelledby="home-vitrina-title">
    <div class="_product-container container">
        ...
        <div class="trust">
            ...
            ?>
            <div class="trust-item"><div class="ticon"><?php site_live_cms_image_tag('home', 'products.trust.' . $ti . '.icon', (string) ($trustItem['icon'] ?? ''), ['class' => 'ui-icon-28', 'alt' => trim((string) ($trustItem['title'] ?? '') . ' ' . $trustSubtitle), 'width' => '28', 'height' => '28', 'loading' => 'lazy', 'decoding' => 'async', 'data-cms-variant' => 'icon']); ?></div><div><?php site_live_cms_tag('home', 'products.trust.' . $ti . '.title', 'b', (string) ($trustItem['title'] ?? '')); ?><span><?php if ($trustSubtitleHref !== ''): ?><?php site_live_cms_tel_link('home', 'products.trust.' . $ti . '.subtitle', $trustSubtitleHref, $trustSubtitle, 'trust-phone-link'); ?><?php else: ?><?php site_live_cms_tag('home', 'products.trust.' . $ti . '.subtitle', 'span', $trustSubtitle); ?><?php endif; ?></span></div></div>
            <?php endforeach; ?>
        </div>
    </div>
</section>
        ...
        <article class="_product-card <?= besoiu_catalog_h($cardClass) ?>"
            ...
            <?php if ($isMinimalVitrina): ?>
                ...
                <div class="_product-card-image _product-card-image--clickable">
                    <img src="<?= besoiu_catalog_h(besoiu_catalog_css_url($image)) ?>" alt="<?= besoiu_catalog_h($shortTitle) ?>" width="300" height="225" loading="lazy" decoding="async">
                </div>
            ...
            <?php else: ?>
                ...
                ?>
                <div class="_product-card-image _product-card-image--clickable">
                    <img src="<?= besoiu_catalog_h(besoiu_catalog_css_url($image)) ?>" alt="<?= besoiu_catalog_h($name) ?>" width="300" height="225" loading="lazy" decoding="async">
                </div>
                <div class="_product-card-head">
