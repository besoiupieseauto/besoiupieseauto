...
if (!function_exists('tecdoc_rapidapi_key')) {
    $importTecdoc = dirname(__DIR__) . '/Import/Scraper/system/tecdoc_stock.php';
    if (is_file($importTecdoc)) {
        require_once $importTecdoc;
    } else {
...
if (!function_exists('tecdoc_desc_looks_like_base_import')) {
    /** True dacă HTML-ul urmează skeleton-ul din Base.html (p/ul/li). */
    function tecdoc_desc_looks_like_base_import(string $note): bool
    {
        if ($note === '' || !str_contains($note, '<p>')) {
            return false;
        }

        return str_contains($note, 'Specificatii tehnice:')
            || str_contains($note, 'Specificații tehnice:')
            || str_contains($note, 'Compatibil cu urmatoarele modele auto:')
            || str_contains($note, 'Coduri OE echivalente:');
    }
}
...
if (!function_exists('tecdoc_note_looks_complete')) {
    function tecdoc_note_looks_complete(string $note): bool
    {
        ...
        // Skeleton Base.html (import-pro / coadă) — nu rescrie cu dl/dt.
        if (tecdoc_desc_looks_like_base_import($note)) {
            return str_contains($note, '<ul>') || str_contains($note, '<li>');
        }
...
if (!function_exists('tecdoc_desc_build_base_import_html')) {
    /**
     * Skeleton fix Base.html: header + Specificatii + Coduri OE (fără dl/dt).
     *
     * @param list<array{label:string,value:string}> $rows
     * @param list<array{brand:string,number:string}> $oemLines
     */
    function tecdoc_desc_build_base_import_html(
        string $productName,
        string $brandName,
        string $productCode,
        array $rows,
        ...
        $title = trim($productName . ' ' . $brandName);
        $html = '<p><b>' . tecdoc_desc_h($title) . '</b>';
        if (trim($productCode) !== '') {
            $html .= ' (Cod: ' . tecdoc_desc_h(trim($productCode)) . ')';
        }
        $html .= '</p>';
        ...
        $specLis = [];
        foreach ($rows as $row) {
            $label = trim(tecdoc_desc_normalize_label((string) ($row['label'] ?? '')));
            $value = trim((string) ($row['value'] ?? ''));
            if ($label === '' && $value === '') {
                continue;
            }
            $line = $value !== '' ? ($label !== '' ? ($label . ': ' . $value) : $value) : $label;
            $key = function_exists('mb_strtolower')
                ? mb_strtolower(preg_replace('/\s+/u', ' ', $label !== '' ? $label : $line) ?? '', 'UTF-8')
                : strtolower(preg_replace('/\s+/u', ' ', $label !== '' ? $label : $line) ?? '');
            ...
            $specLis[] = '<li>' . tecdoc_desc_h($line) . '</li>';
        }
        if ($specLis !== []) {
            $html .= '<p><b>Specificatii tehnice:</b></p><ul>' . implode('', $specLis) . '</ul>';
        }
        ...
        foreach (array_slice($oemLines, 0, 40) as $line) {
            ...
            $number = trim((string) ($line['number'] ?? ''));
            if ($brand === '' && $number === '') {
                continue;
            }
            $text = $brand !== '' && $number !== '' ? ($brand . ': ' . $number) : ($brand . $number);
            $ok = function_exists('mb_strtolower') ? mb_strtolower($text, 'UTF-8') : strtolower($text);
            if (isset($seenOem[$ok])) {
                continue;
            }
            $seenOem[$ok] = true;
            $oemLis[] = '<li>' . tecdoc_desc_h($text) . '</li>';
        }
        if ($oemLis !== []) {
            $html .= '<p><b>Coduri OE echivalente:</b></p><ul>' . implode('', $oemLis) . '</ul>';
        }
...
if (!function_exists('tecdoc_build_product_description')) {
    ...
    function tecdoc_build_product_description(string $code, string $brand = '', string $name = '', int $articleId = 0, string $oemCsv = ''): array
    {
        ...
        // Import + coadă: același skeleton Base.html (p/ul/li), nu dl.tecdoc-desc-sheet.
        $html = tecdoc_desc_build_base_import_html($productName, $brandName, $productCode, $rows, $oemLines);

        if ($html === '' || (!str_contains($html, '<ul>') && !str_contains($html, '<li>'))) {
            return [
                'html' => '',
