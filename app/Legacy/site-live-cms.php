...
if (!function_exists('site_live_cms_tag')) {
    ...
    function site_live_cms_tag(string $page, string $key, string $tag, ?string $default = null, array $attrs = [], bool $html = false): void
    {
        ...
        $styleClasses = [];
        if ($elStyle !== []) {
            // Pe live public: doar marker CMS — fără bpa-pad/mw (cauzează CLS dacă CSS-ul CMS apare async).
            $styleClasses[] = 'bpa-cms-styled';
            if (site_live_edit_mode()) {
                $tmp = site_live_cms_style_attrs($elStyle);
                if (preg_match('/class="([^"]*)"/', $tmp, $mCls)) {
                    $styleClasses = preg_split('/\s+/', trim($mCls[1])) ?: ['bpa-cms-styled'];
                }
            }
        }
        $attrClass = trim((string) ($attrs['class'] ?? ''));
        unset($attrs['class']);
        $mergedClass = trim(implode(' ', array_filter([$attrClass, ...$styleClasses])));
        $attrStr = '';
        if ($mergedClass !== '') {
            $attrStr .= ' class="' . htmlspecialchars($mergedClass, ENT_QUOTES, 'UTF-8') . '"';
        }
        // Culori/bg din stiluri CMS — doar inline (fără utility classes pe live).
        if ($elStyle !== [] && !site_live_edit_mode()) {
            $css = [];
            if (($elStyle['bgColor'] ?? '') !== '') {
                $css[] = 'background-color:' . $elStyle['bgColor'];
            }
            if (($elStyle['textColor'] ?? '') !== '') {
                $css[] = 'color:' . $elStyle['textColor'];
            }
            if ($css !== []) {
                $attrStr .= ' style="' . htmlspecialchars(implode(';', $css), ENT_QUOTES, 'UTF-8') . '"';
            }
        } elseif ($elStyle !== [] && site_live_edit_mode()) {
            $styleAttr = site_live_cms_style_attrs($elStyle);
            if (preg_match('/style="([^"]*)"/', $styleAttr, $mSt)) {
                $attrStr .= ' style="' . $mSt[1] . '"';
            }
        }
        ...
        if (site_live_edit_mode()) {
            if ($mergedClass === '') {
                $attrStr = ' class="bpa-cms-field"' . $attrStr;
            }
            echo '<' . $tag . $attrStr
        ...
        echo '<' . $tag . ($attrStr !== '' ? $attrStr : '') . '>';
        echo $html ? $value : htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
        echo '</' . $tag . '>';
    }
}

if (!function_exists('site_live_cms_tel_link')) {
    /**
     * Link tel:/mailto: — fără span CMS în interior (altfel click-ul nu deschide apelul).
     * În mod edit: câmp CMS ascuns pentru salvare + link vizibil funcțional.
     *
     * @param array<string, string> $attrs
     */
    function site_live_cms_tel_link(
        string $page,
        string $key,
        string $href,
        string $labelDefault,
        ...
        $label = site_live_field_value($page, $key, $labelDefault);
        $cmsKey = htmlspecialchars($page . '.' . $key, ENT_QUOTES, 'UTF-8');
        $hrefResolved = function_exists('site_phone_resolve_href')
            ? site_phone_resolve_href($label, $href)
            : $href;
        if ($hrefResolved === '' && function_exists('site_phone_to_tel_href')) {
            $hrefResolved = site_phone_to_tel_href($label);
        }
        if ($ariaLabel === '') {
            $ariaLabel = 'Sună la ' . $label;
        }

        if (site_live_edit_mode()) {
            echo '<span class="bpa-cms-sr-only" data-cms="' . $cmsKey
                . '" data-cms-page="' . htmlspecialchars($page, ENT_QUOTES, 'UTF-8')
                . '" data-cms-key="' . htmlspecialchars($key, ENT_QUOTES, 'UTF-8')
                . '" data-cms-html="0" contenteditable="true" spellcheck="true">'
        ...
        echo '<a' . $classAttr . ' href="' . htmlspecialchars($hrefResolved, ENT_QUOTES, 'UTF-8')
            . '" aria-label="' . htmlspecialchars($ariaLabel, ENT_QUOTES, 'UTF-8') . '"' . $extra . '>'
...
if (!function_exists('site_live_cms_image_url')) {
    function site_live_cms_image_url(string $page, string $key, ?string $default = null, ?string $itemPage = null): string
    {
        ...
        if (!function_exists('besoiu_prefer_local_webp')) {
            require_once __DIR__ . '/besoiu-image.php';
        }

        return besoiu_prefer_local_webp($url);
    }
}
    ...
    function site_live_cms_image_tag(string $page, string $key, ?string $default = null, array $attrs = [], ?string $itemPage = null): void
    {
        ...
        if ($url === '') {
            if (site_live_edit_mode()) {
                ...
                echo '<span class="bpa-cms-image-placeholder"><i class="fa-solid fa-image" aria-hidden="true"></i> Adaugă imagine</span></span>';
            }
            return;
        }
