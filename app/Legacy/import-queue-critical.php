...
function besoiu_import_row_critical_flags(array $row): array
{
    ...
    $basePrice = $baseRaw === '' ? 0.0 : (float) $baseRaw;
    // Import Pro / vitrină: pBasePrice = achiziție din CSV; pPrice = final după Adaos comercial.
    if ($price <= 0.0 && $basePrice <= 0.0) {
        $flags[] = ['code' => 'zero_price', 'label' => 'Fără preț'];
    }
