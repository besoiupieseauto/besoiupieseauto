...
function baselinker_feed_product_entry(array $product, array $mapping, string $siteBaseUrl): array
{
    ...
    if (class_exists(\Besoiu\Services\ProductCardFormationService::class)) {
        try {
            $marketplaceTitle = \Besoiu\Services\ProductCardFormationService::resolveMarketplaceTitleFromRow($product);
            if ($marketplaceTitle !== '') {
                $name = $marketplaceTitle;
            }
        } catch (Throwable) {
            // păstrează name din mapping
        }
    }
    if ($name === '') {
...
{
    $jobQueuePath = BESOIU_LEGACY . '/JobQueue.php';
