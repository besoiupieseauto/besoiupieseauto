...
function besoiu_storefront_base(): string
{
    if (defined('BESOIU_STOREFRONT_BASE')) {
        return rtrim((string) BESOIU_STOREFRONT_BASE, '/');
    }
...
function besoiu_normalize_internal_path(string $path): string
{
    ...
    if ($base !== '' && $path !== '' && $path[0] === '/') {
        ...
        $queryPart = str_contains($path, '?') ? ('?' . substr(strstr($path, '?'), 1)) : '';
        if ($pathOnly === '/') {
            return $base . '/' . $queryPart;
        }
        return $base . $pathOnly . $queryPart;
    }
