...
if (!class_exists('ScraperPaths', false)) {
    $importBootstrap = (defined('BESOIU_APP') ? BESOIU_APP : dirname(__DIR__))
        . '/Import/Scraper/bootstrap.php';
    if (is_file($importBootstrap)) {
        require_once $importBootstrap;
    } elseif (defined('BESOIU_LIB') && is_file(BESOIU_LIB . '/Scraper/bootstrap.php')) {
        require_once BESOIU_LIB . '/Scraper/bootstrap.php';
    }
}
...
{
    $dir = BESOIU_BACKEND . '/storage/logs';
    if (!is_dir($dir)) {
        @mkdir($dir, 0777, true);
    }
...
function emag_find_image_for_product(array $product): array
{
    ...
    $pipelinePath = BESOIU_LEGACY . '/image_search_pipeline.php';
