...

    {
        ...
        foreach (besoiu_scraper_catalog_products() as $product) {
            ...
            $slides[] = [
                ...
                'image'    => $image !== '' ? besoiu_resolve_product_image($image) : besoiu_default_product_image(),
...
if (!function_exists('besoiu_hero_promo_fallback_slide')) {
...

    {
        ...
        if ($image === '') {
            $image = besoiu_default_product_image();
        } else {
            $image = besoiu_resolve_product_image($image);
        }
            ...
            <a class="hero-promo-slide<?= $isActive ? ' is-active' : '' ?>"
               ...
               <?= $isActive ? 'tabindex="0"' : 'tabindex="-1" aria-hidden="true"' ?>>
                <div class="hero-promo-slide-media">
                    <?php if ($image !== ''): ?>
                    <?php besoiu_render_product_image($image, $title, [
                        'eager' => $index === 0,
                        'width' => 130,
                        'height' => 130,
                        'fetchpriority' => 'auto',
                        'sizes' => '(max-width: 768px) 130px, 340px',
                    ]); ?>
                    <?php else: ?>
