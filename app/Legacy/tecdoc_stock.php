...
if (defined('TECDOC_STOCK_BRIDGE_LOADED')) {
    if (!defined('TECDOC_LEGACY_SKIPPED')) {
        define('TECDOC_LEGACY_SKIPPED', true);
    }
    return;
}
define('TECDOC_STOCK_BRIDGE_LOADED', true);
...
require_once __DIR__ . '/api_automation_guard.php';
...
{
    tecdoc_load_env();
    $helper = tecdoc_admin_root() . '/system/env_settings.php';
    if (is_file($helper)) {
        require_once $helper;
        if (function_exists('besoiu_env_get')) {
            $key = besoiu_env_get('RAPIDAPI_AUTOPARTS_KEY');
            if ($key === '') {
                $key = besoiu_env_get('RAPIDAPI_TECDOC_KEY');
            }
            if ($key !== '') {
                return $key;
            }
...
function tecdoc_rapidapi_is_user_key(): bool
{
    ...
    if (is_file($helper)) {
        require_once $helper;
        if (function_exists('besoiu_env_has')) {
            return besoiu_env_has('RAPIDAPI_AUTOPARTS_KEY') || besoiu_env_has('RAPIDAPI_TECDOC_KEY');
        }
    }
...
{
    return BESOIU_CACHE_TECDOC . '/.rapidapi_quota_exceeded';
}
...
/** După token RapidAPI nou — elimină blocarea locală de cotă. */
function tecdoc_clear_quota_unavailable_flag(): void
{
    $flag = tecdoc_quota_flag_path();
    if (is_file($flag)) {
        @unlink($flag);
...
/** @return ''|'automation'|'quota_flag' */
function tecdoc_api_unavailable_reason(): string
{
    if (function_exists('besoiu_api_automation_blocks_tecdoc_live')
...
{
    $reason = tecdoc_api_unavailable_reason();
    if ($reason === '') {
        return '';
    }

    if ($reason === 'automation') {
        return function_exists('besoiu_api_automation_block_message')
            ? besoiu_api_automation_block_message('rapidapi_tecdoc')
            : 'RapidAPI TecDoc — apelurile live sunt oprite. Apasă «Armez API live» în bara de sus.';
    }
    ...
    if (is_file($flag)) {
        ...
        if ($at > 0) {
            ...
            return 'RapidAPI TecDoc blocat local după o eroare de cotă (acum '
                . (int) floor($ago / 60) . ' min). Cheia poate fi validă — rulează din nou testul sau șterge flag-ul din Setări. Reset automat după 24h.';
        }
    }
...
/**
 * Dacă există flag local de cotă, face un probe live; îl șterge când API răspunde OK.
 */
function tecdoc_try_clear_quota_flag_with_probe(): bool
{
    if (!is_file(tecdoc_quota_flag_path())) {
        return true;
    }
    ...
    if (trim(tecdoc_rapidapi_key()) === '') {
        return false;
    }

    $host = BESOiu_TECDOC_HOST;
    $url = 'https://' . $host . '/models/list/type-id/1/manufacturer-id/16/lang-id/21/country-filter-id/63';
    $response = tecdoc_http_get($url);
    $httpCode = (int) ($response['http_code'] ?? 0);
    $body = (string) ($response['body'] ?? '');
    $apiMessage = tecdoc_extract_api_error_message($body, $httpCode);

    if ($httpCode === 200 && !tecdoc_is_hard_quota_exceeded($apiMessage, $httpCode)) {
...
{
    return tecdoc_api_unavailable_reason() !== '';
}
...
{
    return BESOIU_CACHE_TECDOC . '/.dashboard_ip_probe.json';
}
...
{
    return defined('BESOIU_BACKEND') ? BESOIU_BACKEND : dirname(__DIR__, 2) . '/app/Backend';
}
...
{
    if (besoiu_api_automation_blocks_tecdoc_live()) {
        $message = besoiu_api_automation_block_message('rapidapi_tecdoc');
        tecdoc_set_api_error($message, ['url' => $url, 'automation_paused' => true]);

        return [
            'body' => json_encode(['message' => $message, 'error' => 'automation_paused'], JSON_UNESCAPED_UNICODE),
            'http_code' => 503,
            'error' => null,
        ];
    }
    ...
    if ($httpCode >= 200 && $httpCode < 400 && $error === '') {
        $budgetLib = BESOIU_BACKEND . '/system/api_token_budget.php';
...
function tecdoc_ensure_supplier_price_logic_loaded(): void
{
    ...
    $configPath = BESOIU_CONFIG . '/config.php';
...
function tecdoc_product_image(array $row): string
{
    ...
    return is_array($images) && !empty($images[0])
        ? besoiu_resolve_product_image((string) $images[0])
        : besoiu_default_product_image();
}
...
{
    return BESOIU_CACHE_TECDOC . '/.local_stock_brands.json';
}
...
function serpapi_find_image_url(string $query): string
{
    ...
    $vendorFile = BESOIU_BACKEND . '/vendor/serpapi/google-search-results-php/google-search-results.php';
...
function tecdoc_download_image(string $url, string $code): string
{
    ...
    if (!is_file($target) || (int) filesize($target) < 200) {
        return '';
    }
