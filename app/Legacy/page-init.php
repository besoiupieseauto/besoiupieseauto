...
if (
    PHP_SAPI !== 'cli'
    && !empty($_GET['bpa_dom_marker'])
    && (string) $_GET['bpa_dom_marker'] === '1'
) {
    require_once __DIR__ . '/dom-marker-lib.php';
    if (site_live_admin_authenticated()) {
        if (!defined('BPA_DOM_MARKER_ACTIVE')) {
            define('BPA_DOM_MARKER_ACTIVE', true);
        }
    }
}
...
/** Panou debug / markup admin dev — doar operator admin autentificat (sau CLI cu BESOIU_DEV_TOOLS). */
