...
{
    if (!function_exists('robot_products_load_catalog')) {
        require_once __DIR__ . '/products_catalog.php';
    }

    return fb_search_catalog_products($code, robot_products_load_catalog(), 5);
}

/**
 * Căutare în catalog unificat (MySQL / cache) — folosit de FB + widget (#41).
 *
 * @param list<array<string, mixed>> $products
 * @return list<array<string, mixed>>
 */
function fb_search_catalog_products(string $code, array $products, int $limit = 5): array
{
    $compact = fb_compact_code($code);
    if ($compact === '') {
        return [];
    ...
    foreach ($products as $p) {
        ...
        $oem = fb_compact_code((string) ($p['oem'] ?? $p['pOem'] ?? ''));
        $pc = fb_compact_code((string) ($p['code'] ?? $p['pCode'] ?? ''));
...
function fb_validate_oem_codes(array $codes): array
{
    ...
    foreach ($codes as $code) {
        ...
        $catalogHits = fb_search_products_json($code);
        $hits = $dbHits !== [] ? $dbHits : $catalogHits;
        if ($dbHits !== [] && $catalogHits !== []) {
            $hits = array_slice(array_merge($dbHits, $catalogHits), 0, 5);
        }
        $validated[] = [
            'code' => $code,
            'found' => $hits !== [],
