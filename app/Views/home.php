...
require_once BESOIU_LEGACY . '/home-vitrina-render.php';
require_once BESOIU_LIB . '/Scraper/ScraperModule.php';
require_once BESOIU_LEGACY . '/scraper-home.php';
require_once BESOIU_LEGACY . '/hero-promo-carousel.php';
...
$homeScraperTotal = ScraperModule::instance()->epiesaProductCount();
$homeScraperTabs = besoiu_scraper_home_tabs();
$homeScraperProducts = besoiu_scraper_catalog_products(besoiu_scraper_home_display_limit(), false);
...
<head>
    ...
    besoiu_render_seo_meta(besoiu_seo_page_meta('/', $homeMeta, 'website'));
    ?>

    <?php
    require_once BESOIU_LEGACY . '/besoiu-image.php';
    // LCP real pe homepage = logo (PSI). Preload early + fetchpriority pe <img>.
    if (is_file(BESOIU_ROOT . '/assets/img/logo.webp')) {
        echo '<link rel="preload" as="image" href="assets/img/logo.webp" type="image/webp" fetchpriority="high">' . "\n";
    }
    if (!empty($homeHeroPromoSlides[0]['image'])) {
        $lcpVariants = besoiu_product_image_variants((string) $homeHeroPromoSlides[0]['image']);
        $promoPreload = $lcpVariants[200] ?? $lcpVariants[400] ?? null;
        if ($promoPreload !== null) {
            echo '<link rel="preload" as="image" href="' . htmlspecialchars(besoiu_image_public_path($promoPreload), ENT_QUOTES, 'UTF-8') . '">' . "\n";
        }
    }
    ?>
    <?php besoiu_render_fonts(true); ?>
    <?php besoiu_render_styles('home'); ?>
    <?php require_once BESOIU_LEGACY . '/storefront-config.php'; besoiu_render_storefront_config_script(); ?>
</head>
<body>
<div class="page">
...
<main id="main-content" data-besoiu-home-ajax="1">

<!-- ══ FILTERBAR — categorii + vehicul (marcă / model / motorizare) ══ -->
<div class="filterbar">
    <div class="container home-filter-panel">
        <div class="cat-btn-wrap">
            <button class="cat-btn" type="button" id="cat-toggle" aria-label="Deschide meniul de categorii piese auto" aria-haspopup="true" aria-expanded="false" aria-controls="cat-popup">
                <img src="img/icons/16_meniu_categorii.svg" alt="" class="ui-icon-22 ui-icon--on-dark" role="presentation" width="22" height="22">
                <?php site_live_cms_tag('home', 'filterbar.categories_btn', 'span', (string) ($home['filterbar']['categories_btn'] ?? 'Categorii piese')); ?>
                <img src="img/icons/31_chevron_jos.svg" alt="" class="ui-icon-14 ui-icon--on-dark ui-icon--chevron-auto" role="presentation" width="14" height="14">
            </button>
            <div class="cat-badge" id="cat-badge">
                <img alt="" id="cat-badge-icon" width="24" height="24" hidden>
                <span id="cat-badge-text"></span>
                <button type="button" class="cat-badge-x" id="cat-badge-x" aria-label="Elimină categoria selectată">✕</button>
            </div>
        ...
        <div class="vehicle-box" id="vehicle-box">
            <div class="vehicle-item">
                <div class="vico"><img src="img/icons/17_marca_auto.svg" alt="" class="ui-icon-20" role="presentation" width="20" height="20"></div>
                <div class="vehicle-item-field">
                    <label for="select_marca"><strong>Marcă</strong></label>
                    <select class="_product-input" id="select_marca" aria-label="Alege marca vehiculului">
                        <option value="0">Alege marca</option>
                    </select>
                </div>
            </div>
            <div class="vehicle-item">
                <div class="vico"><img src="img/icons/18_model_auto.svg" alt="" class="ui-icon-20" role="presentation" width="20" height="20"></div>
                <div class="vehicle-item-field">
                    <label for="model_marca"><strong>Model</strong></label>
                    <select class="_product-input" id="model_marca" disabled aria-label="Alege modelul vehiculului">
                        <option value="0">Alege modelul</option>
                    </select>
                </div>
            </div>
            <div class="vehicle-item">
                <div class="vico"><img src="img/icons/20_motorizare_ceas.svg" alt="" class="ui-icon-20" role="presentation" width="20" height="20"></div>
                <div class="vehicle-item-field">
                    <label for="motorizari"><strong>Motorizare</strong></label>
                    <select class="_product-input" id="motorizari" disabled aria-label="Alege motorizarea vehiculului">
                        <option value="0">Alege motorizarea</option>
                    </select>
                </div>
            </div>
            <div class="vehicle-item" id="subcat-slot" style="display:none">
                <div class="vico"><img src="img/icons/16_meniu_categorii.svg" alt="" class="ui-icon-20" role="presentation" width="20" height="20"></div>
                <div class="vehicle-item-field">
                    <label for="select_categorie"><strong>Subcategorie</strong></label>
                    <select class="_product-input" id="select_categorie" aria-label="Alege subcategoria piesei">
                        <option value="0">Alege subcategoria</option>
                    </select>
                </div>
            </div>
            <button class="vehicle-search" type="button" id="btnSearch" disabled aria-label="Caută piesele compatibile cu vehiculul selectat"><?php site_live_cms_tag('home', 'filterbar.search_btn', 'span', (string) ($home['filterbar']['search_btn'] ?? 'CAUTĂ PIESĂ')); ?></button>
        </div>
    ...
    <div class="container">
        <div>
            ...
            <form class="quick-check" onsubmit="return false;">
                <label for="_filter-vin" class="visually-hidden">Caută piesă după cod OEM sau VIN</label>
                <input type="text" id="_filter-vin" placeholder="<?= site_cms_h($home['hero']['search_placeholder'] ?? '') ?>" aria-label="Caută piesă după cod OEM sau VIN" />
                <button type="button" id="_product-apply-filters" aria-label="Caută piesa"><?php site_live_cms_tag('home', 'hero.search_button', 'span', (string) ($home['hero']['search_button'] ?? '')); ?></button>
            </form>

            <div class="hero-benefits">
                <?php foreach (($home['hero']['benefits'] ?? []) as $bi => $benefit): ?>
                <div><span class="bico"><?php site_live_cms_image_tag('home', 'hero.benefits.' . $bi . '.icon', (string) ($benefit['icon'] ?? ''), ['class' => 'ui-icon-28', 'alt' => (string) ($benefit['text'] ?? ''), 'width' => '28', 'height' => '28', 'data-cms-variant' => 'icon']); ?></span><?php site_live_cms_tag('home', 'hero.benefits.' . $bi . '.text', 'b', (string) ($benefit['text'] ?? '')); ?></div>
                <?php endforeach; ?>
            </div>
        </div>
...
<section class="why">
    <div class="container">
        <div class="why-box">
            ...
            <div class="road">
                <?php site_live_cms_image_tag('home', 'why.car_image', (string) ($home['why']['car_image'] ?? 'img/car1.webp'), ['width' => '560', 'height' => '320', 'loading' => 'lazy', 'decoding' => 'async', 'class' => 'why-car-image', 'alt' => 'Piese auto Besoiu — echipă și service auto', 'data-cms-variant' => 'full']); ?>
            </div>
            <div class="help" id="help-consult-block">
                ...
                <div class="help-actions">
                    ...
                    <?php endif; ?>
                    <?php site_live_cms_tel_link('home', 'why.phone', $whyPhoneHref, $whyPhone, 'call-btn'); ?>
                </div>
...
<div id="besoiu-home-notify" class="besoiu-home-notify besoiu-notify--dormant" role="alert" aria-live="assertive"></div>
...
<?php
$homeScripts = ['assets/js/home-tecdoc.js'];
