...
final class EpiesaCatalog
{
  ...
  public static function save(array $catalog): void
  {
    ...
    self::writeCatalogIndex(self::buildCatalogIndex($catalog));
    self::$productsRawCache = null;
    self::$catalogIndexCache = null;
  }
  ...
  /** @var array{updated_at: ?string, product_count: int, items: array<int, array<string, mixed>>}|null */
  private static ?array $catalogIndexCache = null;

  /**
   * @param array<string, mixed> $catalog
   * @return array{updated_at: ?string, product_count: int, items: array<int, array<string, mixed>>}
   */
  private static function buildCatalogIndex(array $catalog): array
  {
    $items = [];
    foreach ($catalog['products'] ?? [] as $row) {
      if (!is_array($row) || ($row['visible'] ?? true) === false) {
        continue;
      }
      $items[] = [
        ...
        'category_slug'  => (string) ($row['category_slug'] ?? 'altele'),
  ...
  /**
   * @param array{updated_at: ?string, product_count: int, items: array<int, array<string, mixed>>} $index
   */
  private static function writeCatalogIndex(array $index): void
  {
    ScraperPaths::ensureDirs();
    file_put_contents(
      ScraperPaths::catalogIndexPath(),
   ...
   * @return array{updated_at: ?string, product_count: int, items: array<int, array<string, mixed>>}
   */
  private static function loadCatalogIndex(): array
  {
    if (self::$catalogIndexCache !== null) {
      return self::$catalogIndexCache;
    }

    $path = ScraperPaths::catalogIndexPath();
    if (is_file($path)) {
      $data = json_decode((string) file_get_contents($path), true);
      if (is_array($data) && is_array($data['items'] ?? null)) {
        self::$catalogIndexCache = [
          'updated_at'    => $data['updated_at'] ?? null,
          'product_count' => (int) ($data['product_count'] ?? count($data['items'])),
    ...
    $catalog = self::load();
    $index = self::buildCatalogIndex($catalog);
    self::writeCatalogIndex($index);
    self::$catalogIndexCache = $index;

    return $index;
  }

  /** @return array<int, array<string, mixed>> */
  private static function filterIndexRows(
    array $rows,
    ?string $categorySlug,
    string $search
  ): array {
    ...
    return array_values(array_filter($rows, static function (array $p) use ($search): bool {
  ...
  {
    $counts = ['toate' => 0];
    foreach (self::loadCatalogIndex()['items'] as $row) {
      $counts['toate']++;
  ...
  public static function listProductsPaginated(
    ...
    $rows = self::filterIndexRows(self::loadCatalogIndex()['items'], $categorySlug, $search);
  ...
  {
    $index = self::loadCatalogIndex();
    $slugCounts = self::categorySlugCounts();
    $byCategory = $slugCounts;
    unset($byCategory['toate']);
    ...
    $lastScan = self::loadCatalogLastScanMeta();

    return [
      'total_products' => (int) ($index['product_count'] ?? count($index['items'] ?? [])),
  ...
  private static function loadCatalogLastScanMeta(): ?array
  {
    $path = ScraperPaths::catalogJsonPath();
    if (!is_file($path)) {
      return null;
    ...
    if (preg_match('/"last_scan"\s*:\s*(\{[^}]+\})/', $raw, $m)) {
