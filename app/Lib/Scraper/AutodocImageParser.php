...
final class AutodocImageParser
{
    ...
    public static function buildSearchQueries(array $product): array
    {
        ...
        if (is_file($pipeline)) {
            ...
            if (function_exists('besoiu_image_search_query_plan_for_product')) {
                $plan = besoiu_image_search_query_plan_for_product($product, 'autodoc');
                $queries = is_array($plan['queries'] ?? null) ? $plan['queries'] : [];
                $fallback = is_array($plan['fallback_queries'] ?? null) ? $plan['fallback_queries'] : [];

                return array_values(array_unique(array_filter(array_merge($queries, $fallback))));
            }
    ...
    public static function oemCodeVariants(string $code): array
    {
        ...
        // Consignație Autopartner: 714520849G → 714520849
        if (preg_match('/^.+\dG$/i', $raw) && strlen($raw) >= 6) {
            $withoutG = substr($raw, 0, -1);
            if ($withoutG !== '' && !in_array($withoutG, $variants, true)) {
                $variants[] = $withoutG;
            }
        }
    ...
    public static function rankMatches(array $items, array $product, ?string $searchQuery = null): array
    {
        ...
        if ($queryTrim !== '' && (str_contains($queryTrim, ' ') || strlen($queryTrim) > 12)) {
            ...
            $minScore = strlen($queryDigits) >= 6 ? 72 : 85;
        }
        ...
        if ($out === []) {
            $crossRef = self::acceptCrossRefListingHit($items, $product, $searchQuery);
            if ($crossRef !== null) {
                return [$crossRef];
            }
        }
    ...
    private static function productContextTokens(array $product): array
    {
        ...
        if (is_file($pipelinePath)) {
            require_once $pipelinePath;
            if (function_exists('besoiu_image_decompose_product_for_search')) {
                $ctx = besoiu_image_decompose_product_for_search($product);
                if (!empty($ctx['is_generic_deposit'])) {
                    return [];
                }
                $blob = strtolower(trim(implode(' ', [
                    (string) ($ctx['title_part'] ?? ''),
    ...
    private static function categoryRelevanceAdjust(string $title, array $product): int
    {
        ...
        if (is_file($pipelinePath)) {
            ...
            if (function_exists('besoiu_detect_fluid_family') && function_exists('besoiu_fluid_family_conflict')) {
                $wantFamily = besoiu_detect_fluid_family($nameBlob);
                $hitFamily = besoiu_detect_fluid_family($titleLower);
        ...
        $groups = [
            ...
            'suspension' => ['pivot', 'capat', 'bieleta', 'amortizor', 'bucsa'],
            'filter' => ['filtru', 'filter'],
            'brake_part' => ['placute', 'plăcuțe', 'disc', 'etrier'],
            'battery' => ['baterie de pornire', 'baterie auto', 'starter battery', 'acumulator', 'baterie'],
            'atf' => ['atf', 'transmisie', 'transmission', 'cutie automata', 'cutie auto', 'ulei transmisie'],
        ];

        if (str_contains($nameBlob, 'baterie') && !str_contains($nameBlob, 'pila')) {
            $want['battery'] = true;
            foreach (['cr2032', 'cr2025', 'cr2016', '3v', 'pila litio', 'pila litiu', 'buton', 'button cell', 'lithium coin'] as $buttonPart) {
                if (str_contains($titleLower, $buttonPart)) {
                    return -95;
                }
            }
        }
    ...
    /**
     * Autodoc găsește cod furnizor (ex. VAT2318HP) ca cross-ref → articol real (50529 VATOIL).
     *
     * @param list<array<string, mixed>> $items
     * @param array<string, mixed> $product
     */
    private static function acceptCrossRefListingHit(array $items, array $product, ?string $searchQuery): ?array
    {
        if (count($items) !== 1 || $searchQuery === null || trim($searchQuery) === '') {
            return null;
        }

        $code = strtoupper(preg_replace('/[^A-Z0-9]/i', '', trim((string) ($product['pCode'] ?? ''))) ?? '');
        $queryNorm = strtoupper(preg_replace('/[^A-Z0-9]/i', '', trim($searchQuery)) ?? '');
        if ($code === '' || $queryNorm !== $code) {
            return null;
        }

        $item = self::sanitizeListingItem($items[0]);
        ...
        if ($brand === '') {
            ...
            if (is_file($pipelinePath)) {
                require_once $pipelinePath;
                if (function_exists('besoiu_image_detect_brand_from_text')) {
                    $brand = mb_strtolower(besoiu_image_detect_brand_from_text((string) ($product['pName'] ?? '')), 'UTF-8');
                }
            }
        }
        ...
        if (self::categoryRelevanceAdjust($title, $product) <= -80) {
            return null;
        }

        $image = trim((string) ($item['image'] ?? $item['image_url'] ?? ''));
        $imageLower = mb_strtolower($image, 'UTF-8');
        if ($image === '' || str_contains($imageLower, 'brands/thumbs') || str_contains($imageLower, 'placeholder')) {
            return null;
        }

        $item['match_score'] = 88;
        $item['match_mode'] = 'autodoc_cross_ref';
    ...
    private static function scoreItem(array $item, array $wantCodes, string $brand, array $product = []): int
    {
        ...
        foreach ($wantCodes as $rawWant) {
            ...
            if ($titleDigits === $wantCode || str_contains($titleDigits, $wantCode)
                || preg_match('/\b' . preg_quote($wantCode, '/') . '\b/i', $title . ' ' . $skuRaw) === 1) {
                $codeInTitle = true;
                break;
            }
        }
        if ($brand !== '' && !$codeInTitle) {
        ...
        foreach ($wantCodes as $rawWant) {
            ...
            if ($wantRef !== '' && strlen($wantRef) >= 4) {
                if ($skuRef === $wantRef || str_contains($titleRef, $wantRef)) {
                    $adj = self::categoryRelevanceAdjust($title, $product);
                    if ($adj <= -80) {
                        return -10;
                    }
