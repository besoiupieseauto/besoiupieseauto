...
final class ScraperAiAgent
{
    ...
    public static function analyze(
        ...
        if (!ScraperLlmConfig::hasAnyKey() && !self::metroOrchestratorAvailable()) {
            return [
                'ok' => false,
                'mode' => 'none',
                'llm_used' => false,
                'error' => 'Lipsește GROQ_KEY, OPENAI_KEY sau alt provider LLM în admin/.env pentru analiză AI.',
                'selectors' => $heuristic ?? [],
                'compass' => $context['compass'],
                'html_hints' => $context['html_hints'],
        ...
        $metro = self::tryMetroOrchestrator($html, $sourceId, $userGoals, $fieldsNeeded, $limit, $heuristic, $context);
        if ($metro !== null) {
            return $metro;
        }
...
PROMPT;
    ...
    private static function suggestionsFromCompass(array $context): array
    {
        ...
        if ((int) ($hints['autodoc_listing_wrap'] ?? 0) > 0) {
            $out[] = ScraperLlmConfig::hasAnyKey()
                ? 'Detectat catalog Autodoc — agentul poate folosi LLM (Groq/OpenAI).'
                : 'Detectat catalog Autodoc — adaugă GROQ_KEY sau OPENAI_KEY pentru auto-configurare.';
        }

        return $out;
    }

    private static function metroOrchestratorAvailable(): bool
    {
        $bridge = __DIR__ . '/ScraperMetroLlmBridge.php';

        return is_file($bridge);
    }
     ...
     */
    private static function tryMetroOrchestrator(
        string $html,
        string $sourceId,
        string $userGoals,
        array $fieldsNeeded,
        ...
        try {
            $metro = ScraperMetroLlmBridge::analyzeHtml(
                ScraperPaths::projectRoot(),
                $html,
                $sourceId,
                $userGoals,
                $fieldsNeeded
            ...
            if (empty($metro['ok'])) {
                ScraperLogger::log('warn', 'ScraperAiAgent Metro: ' . (string) ($metro['error'] ?? 'eșec'));

                return null;
            }

            $selectors = self::normalizeSelectors(
                is_array($metro['selectors'] ?? null) ? $metro['selectors'] : [],
                $fieldsNeeded
            );
            ...
            $items = $validated['items'];
            $itemsCount = $validated['items_count'];
            if ($itemsCount === 0 && is_array($metro['items'] ?? null) && $metro['items'] !== []) {
                $items = self::normalizeAiItems($metro['items'], $limit);
                $itemsCount = count($items);
            }
            ...
            return [
                'ok' => $itemsCount > 0 || $selectors !== [],
                'mode' => (string) ($metro['mode'] ?? 'metro-orchestra'),
                'llm_used' => true,
                'provider' => (string) ($metro['provider'] ?? 'metro'),
                'model' => (string) ($metro['model'] ?? ''),
                'selectors' => $selectors,
                'items' => $items,
                'items_count' => $itemsCount,
                'diagnostics' => $validated['diagnostics'],
                'explanation_ro' => (string) ($metro['explanation_ro'] ?? 'Analiză Metro AI Orchestra.'),
                'compass' => $context['compass'],
                'html_hints' => $context['html_hints'],
                'user_goals' => $userGoals,
            ];
        } catch (Throwable $e) {
            ScraperLogger::log('warn', 'ScraperAiAgent Metro: ' . $e->getMessage());
