...
final class EpiesaScrapeJob
{
    ...
    public static function run(
        ...
        ScraperLogger::log('info', 'Start scan ePiesa (stealth) | categorie=' . $category['label'] . ' | url=' . $url);
        ...
        $html = EpiesaHtmlFetcher::fetch($url, 120);
        ...
        $payload = [
            ...
            'category_slug' => $category['slug'],
            'category_label' => $category['label'],
            'fetch_engine' => 'stealth_browser_mcp',
        ...
        ScraperLogger::log(
            'info',
            'Scan ePiesa finalizat | extrase=' . count($products) . ' | noi în catalog=' . $added . ' | total catalog=' . $catalogTotal
        );
