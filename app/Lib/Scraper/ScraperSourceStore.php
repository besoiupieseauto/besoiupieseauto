...
final class ScraperSourceStore
{
    ...
    public static function defaultConfig(string $sourceId): array
    {
        ...
        $outputFields = !empty($meta['no_html'])
        ...
        return [
            ...
            'fetch' => [
                'via' => (string) ($meta['fetch_via'] ?? 'stealth_browser'),
                'timeout_sec' => 120,
                'super' => false,
                'render' => false,
                'save_raw' => true,
    ...
    private static function defaultStepsFor(string $sourceId, array $meta): array
    {
        ...
        $urlTemplates = [
            'epiesa' => 'https://www.epiesa.ro/cautare-piesa/?q={query}',
    ...
    public static function load(string $sourceId): array
    {
        ...
        return self::normalizeHtmlFetchConfig($sourceId, $merged, $registry[$sourceId]);
    }

    /**
     * Toate sursele HTML folosesc stealth-browser-mcp (nodriver) — nu scrape.do implicit.
     *
     * @param array<string, mixed> $config
     * @param array<string, mixed> $meta
     * @return array<string, mixed>
     */
    private static function normalizeHtmlFetchConfig(string $sourceId, array $config, array $meta): array
    {
        if (!empty($meta['no_html'])) {
            return $config;
        }
        $fetch = is_array($config['fetch'] ?? null) ? $config['fetch'] : [];
        $via = strtolower(trim((string) ($fetch['via'] ?? '')));
        if ($via === '' || in_array($via, ['scrape_do', 'auto'], true)) {
            $fetch['via'] = 'stealth_browser';
            $config['fetch'] = $fetch;
        }
    ...
    public static function save(string $sourceId, array $config): array
    {
        ...
        $merged = self::normalizeHtmlFetchConfig($sourceId, $merged, self::registry()[$sourceId]);
