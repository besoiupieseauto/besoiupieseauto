...
final class PipelineImageAiGate
{
    ...
    public static function filterHit(array $product, array $hit, array $opts = []): array
    {
        ...
        if (isset($opts['min_score_keep'])) {
            $rules['min_score_keep'] = max(0, min(100, (int) $opts['min_score_keep']));
        }
        ...
        if (!empty($opts['reject_partial_verdict']) && ($heuristic['verdict'] ?? '') === 'partial') {
            if (self::isExactSupplierCodeMatch($product, $hit)) {
                $score = max(85, (int) ($heuristic['score'] ?? 0));
                $msg = trim((string) ($heuristic['message'] ?? ''));
                if ($msg === '') {
                    $msg = 'AI heuristic: OK (' . $score . '/100)';
                }

                return self::accept($score, 'match', $msg . ' — cod furnizor confirmat pe sursă');
            }

            return self::reject(
                (int) ($heuristic['score'] ?? 0),
                'partial',
                'Scor parțial insuficient pentru import — necesită potrivire clară (min 85).'
            );
        }
    ...
    /** @param array<string, mixed> $product @param array<string, mixed> $hit */
    private static function isExactSupplierCodeMatch(array $product, array $hit): bool
    {
        require_once __DIR__ . '/CatalogPartImageMatch.php';
        $title = trim((string) ($hit['title'] ?? ''));
        $hitCode = trim((string) ($hit['code'] ?? $hit['sku'] ?? ''));

        return CatalogPartImageMatch::productCodeMatchesHit($product, $title, $hitCode);
    }
    ...
    private static function heuristicValidate(array $product, array $hit, array $rules): array
    {
        ...
        if (function_exists('besoiu_detect_fluid_family') && function_exists('besoiu_fluid_family_conflict')) {
            $wantFamily = besoiu_detect_fluid_family(trim(implode(' ', [$name, $category, $subcategory])));
            $hitFamily = besoiu_detect_fluid_family($hitTitle);
            if (besoiu_fluid_family_conflict($wantFamily, $hitFamily)) {
                return self::reject(
                    10,
                    'mismatch',
                    'Familie fluid nepotrivită (ex. lichid frână vs ulei motor) — «' . $hitTitle . '»'
                );
            }
        }

        if (!empty($rules['reject_placeholder'])) {
            foreach (['brands/thumbs', '360-icon', '.svg', 'lazyload.php', 'placeholder'] as $bad) {
            ...
            if (str_contains($url, '/assets/')) {
                if (self::isTrustedScraperAssetUrl($url)) {
                    $score += 12;
                } elseif (str_starts_with($url, '/assets/scraper/')) {
                    return self::reject(
                        5,
                        'mismatch',
                        'Cache imagine scraper lipsă sau invalidă (' . $url . ').'
                    );
                } else {
                    return self::reject(5, 'mismatch', 'Imagine respinsă: logo / placeholder (/assets/).');
                }
            }
        }
        ...
        if ($wantBrand !== '' && $hitTitle !== '') {
            ...
            if (!self::titleContainsBrand($titleLower, $wantLower)) {
    ...
    private static function visionValidate(array $product, array $hit, array $rules): array
    {
        ...
        try {
            $service = new \Besoiu\Services\ProductImageAuditService(dirname(__DIR__, 2));
            ...
            return self::accept($score, $verdict !== '' ? $verdict : 'match', 'AI Vision: ' . ($summary !== '' ? $summary : ('scor ' . $score)), 'openai_vision');
        } catch (Throwable $e) {
    ...
    private static function categoryMatchScore(string $name, string $category, string $subcategory, string $hitTitle): int
    {
        ...
        $groups = [
            ...
            'brake_fluid' => ['lichid frana', 'lichid frână', 'brake fluid', 'drauliquid', 'dot 4', 'dot 3'],
            'motor_oil' => ['ulei motor', 'motor oil', '5w-', '10w-', '15w-', 'emp'],
            'coolant' => ['antigel', 'coolant'],
            'pasta_montaj' => ['pasta montaj', 'pasta de montaj', 'copper paste', 'montaj'],
            'brake' => ['disc', 'placute', 'plăcuțe', 'etrier'],
            'suspension' => ['suspensie', 'amortizor', 'arc', 'rulment', 'bieleta', 'cap bara', 'bara stabilizatoare'],
            'engine' => ['piston', 'cuzinet', 'segment', 'chiulas'],
        ];
    ...
    private static function titleContainsBrand(string $titleLower, string $wantLower): bool
    {
        if ($wantLower === '' || $titleLower === '') {
            return false;
        }
        if (str_contains($titleLower, $wantLower)) {
            return true;
        }
        foreach (preg_split('/\s+/u', $wantLower) ?: [] as $token) {
            $token = trim((string) $token);
            if (strlen($token) >= 4 && str_contains($titleLower, $token)) {
                return true;
            }
        }
    ...
    private static function isTrustedScraperAssetUrl(string $url): bool
    {
        $url = trim($url);
        ...
        $validatePath = dirname(__DIR__, 2) . '/system/import-image-validate.php';
        if (is_file($validatePath)) {
            require_once $validatePath;
        }

        return function_exists('besoiu_import_image_scraper_asset_exists')
            && besoiu_import_image_scraper_asset_exists($url);
    }

    /** @return array{accepted: bool, score: int, verdict: string, message: string, provider?: string} */
    private static function accept(int $score, string $verdict, string $message, string $provider = 'heuristic'): array
    {
        return ['accepted' => true, 'score' => $score, 'verdict' => $verdict, 'message' => $message, 'provider' => $provider];
    }

    /** @return array{accepted: bool, score: int, verdict: string, message: string, provider?: string} */
    private static function reject(int $score, string $verdict, string $message, string $provider = 'heuristic'): array
    {
        return ['accepted' => false, 'score' => $score, 'verdict' => $verdict, 'message' => $message, 'provider' => $provider];
    }
}
