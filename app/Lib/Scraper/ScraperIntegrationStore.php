...
final class ScraperIntegrationStore
{
    ...
    public static function load(): array
    {
        ...
        $rawPlanCount = count((array) ($raw['image_plans'] ?? []));
        $normalized = ScraperIntegrationSchema::normalizeGlobal($raw);
        ...
        if (is_file($overlayPath)) {
            ...
            if (is_array($overlay['image_plans'] ?? null) && $overlay['image_plans'] !== []) {
                $overlayTs = strtotime((string) ($overlay['synced_at'] ?? ''));
                $cfgTs = strtotime((string) ($normalized['updated_at'] ?? ''));
                $overlayPlans = ScraperIntegrationSchema::dedupePlansBySource(
                    array_values(array_filter($overlay['image_plans'], 'is_array'))
                );
                $cfgPlans = (array) ($normalized['image_plans'] ?? []);
                $plansDiffer = json_encode($overlayPlans) !== json_encode($cfgPlans);
                if ($plansDiffer && ($overlayTs !== false && ($cfgTs === false || $overlayTs >= $cfgTs))) {
                    $normalized['image_plans'] = $overlayPlans;
                    if (is_array($overlay['image_ai'] ?? null)) {
                        $normalized['image_ai'] = array_replace_recursive(
                            (array) ($normalized['image_ai'] ?? []),
                            $overlay['image_ai']
                        );
                    }
        ...
        if ($rawPlanCount !== count((array) ($normalized['image_plans'] ?? []))) {
            return self::save($normalized);
        }
