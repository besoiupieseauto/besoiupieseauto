...
final class ImageSearchService
{
    ...
    public static function findImage(array $product, array $opts = []): array
    {
        ...
        if ($fastMode) {
            // fast_mode: limitează adâncimea TecDoc în cadrul pipeline-ului complet (nu sare Autodoc/ePiesa).
            $opts['fast_mode'] = true;
        }
        ...
        $apiError = self::firstApiErrorFromTried(is_array($resolved['tried'] ?? null) ? $resolved['tried'] : []);
        if ($apiError === null && function_exists('tecdoc_last_api_error')) {
            $last = tecdoc_last_api_error();
            if (is_array($last) && trim((string) ($last['message'] ?? '')) !== '') {
                $apiError = (string) $last['message'];
            }
        }

        if (function_exists('import_find_image_legacy_fallbacks')) {
            $legacy = ImportImageBridge::normalizeHit(import_find_image_legacy_fallbacks(
                $resolved['product'],
                $opts['tecdoc_files'] ?? null,
                $opts['shared_lookup'] ?? null
            ));
            if (trim((string) ($legacy['url'] ?? '')) !== '') {
                return $legacy;
            }
            if ($apiError === null && trim((string) ($legacy['api_error'] ?? '')) !== '') {
                $apiError = (string) $legacy['api_error'];
            }
        }
    ...
    private static function firstApiErrorFromTried(array $tried): ?string
    {
        ...
        foreach (array_reverse($tried) as $step) {
            if (!is_array($step)) {
                continue;
            }
            $msg = trim((string) ($step['message'] ?? ''));
