...
require_once __DIR__ . '/StealthBrowserCleanup.php';
require_once __DIR__ . '/EpiesaCategoryParser.php';
require_once __DIR__ . '/AutodocListingParser.php';
...
require_once __DIR__ . '/TecDocApiTestRunner.php';
...
final class ScraperStepRunner
{
    ...
    public static function runSource(string $sourceId, ?array $config = null, array $options = []): array
    {
        ...
        if (!isset($config['id']) || (string) $config['id'] === '') {
            $config['id'] = $sourceId;
        }
        ...
        if (!empty($meta['no_html'])) {
            return self::runApiSource($sourceId, $query, $limit, $config);
        }
        ...
        StealthBrowserCleanup::prepareBeforeRun();
            ...
            try {
                if ($type === 'fetch' || !empty($step['is_login'])) {
                    ...
                    if (!empty($step['is_login'])) {
                        ...
                        $htmlLen = strlen($context['html']);
                        $fetchStatus = 'ok';
                        $profile = is_array($fetch['browser_profile'] ?? null) ? $fetch['browser_profile'] : [];
                        $uaLabel = trim((string) ($profile['label'] ?? ''));
                        $fetchMsg = $htmlLen . ' bytes HTML';
                        if ($uaLabel !== '') {
                            $fetchMsg .= ' · UA: ' . $uaLabel;
                        }
                        if (ScraperHubTester::htmlIsCloudflareChallenge($context['html'])) {
                            $fetchStatus = 'error';
                            $fetchMsg = $htmlLen . ' bytes — Cloudflare «Just a moment» (nu e lista de produse)';
                            if ($uaLabel !== '') {
                                $fetchMsg .= ' · UA: ' . $uaLabel;
                            }
                        }
                        $trace[] = self::traceStep($step, $fetchStatus, $fetchMsg, $fetch);
                    }
                ...
                } elseif ($type === 'vehicle_tree') {
                    if ($sourceId !== 'epiesa') {
                        throw new RuntimeException('Pas vehicle_tree este doar pentru sursa epiesa.');
                    }
                    $crawlOpts = is_array($step['params'] ?? null) ? $step['params'] : [];
                    if (!isset($crawlOpts['delay_ms'])) {
                        $crawlOpts['delay_ms'] = 120;
                    }
                    $testCapped = false;
                    if (!empty($options['test_mode'])) {
                        if ((int) ($crawlOpts['max_brands'] ?? 0) === 0) {
                            $crawlOpts['max_brands'] = 1;
                            $testCapped = true;
                        }
                        if ((int) ($crawlOpts['max_models_per_brand'] ?? 0) === 0) {
                            $crawlOpts['max_models_per_brand'] = 2;
                            $testCapped = true;
                        }
                    }
                    $result = EpiesaVehicleTreeCrawler::crawl($crawlOpts);
                    if ($testCapped) {
                        $result['test_mode_capped'] = true;
                        $result['message'] = ($result['message'] ?? 'Arbore sincronizat')
                            . ' (test: max 1 marcă, 2 modele — setează limite explicite pentru crawl complet)';
                    }
                    $context['vehicle_tree'] = $result;
                    $context['vehicle_combos'] = $result['sample_combos'] ?? [];
                    $trace[] = self::traceStep($step, 'ok', (string) ($result['message'] ?? 'Arbore vehicule sincronizat'), $result);
        ...
        return $result;
            ...
            if (self::sourceUsesStealth($config)) {
                StealthBrowserCleanup::afterRun(true);
            }
        }
    }

    /** @param array<string, mixed> $config */
    private static function sourceUsesStealth(array $config): bool
    {
        $via = strtolower(trim((string) (($config['fetch'] ?? [])['via'] ?? 'stealth_browser')));

        return in_array($via, ['stealth_browser', 'auto', ''], true);
    }
    ...
    private static function resolveFetchOptions(array $config, array $options): array
    {
        ...
        $out = [
            ...
            'via' => (string) ($base['via'] ?? 'stealth_browser'),
            'source_id' => (string) ($config['id'] ?? $options['source_id'] ?? ''),
        ];
    ...
    private static function buildUrl(string $template, string $query): string
    {
        ...
        if (str_contains($template, '{query}')) {
            $built = str_replace('{query}', rawurlencode($q), $template);

            return (string) preg_replace('#(?<!:)/{2,}#', '/', $built);
        }
    ...
    private static function parseList(string $sourceId, array $step, string $html, int $limit): array
    {
        ...
        if ($builtin === 'AutodocListingParser' || $sourceId === 'autodoc') {
            return AutodocListingParser::parse($html, $limit);
        }
     ...
     * @return array<string, mixed>
     */
    private static function runApiSource(string $sourceId, string $query, int $limit, array $config): array
    {
        if ($sourceId === 'tecdoc_api') {
            $result = TecDocApiTestRunner::run($query, $limit);
            $result['source_id'] = $sourceId;
            $fieldsNeeded = is_array($config['output']['fields_needed'] ?? null)
                ? $config['output']['fields_needed']
                : ['title', 'image', 'oem', 'description'];
            $result['fields_needed'] = $fieldsNeeded;
            if (($result['items_count'] ?? 0) > 0 && is_array($result['items'] ?? null)) {
                $result['matched_fields'] = self::matchedFields($result['items'], $config);
            }
        ...
        return [
            ...
            'trace' => [[
                ...
                'label' => 'Sursă API',
                'status' => 'info',
                'message' => 'Test API neimplementat pentru această sursă.',
            ]],
            'items' => [],
            'items_count' => 0,
            'analysis' => ['quality_score' => 0, 'integration_ready' => false, 'issues' => ['Sursă API fără runner dedicat']],
            'fields_needed' => [],
            'matched_fields' => [],
        ];
    }
