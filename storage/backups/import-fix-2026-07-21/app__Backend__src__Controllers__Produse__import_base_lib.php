...
/** Din HTML descriere Import Pro: doar blocul „Compatibil cu”. */
function import_compat_text_from_description_html(string $html): string
{
    ...
    if (preg_match('/Compatibil\s+cu:\s*<\/[^>]+>\s*<ul>(.*?)<\/ul>/is', $html, $m)) {
        $lines = [];
        if (preg_match_all('/<li[^>]*>(.*?)<\/li>/is', (string) $m[1], $items)) {
    ...
    if (preg_match('/Compatibil\s+cu:\s*(.+)$/is', $plain, $m)) {
