...
final class ProductCardFormationService
{
    ...
    public function applyToImportProduct(array $product, array $rawPayload = []): array
    {
        ...
        try {
            ...
            // Preferă mărci/modele structurale (pMarca/pModel) — evită FRANA/COD din specificații.
            ...
            return [
                ...
                'product_formation' => [
                    ...
                    'title_oem' => $titleOem,
                    'source_name' => $sourceName !== '' ? $sourceName : $pieceName,
                    'applied_at' => date('c'),
                    'applied' => true,
                ],
            ];
    ...
    {
        $columnTitle = trim((string) ($row['pNameMarketplace'] ?? ''));
        if ($columnTitle !== '') {
            return $columnTitle;
        }
        ...
        try {
            ...
            $title = trim((string) ($applied['product_formation']['title_pieseauto'] ?? ''));
            if ($title !== '') {
                return $title;
            }
        } catch (\Throwable $e) {
            error_log('[ProductCardFormationService] resolveMarketplaceTitleFromRow: ' . $e->getMessage());
        }
    ...
    private function resolveBasePieceName(array $product, array $rawPayload, array $entries): string
    {
        ...
        $candidates = [
            trim((string) ($rawPayload['__tecdoc_art_name'] ?? '')),
            trim((string) ($summary['tecdoc_art_name'] ?? '')),
            trim((string) ($formation['source_name'] ?? '')),
        ];
        ...
        foreach ($candidates as $candidate) {
            if ($candidate !== '') {
                if (function_exists('import_base_normalize_product_name')) {
                    return import_base_normalize_product_name($candidate);
                }
        ...
        if ($name !== '' && function_exists('import_base_strip_redundant_title_suffix')) {
            return import_base_strip_redundant_title_suffix($name, $brand, $code);
        }
    ...
    private function compatFromProductText(array $product, array $rawPayload): array
    {
        ...
        if ($desc !== '' && function_exists('import_compat_text_from_description_html')) {
            ...
            if (preg_match('/Compatibil\s+cu:\s*(.+)$/is', strip_tags($desc), $m)) {
    ...
    private function resolveSegmentValues(array $ctx, array $channelCfg): array
    {
        ...
        if ($motor !== '') {
            ...
            if (function_exists('mb_strlen') && mb_strlen($motor, 'UTF-8') > 40) {
                $motor = '';
            }
        }
