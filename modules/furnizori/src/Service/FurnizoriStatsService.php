...
class FurnizoriStatsService
{
    ...
    public function __construct(
        private readonly FurnizoriRepository $furnizoriModel
    ) {
    }
    ...
    {
        $supplierColumn = $this->supplierColumn($pdo, 'produse');
        $countStmt = $pdo->prepare(
            "SELECT COUNT(*) FROM produse WHERE {$supplierColumn} = :code"
        );
    ...
    {
        $supplierColumn = $this->supplierColumn($pdo, 'import_produse');
        $placeholders = implode(',', array_fill(0, count($statuses), '?'));
        $params = array_merge([$code], $statuses);

        $countStmt = $pdo->prepare(
            'SELECT COUNT(*) FROM import_produse
             WHERE ' . $supplierColumn . ' = ?
    ...
    private function loadProductStatsForSupplierCode(string $code): array
    {
        ...
        $pdo = Database::getDB();
        $publishedSupplierColumn = $this->supplierColumn($pdo, 'produse');
        $publishedStmt = $pdo->prepare(
            "SELECT COUNT(*) FROM produse WHERE {$publishedSupplierColumn} = :code"
        );
        ...
        $queue = 0;
        $queuePending = 0;
        $queueSupplierColumn = $this->supplierColumn($pdo, 'import_produse');
        $queueStmt = $pdo->prepare(
            'SELECT status, COUNT(*) AS cnt
             FROM import_produse
             WHERE ' . $queueSupplierColumn . ' = :code
             GROUP BY status'
        );
        $queueStmt->execute([':code' => $code]);
    ...
    private function loadProductStatsBySupplierCode(): array
    {
        ...
        foreach ($pdo->query(
            "SELECT {$publishedSupplierColumn} AS supplier_code, COUNT(*) AS cnt
             FROM produse
             WHERE {$publishedSupplierColumn} IS NOT NULL AND {$publishedSupplierColumn} <> ''
             GROUP BY {$publishedSupplierColumn}"
        ) as $row) {
        ...
        $queueSupplierColumn = $this->supplierColumn($pdo, 'import_produse');
        foreach ($pdo->query(
            "SELECT {$queueSupplierColumn} AS supplier_code, status, COUNT(*) AS cnt
             FROM import_produse
             WHERE {$queueSupplierColumn} IS NOT NULL AND {$queueSupplierColumn} <> ''
             GROUP BY {$queueSupplierColumn}, status"
        ) as $row) {
            $code = (string) ($row['supplier_code'] ?? '');
            if ($code === '') {
                continue;
    ...
    private function supplierColumn(PDO $pdo, string $table): string
    {
        static $cache = [];
        ...
        $stmt->execute([$table, 'pSupplierNorm']);
        return $cache[$table] = (int) $stmt->fetchColumn() > 0 ? 'pSupplierNorm' : 'pSupplier';
    }
