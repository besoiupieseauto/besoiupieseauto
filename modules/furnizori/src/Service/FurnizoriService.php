...
class FurnizoriService
{
    public function __construct(
        private FurnizoriRepository $furnizoriModel,
    ...
    public function updateFurnizor(int $randomId, array $payload): array
    {
        ...
        $stockZeroLib = (defined('BESOIU_BACKEND') ? BESOIU_BACKEND : dirname(__DIR__, 4) . '/app/Backend') . '/src/Controllers/Produse/import_supplier_stock_zero_lib.php';
    ...
    public function changeFurnizorStatus(int $randomId, string $status): void
    {
        ...
        $importLib = (defined('BESOIU_BACKEND') ? BESOIU_BACKEND : dirname(__DIR__, 4) . '/app/Backend') . '/src/Controllers/Produse/import_supplier_lib.php';
        ...
        if ($status === 'blocked') {
            try {
                (new PriceFormationLogicService())->syncBlockedFreeze();
            } catch (\Throwable) {
                // logica de preț se re-sincronizează la următorul getConfig
            }
        }
    }
