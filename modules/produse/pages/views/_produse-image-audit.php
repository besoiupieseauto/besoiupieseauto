...
<style>
    ...
    .image-audit-progress__fill {
        ...
        width: 0%;
        border-radius: 999px;
        background: linear-gradient(90deg, #0d9488, #1abc9c);
    ...
    .image-audit-card--pending { border-left: 4px solid #5eead4; }
    ...
    .image-audit-badge--pending { background: #ede9fe; color: #0f766e; }
    ...
    <div class="image-audit-modal__panel">
        ...
        <div class="image-audit-modal__body" id="imageAuditModalBody">
            ...
            <div id="imageAuditCursorBox" class="image-audit-cursor-box" hidden>
                <strong style="font-size:13px;color:#5b21b6;">Mod manual (Composer IDE)</strong>
                <ol>
                    <li>Setează <code>IMAGE_AUDIT_ENGINE=openai</code> + <code>OPENAI_KEY</code> pentru audit automat</li>
                    <li>Sau deschide <strong>Cursor IDE</strong> → <strong>Composer</strong></li>
...
window.besoiuImageAudit = (function () {
    ...
    function shouldUseAsyncAudit(ids, options) {
        const opts = options || {};
        if (typeof BesoiuAsync === 'undefined') {
            return false;
        }
        if (opts.all || (opts.filters && Object.keys(opts.filters).length > 0)) {
            return true;
        }
        return (ids || []).filter(Boolean).length > 1;
    }

    function buildCursorPromptFromBatchPaths(batchPaths) {
        const paths = (batchPaths || []).map((p) => String(p || '').trim()).filter(Boolean);
        if (!paths.length) {
            return '';
        }
        const lines = [
            '@product-image-audit',
            '',
            'Analizează vizual loturile pregătite în fundal:',
        ];
        paths.forEach((path, index) => {
            const name = path.split(/[\\/]/).pop() || ('lot-' + (index + 1));
            lines.push((index + 1) + '. **' + name + '** — `' + path.replace(/\\/g, '/') + '`');
        });
        lines.push(
            '',
    ...
    async function submitAuditAsyncJob(payload) {
        const created = await BesoiuAsync.submitJob({
            type: 'images.audit_batch',
            queue: 'images',
            payload: Object.assign({}, payload, { circuit_service: 'openai' }),
            timeout_sec: 900,
            idempotencyKey: 'audit-' + (payload.all ? 'all' : JSON.stringify(payload.ids || payload.filters || '')).slice(0, 180),
        });
        if (!created.success || !created.job_id) {
            throw new Error(created.message || 'Nu am putut porni auditul în fundal.');
        }
        return created.job_id;
    }

    async function watchAuditAsyncJob(jobId) {
        return new Promise((resolve, reject) => {
            const watch = BesoiuAsync.watchJob(jobId, {
                onStatus: (snap) => {
                    const progress = Number(snap.progress || 0);
                    const message = String(snap.message || 'Audit imagini în fundal…');
                    updateProgressUi({
                        phase: message,
                        total: Number(snap.result?.total || lastProducts.length || 0),
                        done: Number(snap.result?.audited || snap.result?.processed || 0),
                        percent: progress,
                        status: 'running',
                ...
                onFailed: (snap) => reject(new Error(String(snap.error || snap.message || 'Audit imagini eșuat.'))),
        ...
        while (activeJobId === jobId) {
            ...
            if (data.finished || data.status === 'done' || data.status === 'error') {
                ...
                if (data.status === 'done' && doneCount === 0) {
                    ...
                    setActivity('Setează OPENAI_KEY sau rulează auditul manual în Composer (@product-image-audit).');
    ...
    function renderProductPreview(product, result) {
        ...
        if (hasResult && verdict === 'no_image') {
            body = '<div class="image-audit-card__section" style="color:#0d9488;">'
            ...
            body = '<div class="image-audit-card__section" style="color:#0f766e;">' + escapeHtml(statusHint) + '</div>';
        } else {
    ...
    async function runAudit(ids, options) {
        const opts = options || {};
        const selectAllCatalog = !!opts.all;
        const serverFilters = (opts.filters && typeof opts.filters === 'object') ? opts.filters : null;
        let list = (ids || []).filter(Boolean);
        const useAsync = shouldUseAsyncAudit(list, opts);

        if (!selectAllCatalog && !serverFilters && !list.length) {
            alert('Selectează cel puțin un produs.');
            return null;
        }

        const expectedCount = selectAllCatalog
            ? Number(opts.count || 0)
            : (serverFilters ? Number(opts.count || 0) : list.length);

        if (!useAsync && !selectAllCatalog && !serverFilters && list.length > AUDIT_MAX_BATCH) {
            alert('Maxim ' + AUDIT_MAX_BATCH + ' produse per audit (ai ' + list.length + ').');
            return null;
        }
        if (!useAsync && selectAllCatalog && expectedCount > AUDIT_MAX_BATCH) {
        ...
        try {
        lastIds = selectAllCatalog || serverFilters ? [] : list.slice();
        stopPolling();
        lastAuditById = {};
        lastPipelineById = {};
        clearActivityLog();
        ...
        analyzingProductId = selectAllCatalog || serverFilters ? '' : list[0];
        let preview = { products: [], results: [] };

        if (!selectAllCatalog && !serverFilters) {
            preview = await loadPreview(list);
            applyAuditPreview(preview);
            if (progressFill) progressFill.style.width = '15%';
        } else if (serverFilters) {
            setProgressLine('Pregătesc audit pe setul filtrat (' + (expectedCount || '?') + ' produse)…');
        ...
        if (useAsync) {
            setProgressLine('Pornesc audit imagini în fundal (worker async)…');
            setActivity('Job în coadă — progresul vine prin SSE, fără blocare PHP-FPM.');
            appendActivityLog('Trimit job images.audit_batch…');

            const asyncPayload = { chunk_size: 25 };
            if (selectAllCatalog) {
                asyncPayload.all = true;
            } else if (serverFilters) {
                asyncPayload.filters = serverFilters;
            } else {
                asyncPayload.ids = list;
            }

            const jobId = await submitAuditAsyncJob(asyncPayload);
            appendActivityLog('Job ' + jobId + ' pornit — urmăresc progresul.');
            const asyncResult = await watchAuditAsyncJob(jobId);
            const batchPaths = Array.isArray(asyncResult.batch_paths) ? asyncResult.batch_paths : [];
            const cursorPrompt = buildCursorPromptFromBatchPaths(batchPaths);
            const audited = Number(asyncResult.audited || 0);
            const total = Number(asyncResult.total || expectedCount || audited);

            if (cursorPrompt) {
                if (cursorBox) cursorBox.hidden = false;
                if (promptEl) promptEl.value = cursorPrompt;
            }

            if (lastIds.length) {
                const reloaded = await loadPreview(lastIds);
                mergeResults(lastProducts.length ? lastProducts : reloaded.products, reloaded.results || []);
            } else if (preview.products.length) {
                mergeResults(preview.products, preview.results || []);
            }

            updateProgressUi({
                phase: asyncResult.message || ('Audit pregătit: ' + audited + ' / ' + total),
                total: total,
                done: audited,
                percent: 100,
                status: 'done',
            ...
            setProgressLine(asyncResult.message || 'Lot pregătit pentru Composer 2.5.');
            setActivity(cursorPrompt
                ? 'Deschide Cursor → Composer 2.5, lipește promptul (@product-image-audit), apoi Încarcă rezultate.'
                : 'Audit finalizat în fundal.');
            if (progressFill) progressFill.style.width = '100%';
            stopPolling();
            return {
                success: true,
        ...
        if (!result.success) {
            ...
            if (cached.results.length) {
                ...
                setActivity('Verifică OPENAI_KEY în admin/.env sau reîncearcă auditul.');
                setProgressState('error');
                await runPipelineRetryAfterAudit();
                stopPolling();
                return result;
        ...
        if (auditResults.length > 0) {
            ...
            setProgressState('error');
            setActivity(result.message || 'Nu am putut analiza imaginea. Verifică OPENAI_KEY sau modul manual.');
        }
        stopPolling();
        return result;
        } catch (err) {
