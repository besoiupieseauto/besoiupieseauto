...
$clientRoot = dirname(__DIR__, 4);
require_once (defined('BESOIU_LEGACY') ? BESOIU_LEGACY : $clientRoot . '/app/Legacy') . '/home-vitrina-render.php';
...
$vitrinaActivePage = max(1, (int) ($_GET['vpage'] ?? 1));
$vitrinaActivePerPage = 20;

$vitrinaCount = $service->countVitrinaProducts();
$vitrinaHomeMax = $service->vitrinaHomepageMax();
$vitrinaHomeRows = $service->getVitrinaProducts($vitrinaHomeMax);

$homeVitrinaProducts = [];
foreach ($vitrinaHomeRows as $row) {
    if (is_array($row)) {
...
$vitrinaActivePaged = $service->getVitrinaProductsPaginated($vitrinaActivePage, $vitrinaActivePerPage);
$vitrinaOnSite = $vitrinaActivePaged['items'] ?? [];
$vitrinaActiveTotal = (int) ($vitrinaActivePaged['total'] ?? $vitrinaCount);
$vitrinaActiveTotalPages = max(1, (int) ($vitrinaActivePaged['total_pages'] ?? 1));
$vitrinaActiveCurrentPage = (int) ($vitrinaActivePaged['page'] ?? $vitrinaActivePage);
if ($vitrinaActiveCurrentPage > $vitrinaActiveTotalPages) {
    $vitrinaActiveCurrentPage = $vitrinaActiveTotalPages;
}
...
$buildVitrinaQuery = static function (array $overrides = []) use ($search, $vitrinaActivePage): string {
    $params = [];
    if ($search !== '') {
        $params['q'] = $search;
    }
    if ($vitrinaActivePage > 1) {
        $params['vpage'] = $vitrinaActivePage;
    }
...
$buildVitrinaActiveQuery = static function (array $overrides = []) use ($search): string {
    $params = [];
    if ($search !== '') {
        $params['q'] = $search;
    }
...
<div class="-mt-5 admin-content produse-list-page produse-section-page" id="besoiuVitrinaPage" data-page-title="Produse vetrină">
<div class="admin-panel">
    <div class="admin-panel__head">
        <div class="pl-head-main">
            <span class="pl-head-icon" aria-hidden="true"><i class="fa-solid fa-store"></i></span>
            <h2 class="mt-0 text-lg font-medium">Vetrina homepage</h2>
        </div>
        <span class="pl-head-meta"><span class="pl-head-meta__dot" aria-hidden="true"></span><i class="fa-solid fa-bolt"></i> Pe vitrină · <?= (int) $vitrinaCount ?> / <?= (int) $vitrinaHomeMax ?></span>
    </div>
    <?php require __DIR__ . '/_produse-section-nav.php'; ?>

<div class="besoiu-page besoiu-produse-page besoiu-vitrina-page">
        ...
        <div class="besoiu-dash-hero__actions" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
            ...
            <?php if ($vitrinaCount > 0): ?>
                <button type="button" class="besoiu-btn-secondary besoiu-vitrina-clear-all-btn" id="vitrinaClearAllBtn" style="border-color:#fca5a5;color:#b91c1c;">
                    <i class="fa-solid fa-trash-can"></i> Golește vitrina (<?= (int) $vitrinaCount ?>)
                </button>
            <?php endif; ?>
        </div>
    </div>
    ...
    <section class="besoiu-produse-panel besoiu-vitrina-on-panel" style="margin-bottom:1.5rem;">
        ...
        <?php if ($vitrinaActiveTotalPages > 1): ?>
            <nav class="besoiu-pagination" aria-label="Paginare produse pe vitrină" style="margin-top:12px;">
                <span class="besoiu-counter"><?= (($vitrinaActiveCurrentPage - 1) * $vitrinaActivePerPage) + 1 ?>–<?= min($vitrinaActiveCurrentPage * $vitrinaActivePerPage, $vitrinaActiveTotal) ?> din <?= (int) $vitrinaActiveTotal ?></span>
                <?php if ($vitrinaActiveCurrentPage > 1): ?>
                    <a class="besoiu-pagination__btn" href="?<?= produse_list_h($buildVitrinaActiveQuery(['vpage' => $vitrinaActiveCurrentPage - 1])) ?>">‹</a>
                <?php endif; ?>
                <?php for ($vp = max(1, $vitrinaActiveCurrentPage - 2); $vp <= min($vitrinaActiveTotalPages, $vitrinaActiveCurrentPage + 2); $vp++): ?>
                    <a class="besoiu-pagination__btn<?= $vp === $vitrinaActiveCurrentPage ? ' besoiu-pagination__btn--active' : '' ?>"
                       href="?<?= produse_list_h($buildVitrinaActiveQuery(['vpage' => $vp])) ?>"><?= $vp ?></a>
                <?php endfor; ?>
                <?php if ($vitrinaActiveCurrentPage < $vitrinaActiveTotalPages): ?>
                    <a class="besoiu-pagination__btn" href="?<?= produse_list_h($buildVitrinaActiveQuery(['vpage' => $vitrinaActiveCurrentPage + 1])) ?>">›</a>
                <?php endif; ?>
            </nav>
        <?php endif; ?>
