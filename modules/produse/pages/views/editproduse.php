...
$badgeConfig = require dirname(__DIR__, 4) . '/app/Config/product-badges.php';
...
$productStatusOptions = require dirname(__DIR__, 4) . '/app/Backend/config/product-status.php';
...
$imageAuditLast = null;
$auditRoot = dirname(__DIR__, 4);
$auditServicePath = $auditRoot . '/app/Backend/src/Services/ProductImageAuditService.php';
if (is_file($auditServicePath)) {
    require_once $auditServicePath;
    $imageAuditLast = (new \Besoiu\Services\ProductImageAuditService($auditRoot))->loadProductAuditResult($id);
}

$marketplaceTitleValue = oldv($product, 'pNameMarketplace');
if ($marketplaceTitleValue === '' && function_exists('besoiu_resolve_marketplace_product_title')) {
    $marketplaceTitleValue = besoiu_resolve_marketplace_product_title($product);
}
if ($marketplaceTitleValue === '') {
    $marketplaceTitleValue = oldv($product, 'pName');
}

$productFormValues = [
    'pName' => oldv($product, 'pName'),
    'pNameMarketplace' => $marketplaceTitleValue,
        ...
        <div class="col-span-11 2xl:col-span-9">
            ...
            <div class="box relative mt-8 p-5 before:absolute before:inset-0 before:mx-3 before:-mb-3 before:border before:border-foreground/10 before:bg-background/30 before:shadow-[0px_3px_5px_#0000000b] before:z-[-1] before:rounded-xl after:absolute after:inset-0 after:border after:border-foreground/10 after:bg-background after:shadow-[0px_3px_5px_#0000000b] after:rounded-xl after:z-[-1] after:backdrop-blur-md">
                <div class="rounded-lg border p-5">
                    ...
                    <div class="mt-5 flex flex-col gap-5">
                        <?php row_start('Titlu site (web)', 'Titlul afișat pe magazinul online.', true); ?><?php ph_input('pName', 'Ex: Plăcuțe frână Bosch 0 986 424 268 pentru Audi A3', 'text', 'required'); ?><?php row_end(); ?>
                        <?php row_start('Titlu marketplace', 'Titlul folosit la export PieseAuto / BaseLinker. Același produs, altă formulare.'); ?><?php ph_input('pNameMarketplace', 'Ex: Plăcuțe frână Bosch 0 986 424 268 pentru AUDI/VW'); ?><?php row_end(); ?>
        ...
        <div class="col-span-2 hidden 2xl:block">
            <div class="sticky top-0 pt-5">
                <ul class="before:bg-foreground/10 relative before:absolute before:left-0 before:z-[-1] before:h-full before:w-[2px]">
                    ...
                    <li class="mb-4 border-l-2 border-transparent py-1 pl-5 opacity-70">Titluri duale</li>
...
(function () {
    ...
    const selectedFiles = [];
    const previewFields = ['pName', 'pNameMarketplace', 'pCode', 'pCar', 'pBrand', 'pCategory', 'pSubcategory', 'pStock', 'pBasePrice'];
