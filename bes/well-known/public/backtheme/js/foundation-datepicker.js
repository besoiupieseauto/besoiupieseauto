...
!function( $ ) {
	...
	Datepicker.prototype = {
		...
		place: function(){
			...
			const textbox = this.component ? this.component : this.element;
			const offset = textbox.offset();
			const height = textbox.outerHeight() + parseInt(textbox.css('margin-top'));
			const width = textbox.outerWidth() + parseInt(textbox.css('margin-left'));
		...
		fill: function() {
			const d = new Date(this.viewDate.valueOf());
			...
			while(prevMonth.valueOf() < nextMonth) {
				if (prevMonth.getUTCDay() == this.weekStart) {
					...
					if(this.calendarWeeks){
						...
						const a = new Date(prevMonth.getUTCFullYear(), prevMonth.getUTCMonth(), prevMonth.getUTCDate() - prevMonth.getDay() + 10 - (this.weekStart && this.weekStart % 7 < 5 && 7)),
			...
			const currentYear = this.date && this.date.getUTCFullYear();
					...
					case 'th':
						switch(target[0].className) {
							...
							case 'today':
								...
								this.showMode(-2);
								const which = this.todayBtn == 'linked' ? null : 'view';
