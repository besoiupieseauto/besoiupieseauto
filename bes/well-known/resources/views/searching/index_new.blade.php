...
@section('title', 'Supplier Search New')
...
@section('content')
@php
    $searchSuppliersRoute = 'searching.searchSuppliersNew';
    $searchPanelHeading = 'Supplier Search New';
@endphp
<div class="jumbotron">
<div class="container">
...
<div class="panel-heading">
    <h4><i class="glyphicon glyphicon-search"></i> {{ $searchPanelHeading }}</h4>
</div>
...
/** Autonet returns LEMFÖRDER articles as e.g. 38956LMI; parts_catalog uses "38956 01". */
...
/**
 * Display-only: for LEMFÖRDER, map trailing LMI → " 01" so codes align with DB / search.
 * Otherwise use standard Autonet cleaning.
 */
...
function loadProducts() {
    const query = String($('#customSearch').val() || '').trim();
    ...
    $.post("{{ route($searchSuppliersRoute) }}", {
        _token: "{{ csrf_token() }}",
        query: query,
...
function buildRenderData(product, selectedSuppliers, selectedPlants) {
    ...
    supplierData.forEach(data => {
        ...
        const mainVariantCodeForOrder =
            supplierName === 'autototal' && mainVariant.variant_code
                ? mainVariant.variant_code
                : mainVariant.order_code;
        const mainApiLookupCode =
            supplierName === 'autototal'
                ? (mainVariant.api_lookup_code || mainVariant.order_code || mainVariantCodeForOrder || '')
                : (
                    supplierName === 'autonet'
                        ? (mainVariant.autonet_partno || mainVariant.order_code || mainVariantCodeForOrder || '')
                        : (mainVariantCodeForOrder || '')
                );
            ...
            <tr>
                ...
                <td>
                    ...
                    <button class="btn btn-sm btn-success add-to-cart"
                        ...
                        data-variant="${mainVariantCodeForOrder}"
                        data-lookup-code="${mainApiLookupCode}"
...
function openPriceModal(payload) {
    payload.searched_code = String(searchState.lastQuery || '').trim();
    const rawName = payload.product_name;
    if (rawName == null || String(rawName).trim() === '') {
        payload.product_name = payload.product_code || '-';
    }
    // Keep raw supplier API price before UI selling-price override.
    const rawApiPrice = payload.price;
    payload.raw_price = rawApiPrice;
...
$('#confirmAddToCart').on('click', function () {
    ...
    if (payload.product_name == null || String(payload.product_name).trim() === '') {
        payload.product_name = payload.product_code || '-';
    }
