...
(function () {
  ...
  function toast(msg, isErr) {
    ...
    setTimeout(function () { toastEl.classList.add('hidden'); }, 4200);
  }
  ...
  function syncFormFromChannel() {
    ...
    document.getElementById('bpa-pcf-template-pane').classList.toggle('bpa-pcf-is-hidden', (ch.mode || 'segments') !== 'template');
    document.getElementById('bpa-pcf-segments-pane').classList.toggle('bpa-pcf-is-hidden', (ch.mode || 'segments') === 'template');
    document.getElementById('bpa-pcf-template').value = ch.template || '';
    document.getElementById('bpa-pcf-separator').value = ch.separator != null ? ch.separator : ' ';
    document.getElementById('bpa-pcf-vehicle-prefix').value = ch.vehicle_prefix || 'pentru';
    document.getElementById('bpa-pcf-compat-prefix').value = ch.compat_prefix || 'pentru';
    document.getElementById('bpa-pcf-max-length').value = ch.max_length || 150;
    document.getElementById('bpa-pcf-uppercase').checked = !!ch.uppercase;
    renderSegmentsList();
  }
  ...
  function syncChannelFromForm() {
    ...
    ch.template = document.getElementById('bpa-pcf-template').value;
    ch.separator = document.getElementById('bpa-pcf-separator').value;
    ch.vehicle_prefix = document.getElementById('bpa-pcf-vehicle-prefix').value;
    ch.compat_prefix = document.getElementById('bpa-pcf-compat-prefix').value;
    ch.max_length = parseInt(document.getElementById('bpa-pcf-max-length').value, 10) || 150;
    ch.uppercase = document.getElementById('bpa-pcf-uppercase').checked;
  }
  ...
  function saveCurrentScope(message) {
    ...
    return apiPost({
      ...
      applySavedConfig(j.data, j.message || message || 'Salvat.');
    });
  }

  bindEl('bpa-pcf-save-tabs', 'click', function () {
    syncTabFormToState();
    saveCurrentScope('Tab-uri salvate.').catch(function (e) { toast(e.message, true); });
  });
  ...
  bindEl('bpa-pcf-save', 'click', function () {
    saveCurrentScope().catch(function (e) { toast(e.message, true); });
  });
