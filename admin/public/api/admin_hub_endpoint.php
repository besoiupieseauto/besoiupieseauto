...
use Config\Database;
use Besoiu\Services\AlertsService;
use Besoiu\Services\DashboardService;
use Besoiu\Services\ReportService;
use Besoiu\Controllers\Scan\ScanCancelledException;
use Besoiu\Services\Scan\ScanService;
...
use Besoiu\Services\ChatOrganismOrchestrator;
use Besoiu\Services\SectionAssistantService;
use Besoiu\Services\SectionAssistantQueryLogService;
use Besoiu\Services\SectionAssistantSupervisorService;
use Besoiu\Services\PublicShopChatOrchestrator;
use Besoiu\Services\SettingsService;
...
try {
    ...
    switch ($action) {
        ...
        case 'section_assist':
            ...
            // Eliberează lock sesiune — assist poate dura (Ollama); altfel chip-urile rapide blochează 20s+.
        ...
        case 'section_assist_feedback':
            ...
            $entry = (new SectionAssistantQueryLogService())->setOperatorRating($queryId, $rating, $note);
            if ($entry === null) {
                ApiBootstrap::json(['success' => false, 'message' => 'Query negasit.'], 404);
            }
            $feedback = is_array($entry['feedback'] ?? null) ? $entry['feedback'] : [];
            ApiBootstrap::json([
                'success' => true,
                'message' => 'Feedback salvat.',
                'data' => [
                    'query_id' => (string) ($entry['id'] ?? $queryId),
                    'query' => $entry,
                    'feedback' => $feedback,
                    'operator_rating' => (string) ($entry['operator_rating'] ?? $rating),
                    'evaluation_log' => is_array($feedback['evaluation_log'] ?? null) ? $feedback['evaluation_log'] : [],
                    'checks' => is_array($feedback['checks'] ?? null) ? $feedback['checks'] : [],
                    'correct_items' => is_array($feedback['correct_items'] ?? null) ? $feedback['correct_items'] : [],
                ],
            ], 200);
        ...
        case 'shop_chat_supervisor':
            ApiBootstrap::json([
                'success' => true,
                'data' => PublicShopChatOrchestrator::create()->supervisorStats(
                    max(1, (int) ($_GET['days'] ?? 7))
                ),
            ]);

        case 'section_assist_scorecard':
            $scSection = trim((string) ($_GET['section'] ?? ''));
            if ($scSection === '' && is_array($payload ?? null)) {
                $scSection = trim((string) ($payload['section'] ?? ''));
            }
            ApiBootstrap::json([
                'success' => true,
                'data' => (new SectionAssistantSupervisorService(dirname(__DIR__, 3)))->scorecard($scSection),
            ]);

        case 'chat_organism_capabilities':
            $orgSection = trim((string) ($_GET['section'] ?? ''));
            if ($orgSection === '' && is_array($payload ?? null)) {
                $orgSection = trim((string) ($payload['section'] ?? ''));
            }
            $projectRoot = dirname(__DIR__, 3);
            $status = ChatOrganismOrchestrator::create($projectRoot)->status($orgSection !== '' ? $orgSection : null);
            ApiBootstrap::json([
                'success' => true,
                'data' => $status,
            ]);
        ...
        case 'scan_activity':
            ...
            $progress = $scanService->reconcileProgressWithLock();
            $scanRunning = $scanService->isScanActive($progress);
            ApiBootstrap::json([
                'success' => true,
                ...
                'lock_max_sec' => (static function (): int {
                    require_once ApiBootstrap::projectRoot() . '/config/cron_import.php';
        ...
        case 'scan_unlock':
            ...
            (new \Besoiu\Services\Scan\SupplierScanDashboardService())->updateProgress([
                'running' => false,
                'pct' => 0,
                'phase' => 'stopped',
                'phase_label' => 'Oprit',
        ...
        case 'cron':
            require_once ApiBootstrap::projectRoot() . '/config/cron_tasks.php';
            $scanService = new ScanService();
            $scanProgress = $scanService->reconcileProgressWithLock();
            $scanRunning = $scanService->isScanActive($scanProgress);
            ...
            ApiBootstrap::json([
                ...
                'setup_doc' => 'app/Backend/docs/CRON_WINDOWS_SETUP.md',
                'health' => [
                    'database_ok' => true,
        ...
        case 'scan_run':
            ...
            if ($method === 'POST' || $action === 'scan_run') {
                ...
                try {
                    ...
                    $dashErr = new \Besoiu\Services\Scan\SupplierScanDashboardService();
