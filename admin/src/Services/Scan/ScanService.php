...
namespace Besoiu\Services\Scan;

use Config\Database;
use Besoiu\Controllers\Scan\ScanCancelledException;
use Besoiu\Controllers\Scan\ScanProcessManager;
use Besoiu\Controllers\Scan\SupplierCsvScanHelper;
...
final class ScanService
{
    ...
    private ?ScanProcessManager $processManager = null;

    public function __construct(
        private readonly ?object $statsService = null,
        private readonly ?SupplierCsvScanHelper $csvHelper = null,
        private readonly ?object $syncReport = null,
        private readonly ?object $feedFolder = null,
    ...
    {
        $supplierRows = $this->supplierRowsForScan();
        ...
        foreach ($supplierRows as $row) {
            ...
            if ($mirrorFiles && $randomnId > 0 && OptionalModuleBridge::furnizoriAvailable()) {
                try {
                    $feed = $this->feed();
                    if ($feed !== null) {
                        $feed->mirrorImportFilesForSupplier($code, $randomnId);
                    }
                } catch (\Throwable) {
                    // modul dezactivat sau feed indisponibil
                }
            }
    ...
    /**
     * Dacă lock-ul lipsește dar progresul rămâne running=true, curăță starea UI (scan mort / timeout).
     *
     * @return array<string, mixed>
     */
    public function reconcileProgressWithLock(): array
    {
        $dash = $this->dashboard();
        $lockRunning = $this->isScanRunning();
        $progress = $dash->readProgress();

        if ($lockRunning) {
            if (!empty($progress['running'])) {
                return $progress;
            }
            $dash->updateProgress(['running' => true]);

            return $dash->readProgress();
        }

        $pct = (int) ($progress['pct'] ?? 0);
        $msg = trim((string) ($progress['message'] ?? ''));
        $phase = strtolower(trim((string) ($progress['phase'] ?? '')));
        $recentSync = $this->hasRecentSyncActivity();

        // Scan încă activ în fundal (jurnal recent) — restaurare după reconciliere greșită.
        if ($recentSync && !in_array($phase, ['done', 'error', 'idle'], true)) {
            if ($phase === 'stopped' || !empty($progress['running']) || self::isStaleSupplierProgressMessage($msg)) {
                $dash->updateProgress([
                    'running' => true,
                    'phase' => 'sync',
        ...
        if (empty($progress['running']) && empty($progress['pct']) && empty($progress['message'])) {
            return $progress;
        }

        if (in_array($phase, ['done', 'error', 'stopped', 'idle'], true)) {
            if (!empty($progress['running'])) {
                $dash->updateProgress(['running' => false]);
            }
            if ($phase === 'stopped' && self::isStaleSupplierProgressMessage($msg)) {
                $dash->updateProgress([
                    'pct' => 0,
                    'supplier' => '',
                    'supplier_index' => 0,
                    'supplier_total' => 0,
                    'message' => 'Scan oprit — apasă «Scanează furnizori» din nou.',
                ]);
            } elseif ($phase === 'stopped' && $pct > 0 && $pct < 100) {
                $dash->updateProgress([
                    'pct' => 0,
                    'supplier' => '',
                    'supplier_index' => 0,
                    ...
                    'message' => $msg !== ''
                        ? $msg
                        : 'Scan oprit — apasă «Scanează furnizori» din nou.',
                ]);
            }
        ...
        $looksDone = $pct >= 90
            || stripos($msg, 'finalizat') !== false
            || stripos($msg, 'importreview') !== false;

        if (!empty($progress['running'])) {
            if (!$lockRunning && self::isProgressFresh($progress, 300)) {
                return $progress;
            }
            ...
            if ($looksDone) {
                $dash->updateProgress([
                    ...
                    'pct' => $looksDone ? max($pct, 100) : $pct,
                    'message' => $msg !== '' ? $msg : 'Scan finalizat',
                ]);
            } else {
                $dash->updateProgress([
                    'running' => false,
                    ...
                    'phase_label' => 'Oprit',
                    'pct' => 0,
                    'message' => self::isStaleSupplierProgressMessage($msg)
                        ? 'Scan întrerupt — apasă «Scanează furnizori» din nou.'
                        : ($msg !== ''
                            ? $msg
                            : 'Scan întrerupt — apasă «Scanează furnizori» din nou.'),
                    'supplier' => '',
        ...
        // Zombie: running=false, fază intermediară, fără jurnal recent — curăță UI
        if (
            !$recentSync
            && empty($progress['running'])
            && $pct > 0
            && $pct < 100
            && $phase !== ''
            && !in_array($phase, ['done', 'error', 'stopped', 'idle'], true)
        ) {
            $dash->updateProgress([
                'running' => false,
                'phase' => 'stopped',
                'phase_label' => 'Oprit',
                'pct' => 0,
                'message' => 'Scan întrerupt la ' . $pct . '%'
                    . ($msg !== '' ? ' — ' . $msg : '')
                    . '. Apasă «Scanează furnizori» sau «Deblochează».',
                'supplier' => '',
                'supplier_index' => 0,
                'supplier_total' => 0,
            ]);
    ...
    private static function isStaleSupplierProgressMessage(string $msg): bool
    {
        if ($msg === '') {
            return false;
        }

        return stripos($msg, 'Procesez furnizor') !== false
            || preg_match('/^Import\s+[A-Z0-9]+:\s*produs\s+\d+/i', $msg) === 1;
    }

    /**
     * Progres actualizat recent — scan probabil încă activ (lock/pid nesigur pe unele hosturi).
     *
     * @param array<string, mixed> $progress
     */
    public static function isProgressFresh(array $progress, int $maxSec = 120): bool
    {
        ...
        return (time() - $updatedAt) <= $maxSec;
    }

    /**
     * Scan considerat activ: lock pe disc SAU progres recent cu running=true.
     *
     * @param array<string, mixed> $progress
     */
    public function isScanActive(array $progress): bool
    {
        if ($this->isScanRunning()) {
            return true;
        }

        if ($this->hasRecentSyncActivity()) {
            $phase = strtolower(trim((string) ($progress['phase'] ?? '')));
            if (!in_array($phase, ['done', 'error', 'idle'], true)) {
                return true;
            }
        }

        if (!empty($progress['running']) && self::isProgressFresh($progress, 300)) {
            return true;
        }
    ...
    /** Jurnal activ în ultimele N secunde — scan poate continua fără lock (heartbeat pierdut). */
    public function hasRecentSyncActivity(int $maxSec = 180): bool
    {
        foreach ($this->dashboard()->listActivity(8) as $item) {
            if (!is_array($item)) {
                continue;
            }
            $step = strtolower(trim((string) ($item['step'] ?? '')));
            if (!in_array($step, ['run', 'validate', 'sync', 'ftp'], true)) {
                continue;
            }
            $at = strtotime((string) ($item['at'] ?? ''));
            if ($at > 0 && (time() - $at) <= $maxSec) {
                return true;
            }
        }
    ...
    public function isScanRunning(): bool
    {
        ...
        $heartbeatAt = is_array($decoded) ? strtotime((string) ($decoded['heartbeat_at'] ?? '')) : false;
        $refTs = $heartbeatAt > 0 ? $heartbeatAt : $startedAt;
        $age = $refTs > 0 ? (time() - $refTs) : (time() - (int) filemtime($path));
        ...
        if ($pid > 0 && !$this->isProcessAlive($pid)) {
            // Pe unele hosturi verificarea PID e fals-negativă imediat după fastcgi_finish_request.
            if ($age < 180) {
                return true;
            }
            $this->releaseScanLock();
    ...
    public function touchScanLock(): void
    {
        $path = $this->runLockPath();
        if (!is_file($path)) {
            return;
        ...
        if (empty($decoded['started_at'])) {
            $decoded['started_at'] = date('c');
        }
        if (empty($decoded['pid'])) {
            $decoded['pid'] = getmypid();
        }
        $decoded['heartbeat_at'] = date('c');
    ...
    /**
     * Oprește scanarea în curs: semnal cancel, termină workeri PHP CLI, eliberează lock.
     *
     * @return array{
     *   success:bool,
     *   message:string,
     *   killed:bool,
     *   killed_lock_pid:bool,
    ...
    public function stopRunningScan(): array
    {
        ...
        $pid = (int) ($lock['pid'] ?? 0);
        $killedLockPid = $this->tryTerminateScanProcess($pid);
        $killedBackground = $this->terminateAllBackgroundImportProcesses();
        $cancelledJobs = $this->cancelRunningImportJobs();

        $dash = $this->dashboard();
        $killedTotal = $killedLockPid || $killedBackground > 0;
        $dash->log(
            'Scan oprit manual'
                . ($killedTotal ? ' — ' . ($killedBackground + ($killedLockPid ? 1 : 0)) . ' proces(e) oprit(e)' : ($wasRunning ? ' — aștept oprire…' : ''))
                . ($cancelledJobs > 0 ? ', ' . $cancelledJobs . ' job(uri) anulate' : ''),
            '',
            'warn',
            'run'
        );
        $dash->updateProgress([
            ...
            'phase' => 'stopped',
            'phase_label' => 'Oprit',
            'message' => 'Scan oprit de utilizator — toate procesele au fost oprite',
            'supplier' => '',
            'supplier_index' => 0,
            'supplier_total' => 0,
        ]);
        ...
        if (!$wasRunning && !$killedTotal && $cancelledJobs === 0) {
            return [
                'success' => true,
                'message' => 'Niciun scan activ — flag-ul de oprire a fost setat oricum.',
                'killed' => false,
                'killed_lock_pid' => false,
                'killed_processes' => 0,
                'cancelled_jobs' => 0,
            ];
        }

        $messageParts = ['Scan oprit — toate procesele au fost oprite.'];
        if ($killedBackground > 0) {
            $messageParts[] = $killedBackground . ' worker(i) PHP oprit(i).';
        }
        if ($killedLockPid) {
            $messageParts[] = 'Procesul scan principal a fost întrerupt.';
        }
        if ($cancelledJobs > 0) {
            $messageParts[] = $cancelledJobs . ' job(uri) import anulate.';
        }
        ...
        return [
            ...
            'killed' => $killedTotal,
            'killed_lock_pid' => $killedLockPid,
            'killed_processes' => $killedBackground,
            'cancelled_jobs' => $cancelledJobs,
        ];
    }

    /**
     * Termină procesele PHP de fundal legate de import/cron.
     * Delegat către ScanProcessManager (extras la refactorizarea Faza 3).
     */
    public function terminateAllBackgroundImportProcesses(): int
    {
        return $this->processManager()->terminateAllBackgroundImportProcesses();
    }

    private function processManager(): ScanProcessManager
    {
        return $this->processManager ??= new ScanProcessManager();
    }
    ...
    {
        return $this->processManager()->tryTerminateProcess($pid);
    }
    ...
    private function runFullSyncBody(bool $remoteFtp = false): array
    {
        ...
        if (!defined('BESOIU_CRON_SYNC')) {
            define('BESOIU_CRON_SYNC', true);
        }

        $guardFile = dirname(__DIR__, 3) . '/system/api_automation_guard.php';
        if (is_file($guardFile)) {
            require_once $guardFile;
            if (function_exists('besoiu_api_automation_guard_cron')) {
                besoiu_api_automation_guard_cron('cron_sync');
            }
        }

        $scraperBootstrap = dirname(__DIR__, 3) . '/lib/Scraper/bootstrap.php';
        if (is_file($scraperBootstrap)) {
            require_once $scraperBootstrap;
            $scrapeUsage = \ScrapeDoConfig::budgetUsage();
            if (is_array($scrapeUsage)
                && (int) ($scrapeUsage['requests_left'] ?? $scrapeUsage['queries_left'] ?? 0) > 0) {
        ...
        $supplierRows = $this->supplierRowsForScan();
        $syncLib = dirname(__DIR__, 3) . '/tools/besoiupieseimport_sync_lib.php';
        if (is_file($syncLib)) {
            require_once $syncLib;
            if (function_exists('besoiupieseimport_sync_is_enabled') && besoiupieseimport_sync_is_enabled()) {
                $syncResult = besoiupieseimport_sync_run(false);
                $dash->log(
                    $syncResult['summary'] ?? 'Sync besoiupieseimport',
                    '',
                    !empty($syncResult['ok']) ? 'ok' : 'warn',
                    'run',
                    'Sursă: besoiupieseimport → admin/storage/supplier_feeds + tecdoc'
                );
            }
        }
        ...
        foreach ($supplierRows as $row) {
            ...
            $folderRel = $this->requireFeed()->folderRelative($code, $randomnId);
            $folderAbs = $this->requireFeed()->folderPath($code, $randomnId);
            ...
            $mirror = $this->requireFeed()->mirrorImportFilesForSupplier($code, $randomnId);
            ...
            $localCsv = $this->requireFeed()->listFeedCsvFiles($code, $randomnId);
            if ($localCsv !== []) {
                $names = array_map(
            ...
            try {
                ...
                $t0 = microtime(true);
                $remoteSync = SupplierHooks::remoteSync();
                $result = $remoteSync !== null
                    ? $remoteSync->syncFromFtp($row)
                    : ['success' => false, 'message' => 'Modul furnizori indisponibil'];
                $elapsed = (int) round((microtime(true) - $t0) * 1000);
                $ftpDetail = $this->formatFtpDebugDetail($result['debug'] ?? [], $elapsed);

                if (!empty($result['success'])) {
                    ++$syncedRemote;
                    $this->requireFeed()->mirrorImportFilesForSupplier($code, $randomnId);
                    $dash->log(
    ...
    private function buildSupplierRow(array $row, array $catalogDef, bool $analyzeCsv): array
    {
        ...
        $randomnId = (int) ($row['randomn_id'] ?? 0);
        $report = [];
        $sync = $this->syncReports();
        if ($sync !== null) {
            $report = $sync->buildReportForCode($code, $row);
        }
        ...
        return array_merge($row, $report, $validation, [
            'supplier_code' => $code,
            'slug' => $this->feedSlug($code, $randomnId),
            ...
            'scan_interval_label' => SupplierHooks::scanScheduleFormatLabel($row),
            'scan_schedule_label' => SupplierHooks::scanScheduleFormatLabel($row),
            'profile_url' => $randomnId > 0 ? ('/admin/profilefurnizori?randomn_id=' . $randomnId) : '/admin/furnizori',
            'categorii_url' => '/admin/categorii',
            'import_url' => '/admin/suppliers',
    ...
    private function resolveLatestFilePath(string $code, int $randomnId, array $files): ?string
    {
        if (!function_exists('list_uploaded_import_files')) {
            require_once dirname(__DIR__, 2) . '/Controllers/Produse/import_uploaded_files_lib.php';
        }
        ...
        $feed = $this->feed();
        if ($feed !== null) {
            foreach ($feed->listFeedCsvFiles($code, $randomnId) as $entry) {
    ...
    /** @return array<int, array<string, mixed>> */
    private function supplierRowsForScan(): array
    {
        $stats = $this->stats();
        if ($stats !== null) {
            ...
            return array_values(array_filter($rows, static function (array $row): bool {
                ...
                if (strtolower(trim((string) ($row['status'] ?? 'active'))) === 'blocked') {
                    return false;
                }

                return !function_exists('import_furnizor_is_blocked') || !import_furnizor_is_blocked($code);
            }));
        }

        return array_values(array_filter(
            import_furnizori_catalog(),
            static function (array $entry): bool {
                $code = strtoupper(trim((string) ($entry['code'] ?? '')));

                return $code !== ''
                    && (!function_exists('import_furnizor_is_blocked') || !import_furnizor_is_blocked($code));
            }
        ));
    }
    ...
    private function stats(): ?object
    {
        ...
        return SupplierHooks::stats();
    }

    private function feedSlug(string $code, int $randomnId): string
    {
        $feed = $this->feed();
        if ($feed !== null) {
            return (string) $feed->slugFromSupplier($code, $randomnId);
        }

        return strtolower($code);
    }

    private function requireFeed(): object
    {
        $feed = $this->feed();
        if ($feed === null) {
            throw new \RuntimeException('Serviciul feed furnizori indisponibil.');
        }
    ...
    private function syncReports(): ?object
    {
        return $this->syncReport ?? SupplierHooks::syncReports();
    }

    private function feed(): ?object
    {
        return $this->feedFolder ?? SupplierHooks::feed();
    }
     ...
     * @return array<string, mixed>
     */
    public function resetForTesting(bool $clearFeedFiles = false, bool $disableAutoScan = false): array
    {
        $stop = $this->stopRunningScan();
        self::clearStopRequest();

        $cancelledJobs = (int) ($stop['cancelled_jobs'] ?? 0);
        ...
        return [
            ...
            'cancelled_jobs' => $cancelledJobs,
            'killed_processes' => (int) ($stop['killed_processes'] ?? 0),
    ...
    {
        $lib = dirname(__DIR__, 2) . '/Controllers/Produse/import_job_lib.php';
        if (is_file($lib)) {
            require_once $lib;
            if (function_exists('import_job_cancel_all')) {
                return import_job_cancel_all();
            }
        }
