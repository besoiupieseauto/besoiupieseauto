...
final class ProductImageAuditService
{
    ...
    {
        $raw = strtolower(trim((string) ($_ENV['IMAGE_AUDIT_ENGINE'] ?? getenv('IMAGE_AUDIT_ENGINE') ?: 'openai')));
        if (in_array($raw, ['manual', 'off', 'disabled', 'none', 'cursor'], true)) {
            return 'manual';
        }
    ...
    public function fillMissingCursorAuditResults(PDO $pdo, string $jobId): int
    {
        ...
        if (!self::openAiFallbackEnabled() && self::auditEngine() !== 'ollama') {
            return 0;
        }

        if (self::auditEngine() === 'ollama') {
            $ollama = new OllamaLlmClient($this->projectRoot);
            ...
            $saved = 0;
            foreach ($products as $product) {
                $result = $this->analyzeProductWithOllamaVision($product, $ollama);
    ...
    {
        if (self::auditEngine() === 'ollama') {
            return $this->analyzeProductWithOllamaVision($product, new OllamaLlmClient($this->projectRoot));
        }
    ...
    /**
     * Audit imagine via Ollama vision (llava / moondream) — local, fără OpenAI.
     *
     * @param array<string, mixed> $product
     * @return array<string, mixed>
     */
    public function analyzeProductWithOllamaVision(array $product, ?OllamaLlmClient $client = null): array
    {
        $imagePayload = $this->buildVisionImagePayload($product);
        if ($imagePayload === null) {
            return $this->resultTemplate($product, [
        ...
        $b64 = $this->extractBase64FromVisionPayload($imagePayload);
        if ($b64 === '') {
            return $this->resultTemplate($product, [
                'verdict' => 'error',
                'match_score' => 0,
                'image_shows' => '',
                'issues' => ['Nu s-a putut încărca imaginea pentru Ollama'],
                'recommendation' => 'review',
                'summary_ro' => 'Eroare încărcare imagine.',
            ]);
        }

        $client = $client ?? new OllamaLlmClient($this->projectRoot);
        $vision = $client->visionReadiness();
        if (empty($vision['ready'])) {
            $err = (string) ($vision['error'] ?? 'Ollama vision indisponibil');
        ...
        $prompt = $this->buildVisionPrompt($product) . "\n\nRăspunde DOAR JSON valid cu cheile: verdict, match_score, image_shows, issues, recommendation, summary_ro.";

        $res = $client->visionChat(
            $prompt,
            [$b64],
            'Ești auditor imagini piese auto Besoiu. Răspuns DOAR JSON, fără markdown.',
            $client->modelForAgent('agent-imagini'),
            0.15,
            120
        );
        ...
        if (empty($res['ok'])) {
            return $this->resultTemplate($product, [
                ...
                'match_score' => 0,
                'image_shows' => '',
                'issues' => [(string) ($res['error'] ?? 'Ollama vision eșuat')],
                'recommendation' => 'review',
                'summary_ro' => (string) ($res['error'] ?? 'Ollama vision eșuat'),
            ]);
        }
        ...
        if (!is_array($parsed)) {
            return $this->resultTemplate($product, [
                'verdict' => 'review',
                'match_score' => 0,
                'image_shows' => mb_substr($content, 0, 200),
                'issues' => ['Răspuns Ollama non-JSON — verificare manuală'],
                'recommendation' => 'review',
                'summary_ro' => mb_substr($content, 0, 280),
        ...
        return $this->resultTemplate($product, [
            ...
            'recommendation' => (string) ($parsed['recommendation'] ?? 'review'),
            'summary_ro' => (string) ($parsed['summary_ro'] ?? ''),
            'model' => (string) ($res['model'] ?? 'ollama-vision'),
        ]);
    }

    /** @param array{type:string,image_url?:array<string,string>} $payload */
    private function extractBase64FromVisionPayload(array $payload): string
    {
        $url = (string) ($payload['image_url']['url'] ?? '');
        if ($url === '') {
            return '';
        }
        ...
        if (is_file($diskPath)) {
            return base64_encode((string) file_get_contents($diskPath));
        }
    ...
    private function buildVisionPrompt(array $product): string
    {
        ...
        if ($sourceId !== '') {
            require_once dirname(__DIR__, 3) . '/lib/Scraper/ScraperModule.php';
            try {
                $srcCfg = ScraperModule::instance()->getSourceConfig($sourceId);
                $config = is_array($srcCfg['config'] ?? null) ? $srcCfg['config'] : [];
                $srcAi = is_array($config['integration']['image_ai'] ?? null) ? $config['integration']['image_ai'] : [];
                ...
                // sursa necunoscuta — reguli globale doar
            }
        }
