...
final class ApiBootstrap
{
    ...
    /** Rădăcina site-ului (părintele admin/ și app/). */
    public static function siteRoot(): string
    {
        ...
        return dirname(self::projectRoot(), 2);
    }
    ...
    public static function adminRoot(): string
    {
        ...
        return self::siteRoot() . DIRECTORY_SEPARATOR . 'admin';
    }

    private static function resolveSystemFile(string $filename): ?string
    {
        $root = self::siteRoot();
        foreach ([
    ...
    private static function loadDotenvFromKnownPaths(): void
    {
    ...
    public static function boot(bool $withDatabase = false): array
    {
        if (!self::$booted) {
            ...
            self::loadDotenvFromKnownPaths();

            self::$config = require self::projectRoot() . '/config/config.php';
            self::$booted = true;

            try {
                ModuleRegistry::instance(self::adminRoot())->boot();
            } catch (Throwable) {
                // Module opționale — nu blochează API.
            }

            $previewGate = self::resolveSystemFile('preview-gate.php');
            if ($previewGate !== null) {
    ...
    {
        $sessionBridge = self::resolveSystemFile('session-bridge.php');
        ...
        $guardPath = self::resolveSystemFile('api_automation_guard.php');
        if ($guardPath !== null) {
    ...
    public static function requireAuthenticatedSession(): void
    {
        ...
        if (empty($_SESSION['user_id'])) {
            ...
            if (empty($_SESSION['user_id']) && function_exists('besoiu_admin_session_user_id')) {
                $userId = besoiu_admin_session_user_id();
                if ($userId > 0) {
                    besoiu_admin_session_start();
    ...
    public static function enforceApiWorkspaceIfNeeded(): void
    {
        ...
        if (!ModuleGate::apiScriptAllowed($script)) {
            self::json([
                'success' => false,
                'message' => 'Modul opțional dezactivat sau lipsă.',
            ], 403);
        }
    ...
    /**
     * Pregătire request JSON lung (supplier search, scan) — limită timp + eliberare sesiune.
     */
    public static function beginBoundedJsonWork(int $maxSeconds = 45): void
    {
        $limit = max(10, $maxSeconds);
        @set_time_limit($limit + 5);
        @ini_set('max_execution_time', (string) ($limit + 5));
        self::releaseSession();
    }

    /** Răspuns JSON la fatal PHP dacă scriptul moare înainte de ApiBootstrap::json(). */
    public static function registerJsonFatalGuard(string $context): void
    {
        static $registered = [];
        if (isset($registered[$context])) {
            return;
        }
        $registered[$context] = true;
