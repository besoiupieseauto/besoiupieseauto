...
namespace Besoiu\Core\CaietComenzi;
...
final class CaietComenziModel
{
    ...
    {
        return $this->findProdusePage($filters)['items'];
    }

    /**
     * Paginare keyset pentru catalogul ERP. Cursorul este ultimul ID primit,
     * astfel încât lista nu folosește OFFSET pe tabelele legacy mari.
     *
     * @param array<string, mixed> $filters
     * @return array{items:array<int, array<string, mixed>>, next_cursor:?int, has_more:bool, page_size:int}
     */
    public function findProdusePage(array $filters = []): array
    {
        $search = trim((string) ($filters['search'] ?? ''));
        $searchLength = function_exists('mb_strlen') ? mb_strlen($search) : strlen($search);
        if ($search !== '' && $searchLength < 2) {
            throw new \InvalidArgumentException('Căutarea produselor necesită minimum 2 caractere.');
        }

        $limit = $this->resolveProductLimit($filters['limit'] ?? null);
        $cursor = isset($filters['cursor']) && is_numeric($filters['cursor'])
            ? max(0, (int) $filters['cursor'])
            : 0;

        if ($this->tableHasRows('produse')) {
            return $this->findProdusePageFromLegacyProduse($search, $cursor, $limit);
        }

        if (!$this->hasLeadingIndex('parts_catalog', 'id')) {
            return $this->emptyProductPage(
                $limit,
                'Catalogul ERP lipsește sau necesită indexul parts_catalog.id. Rulează migrarea legacy numai după backup.'
            );
        }
        if (
            $search !== ''
            && !$this->hasLeadingIndex('parts_catalog', 'mainart_code_parts')
            && !$this->hasLeadingIndex('parts_catalog', 'code_parts')
        ) {
            return $this->emptyProductPage(
                $limit,
                'Căutarea ERP necesită indexurile de cod din migrarea legacy.'
            );
        }
        if ($search !== '') {
            return $this->findPartsCatalogSearchPage($search, $cursor, $limit);
        }
        ...
        return $this->buildProductPage($stmt->fetchAll(PDO::FETCH_ASSOC), $limit);
    }

    /**
     * Rulează lookup-uri indexabile separat; OR pe mai multe coloane combinat
     * cu ORDER BY id făcea MySQL să scaneze milioane de rânduri.
     *
     * @return array{items:array<int,array<string,mixed>>,next_cursor:?int,has_more:bool,page_size:int}
     */
    private function findPartsCatalogSearchPage(string $search, int $cursor, int $limit): array
    {
        $prefix = $this->escapeLikePrefix($search);
        $probes = [
            ['column' => 'mainart_code_parts', 'operator' => '=', 'value' => $search],
            ['column' => 'code_parts', 'operator' => '=', 'value' => $search],
            ['column' => 'mainart_code_parts', 'operator' => 'LIKE', 'value' => $prefix],
            ['column' => 'code_parts', 'operator' => 'LIKE', 'value' => $prefix],
            ['column' => 'mainart_brands', 'operator' => 'LIKE', 'value' => $prefix],
        ];
        $rowsById = [];

        foreach ($probes as $probeIndex => $probe) {
            $column = $probe['column'];
            $operator = $probe['operator'];
            $params = [':value' => $probe['value']];
            $cursorSql = '';
            if ($cursor > 0) {
                $cursorSql = ' AND p.id < :cursor';
                $params[':cursor'] = $cursor;
            }
            $escapeSql = $operator === 'LIKE' ? " ESCAPE '!'" : '';
            $sql = "SELECT
                        p.id AS idprodus,
                        TRIM(CONCAT(COALESCE(p.mainart_brands, ''), ' ', COALESCE(p.mainart_code_parts, ''))) AS denumire,
                        COALESCE(NULLIF(p.mainart_code_parts, ''), p.code_parts, '-') AS cod_produs,
                        0 AS pret, '' AS TVA, '' AS um, 0 AS created_at
                    FROM parts_catalog p
                    WHERE p.{$column} {$operator} :value{$escapeSql}{$cursorSql}
                    ORDER BY p.id DESC
                    LIMIT " . ($limit + 1);
            $statement = $this->legacyDb->prepare($sql);
            $statement->execute($params);
            foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
                $rowsById[(int) ($row['idprodus'] ?? 0)] = $row;
            }
            if ($probeIndex === 1 && $rowsById !== []) {
                break;
            }
        }
        ...
        return $this->buildProductPage(array_slice(array_values($rowsById), 0, $limit + 1), $limit);
    }
    ...
    {
        $statement = $this->legacyDb->prepare(
    ...
    private function tableHasRows(string $tableName): bool
    {
        ...
        try {
            $statement = $this->legacyDb->query("SELECT 1 FROM `{$tableName}` LIMIT 1");
    ...
    private function hasLeadingIndex(string $tableName, string $columnName): bool
    {
        $statement = $this->legacyDb->prepare(
            'SELECT 1
     ...
     * @return array{items:array<int, array<string, mixed>>, next_cursor:?int, has_more:bool, page_size:int}
     */
    private function findProdusePageFromLegacyProduse(string $search, int $cursor, int $limit): array
    {
        $params = [];
        $where = [];
        ...
        if ($search !== '') {
            ...
            $params[':search_prefix_code'] = $prefix;
            $params[':search_prefix_name'] = $prefix;
            $where[] = '(p.cod_produs = :search_exact
                OR p.cod_produs LIKE :search_prefix_code ESCAPE \'!\'
                OR p.denumire LIKE :search_prefix_name ESCAPE \'!\')';
        }

        if ($cursor > 0) {
            $params[':cursor'] = $cursor;
            $where[] = 'p.idprodus < :cursor';
        }
        ...
        $sql .= ' ORDER BY p.idprodus DESC LIMIT ' . ($limit + 1);
     ...
     * @return array{items:array<int, array<string, mixed>>, next_cursor:?int, has_more:bool, page_size:int}
     */
    private function buildProductPage(array $rows, int $limit): array
    {
        ...
        $nextCursor = $hasMore && is_array($last) ? (int) ($last['idprodus'] ?? 0) : null;

        return [
            'items' => $rows,
            'next_cursor' => $nextCursor !== null && $nextCursor > 0 ? $nextCursor : null,
            'has_more' => $hasMore,
            'page_size' => $limit,
        ];
    }
     ...
     * @return array{items:array<int, array<string, mixed>>, next_cursor:?int, has_more:bool, page_size:int}
     */
    private function emptyProductPage(int $limit, ?string $maintenanceMessage = null): array
    {
        $page = [
            'items' => [],
            'next_cursor' => null,
        ...
        if ($maintenanceMessage !== null) {
            $page['maintenance_required'] = true;
            $page['maintenance_message'] = $maintenanceMessage;
        }

        return $page;
    }

    private function resolveProductLimit($limit): int
    {
        $value = is_numeric($limit) ? (int) $limit : 25;
