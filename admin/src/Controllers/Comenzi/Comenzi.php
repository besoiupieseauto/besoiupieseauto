...
final class Comenzi
{
    ...
    /**
     * Cotatie cos site — validare preț/stoc fără INSERT (#42).
     *
     * @param array<string, mixed> $rawInput
     * @return array{lines: list<array<string, mixed>>, total_amount: float, quantity: int}
     */
    public function quote(array $rawInput): array
    {
        if (!$this->hasWebsiteItems($rawInput)) {
            throw new ValidationException('Lipsește lista de produse pentru cotatie.');
        }

        return $this->comenziService->quoteWebsiteCart($rawInput['items']);
    }
