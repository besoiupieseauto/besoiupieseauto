 ...
 */
function import_consumable_list_supplier_feed_files(): array
{
    $root = dirname(__DIR__, 3) . '/storage/supplier_feeds';
    if (!is_dir($root)) {
        return [];
    }

    $files = [];
    foreach (scandir($root) ?: [] as $supplierDir) {
        if ($supplierDir === '.' || $supplierDir === '..') {
            continue;
        }
        $dir = $root . '/' . $supplierDir;
        if (!is_dir($dir)) {
            continue;
        }
        foreach (scandir($dir) ?: [] as $name) {
            ...
            $path = $dir . '/' . $name;
            if (!is_file($path) || !preg_match('/\.(csv|txt|tsv)$/i', $name)) {
                continue;
            }
            if (function_exists('import_tecdoc_library_is_tecdoc_name')
                && import_tecdoc_library_is_tecdoc_name($name)) {
                continue;
            }
            if (!function_exists('import_classify_file')) {
                require_once __DIR__ . '/import_supplier_lib.php';
            }
            $kind = import_classify_file($path, $name);
            if (!str_starts_with($kind, 'supplier:')) {
                continue;
            }
            $supplierType = substr($kind, 9);
            if ($supplierType !== '' && function_exists('import_furnizor_is_blocked') && import_furnizor_is_blocked($supplierType)) {
                continue;
            }
            $files[] = ['path' => $path, 'name' => $name, 'source' => 'supplier_feed'];
        }
    }
...
function import_consumable_dedupe_products(array $products): array
{
    $bestByKey = [];
    $priorityMap = function_exists('import_supplier_priority_map') ? import_supplier_priority_map() : [];
...
function import_consumable_resolve_supplier_files(array $filesMeta, bool $includeFeeds = true): array
{
    ...
    if (!function_exists('import_tecdoc_library_is_tecdoc_name')) {
        require_once __DIR__ . '/import_tecdoc_library_lib.php';
    }
    $missing = [];
    $supplierFiles = [];
    $seenNames = [];
    ...
    foreach ($filesMeta as $fileMeta) {
        ...
        if ($kind === 'tecdoc' || import_tecdoc_library_is_tecdoc_name($originalName)) {
            continue;
        }
        ...
        if (str_starts_with($kind, 'supplier:')
            || ($fileMeta['upload_role'] ?? '') === 'supplier'
            || ($fileMeta['resolved_role'] ?? '') === 'supplier') {
            $safeName = import_tecdoc_library_safe_basename($originalName);
            $supplierFiles[] = ['path' => $path, 'name' => $originalName, 'source' => 'upload'];
            $seenNames[strtolower($safeName)] = true;
        }
    }

    if ($includeFeeds) {
        foreach (import_consumable_list_supplier_feed_files() as $feedFile) {
            $safeName = strtolower(import_tecdoc_library_safe_basename((string) ($feedFile['name'] ?? '')));
            if ($safeName === '' || isset($seenNames[$safeName])) {
                continue;
            }
            $supplierFiles[] = $feedFile;
...
function import_consumable_scan_supplier_stream(
    ...
    foreach ($supplierFiles as $file) {
        ...
        if (function_exists('import_furnizor_is_blocked') && import_furnizor_is_blocked($supplierType)) {
            continue;
        }

        $stop = false;
        $rowsReadLast = import_stream_supplier_file(
            $path,
            $filename,
            $supplierType,
    ...
    return [
        ...
        'rows_read' => $rowsReadLast,
        'file_exhausted' => $maxRowsPerPass <= 0 || $rowsReadLast < $maxRowsPerPass,
    ];
}
...
function import_consumable_scan_preview(array $filesMeta, array $options = []): array
{
    ...
    $resolved = import_consumable_resolve_supplier_files($filesMeta, true);
    if ($resolved['missing'] !== []) {
        return [
            'success' => false,
    ...
    if ($resolved['supplier_files'] === []) {
        return [
            'success' => false,
            'message' => 'Încarcă cel puțin o listă furnizor (Pas 1) sau pune CSV-uri în admin/storage/supplier_feeds/.',
            'products' => [],
            'stats' => ['total_scanned' => 0, 'matched' => 0, 'with_price' => 0],
        ];
    }
    ...
    // Prețul vine din rândul furnizor (net_price) — fără index SQLite complet (timeout).
    $priceIndex = [];
...
/**
 * @param array<int, array<string, mixed>> $filesMeta
 * @return array{ok:bool,job_id?:string,error?:string,message?:string}
 */
function import_consumable_scan_job_start(array $filesMeta, array $options = []): array
{
    import_job_extend_runtime(900);
    ...
    $resolved = import_consumable_resolve_supplier_files($filesMeta, true);
    if ($resolved['missing'] !== []) {
        return ['ok' => false, 'error' => 'missing_files', 'message' => 'Fișiere lipsă pe server.'];
    }
    if ($resolved['supplier_files'] === []) {
        return ['ok' => false, 'error' => 'no_supplier_files', 'message' => 'Niciun fișier furnizor.'];
    }

    $validation = import_validate_uploaded_files($resolved['supplier_files'], [], []);
    if (!$validation['ok']) {
        return ['ok' => false, 'error' => 'validation_failed', 'message' => implode(' ', $validation['errors'] ?? [])];
    }

    $jobId = import_job_create('consumable_scan', [
        'phase' => 'scan_supplier_files',
        'progress' => 1.0,
        'message' => 'Pornesc scan consumabile (ulei · lichide · electrice)...',
        'max_preview' => $maxPreview,
        'supplier_files_count' => count($resolved['supplier_files']),
    ]);

    import_job_save_state($jobId, [
        'supplier_files' => $resolved['supplier_files'],
        'file_index' => 0,
        'row_offset' => 0,
        'matched' => [],
        'total_scanned' => 0,
        ...
        'rows_per_step' => max(5000, min(50000, (int) ($options['rows_per_step'] ?? 25000))),
    ]);

    return ['ok' => true, 'job_id' => $jobId];
}

/** @return array{ok:bool,status?:array<string,mixed>,result?:array<string,mixed>,error?:string} */
function import_consumable_scan_job_step(string $jobId, array $meta, array $state): array
{
    import_job_extend_runtime(900);

    $fileIndex = (int) ($state['file_index'] ?? 0);
    $rowOffset = (int) ($state['row_offset'] ?? 0);
    $rowsPerStep = max(5000, (int) ($state['rows_per_step'] ?? 25000));
    $supplierFiles = is_array($state['supplier_files'] ?? null) ? $state['supplier_files'] : [];
    $matched = is_array($state['matched'] ?? null) ? $state['matched'] : [];
    $maxPreview = max(1, (int) ($state['max_preview'] ?? ($meta['max_preview'] ?? 200)));
    $selected = import_consumable_normalize_categories((array) ($state['selected_categories'] ?? []));
    $brandFilter = trim((string) ($state['brand_filter'] ?? ''));
    $totalScanned = (int) ($state['total_scanned'] ?? 0);

    if ($fileIndex >= count($supplierFiles)) {
        $result = import_consumable_finalize_scan_result($matched, $totalScanned, $selected, count($supplierFiles));
        import_job_update($jobId, [
            'status' => 'done',
            'phase' => 'done',
            'progress' => 100.0,
            'message' => (string) ($result['message'] ?? 'Scan finalizat.'),
            'result' => $result,
        ]);
        import_job_save_state($jobId, []);
    ...
    $current = $supplierFiles[$fileIndex];
    $fileName = (string) ($current['name'] ?? basename((string) ($current['path'] ?? '')));
    $remaining = max(1, $maxPreview - count($matched));
    ...
    $matched = import_consumable_dedupe_products($matched);
    if (count($matched) > $maxPreview) {
        $matched = array_slice($matched, 0, $maxPreview);
    }

    $totalScanned += (int) ($stream['total_scanned'] ?? 0);
    $rowsRead = (int) ($stream['rows_read'] ?? 0);
    $fileExhausted = !empty($stream['file_exhausted']);
    $stoppedEarly = !empty($stream['stopped_early']);

    if ($fileExhausted || $stoppedEarly) {
        $fileIndex++;
        $rowOffset = 0;
    } else {
        $rowOffset += max(1, $rowsRead);
    }

    $progress = count($supplierFiles) > 0
        ? min(99.0, (($fileIndex + ($fileExhausted ? 0 : 0.5)) / count($supplierFiles)) * 100.0)
        : 99.0;

    $state['file_index'] = $fileIndex;
    $state['row_offset'] = $rowOffset;
    $state['matched'] = $matched;
    $state['total_scanned'] = $totalScanned;
    import_job_save_state($jobId, $state);

    $doneEarly = count($matched) >= $maxPreview || $stoppedEarly;
    if ($doneEarly || $fileIndex >= count($supplierFiles)) {
        $result = import_consumable_finalize_scan_result($matched, $totalScanned, $selected, count($supplierFiles));
        import_job_update($jobId, [
            'status' => 'done',
            'phase' => 'done',
    ...
    import_job_update($jobId, [
        'phase' => 'scan_supplier_files',
        'progress' => $progress,
        'message' => 'Scan consumabile (' . min($fileIndex + 1, count($supplierFiles)) . '/' . count($supplierFiles) . '): ' . $fileName
            . ' — ' . count($matched) . ' găsite, ' . number_format($totalScanned, 0, '.', '.') . ' rânduri',
    ]);
 ...
 * @return array<string, mixed>
 */
function import_consumable_finalize_scan_result(
    array $matched,
    int $totalScanned,
    array $selected,
    int $supplierFileCount
): array {
    $defs = import_consumable_category_defs();
    $out = [];
    ...
    if ($out === []) {
        return [
            'success' => true,
            'message' => 'Am parcurs ' . $totalScanned
                . ' rânduri — niciun ulei/lichid/electric consumabil cu preț în listele curente.',
            'products' => [],
            'stats' => [
                'total_scanned' => $totalScanned,
                'matched' => 0,
                'with_price' => 0,
                'supplier_files' => $supplierFileCount,
                'categories' => $selected,
            ],
        ];
    }
    ...
    return [
        'success' => true,
        'message' => count($out) . ' consumabile găsite (parcurse '
            . $totalScanned . ' rânduri din ' . $supplierFileCount . ' fișier(e)).',
        'products' => $out,
        'stats' => [
            'total_scanned' => $totalScanned,
            'matched' => count($out),
            'with_price' => count($out),
            'supplier_files' => $supplierFileCount,
            'categories' => $selected,
        ],
    ];
 ...
 */
function import_consumable_publish_default_stats(int $limit, int $selected): array
{
    return [
        'limit' => $limit,
        'selected' => $selected,
...
function import_consumable_publish_push_log(array &$log, string $message, string $level = 'info'): void
{
    $log[] = [
        'at' => date('c'),
        'level' => $level,
    ...
    if (count($log) > 250) {
        $log = array_values(array_slice($log, -250));
    }
}

/**
 * Procesează un singur produs consumabil (enrich + scraper + ePiesa + publish).
 *
 * @param array<string, mixed> $product
 * @param array<string, mixed> $options limit, check_epiesa, publish_mode
 * @param array<string, int|float> $stats
 * @param array<int, array<string, mixed>> $log
 * @return array{stats: array<string, int|float>, log: array<int, array<string, mixed>>}
 */
function import_consumable_publish_process_one(
    PDO $pdo,
    array $product,
    array $options,
    AdaosComercialService $markupService,
    ...
    $publishMode = (string) ($options['publish_mode'] ?? admin_cron_import_publish_mode());
    $checkEpiesa = import_consumable_wants_epiesa($options);

    if ((int) ($stats['published'] ?? 0) >= $limit) {
        import_consumable_publish_push_log($log, 'Limită atinsă (' . $limit . ' produse). Oprire.', 'warn');
    ...
    if (!import_consumable_has_supplier_price($product)) {
        ++$stats['skipped'];
        import_consumable_publish_push_log(
            $log,
            'Sărit (fără preț furnizor): ' . mb_substr((string) ($product['pName'] ?? ''), 0, 60, 'UTF-8'),
            'warn'
        );
    ...
    $imageOpts = [
        'force_scraper' => true,
        'budget_sec' => 75,
        'skip_sources' => import_consumable_skip_scraper_sources(),
    ];
    $product = import_consumable_resolve_image($product, static function (string $msg, string $level) use (&$log): void {
        import_consumable_publish_push_log($log, $msg, $level);
    }, import_consumable_image_source_mode($product), $imageOpts);
    ...
    if (is_file($pipelinePath)) {
        ...
        if (!empty($staging['__image_audit_retry'])) {
            import_consumable_publish_push_log($log, 'Audit imagini: recomandare înlocuire imagine — re-caut surse.', 'warn');
            $staging = import_consumable_resolve_image($staging, static function (string $msg, string $level) use (&$log): void {
                import_consumable_publish_push_log($log, $msg, $level);
            }, 'auto', $imageOpts);
        }
    }
    ...
    import_consumable_publish_push_log($log, 'Procesez: ' . mb_substr($name, 0, 72, 'UTF-8'), 'info');

    $epiesa = ['checked' => false, 'found' => false, 'vitrina' => false];
    if ($checkEpiesa) {
        $epiesa = import_consumable_enrich_epiesa($staging, static function (string $msg, string $level) use (&$log): void {
            import_consumable_publish_push_log($log, $msg, $level);
        });
        if ($epiesa['checked']) {
    ...
    if (import_consumable_blocks_auto_publish($staging)) {
        ++$stats['skipped'];
        ++$stats['blocked_critical'];
        import_consumable_publish_push_log(
            $log,
            'Sărit — date critice lipsă (categorie/brand/preț): '
    ...
    if (!import_consumable_has_trusted_image($staging)) {
        import_consumable_publish_push_log(
            $log,
            'Public fără imagine (fallback catalog) — adaugă poză din admin sau rerulează scraper.',
            'warn'
        );
    }
    ...
    if ($productId > 0 && !empty($epiesa['vitrina'])) {
        ...
        ++$stats['vitrina'];
        import_consumable_publish_push_log($log, 'ePiesa OK → vitrină homepage: ' . mb_substr($name, 0, 60, 'UTF-8'), 'ok');
    }
...
function import_consumable_publish_build_result(array $stats, array $log): array
{
    $limit = (int) ($stats['limit'] ?? 0);
    $summary = 'Import consumabile: ' . (int) ($stats['published'] ?? 0) . '/' . $limit
        . ' — +' . (int) ($stats['added'] ?? 0) . ' noi, ~' . (int) ($stats['updated'] ?? 0) . ' actualizate'
        . ((int) ($stats['vitrina'] ?? 0) > 0 ? (', ' . (int) $stats['vitrina'] . ' pe vitrină') : '') . '.';
    import_consumable_publish_push_log($log, $summary, ($stats['published'] ?? 0) > 0 ? 'ok' : 'info');

    return [
        'success' => ($stats['published'] ?? 0) > 0 || ($stats['skipped'] ?? 0) > 0,
        'message' => $summary,
        'stats' => $stats,
 ...
 * @return array{ok:bool,job_id?:string,error?:string,message?:string}
 */
function import_consumable_publish_job_start(array $products, array $options = []): array
{
    import_job_extend_runtime(120);

    if ($products === []) {
        return ['ok' => false, 'error' => 'no_products', 'message' => 'Nu există produse de importat.'];
    }
    ...
    $jobId = import_job_create('consumable_publish', [
        'phase' => 'publish_products',
        'progress' => 1.0,
        'message' => 'Pornesc import consumabile (1 produs / pas)...',
        'limit' => $limit,
        'selected_count' => count($products),
    ]);
    ...
    import_job_save_state($jobId, [
        ...
        'stats' => import_consumable_publish_default_stats($limit, count($products)),
        'log' => [],
    ]);
...
/** @return array{ok:bool,status?:array<string,mixed>,result?:array<string,mixed>,error?:string} */
function import_consumable_publish_job_step(string $jobId, array $meta, array $state, PDO $pdo): array
{
    import_job_extend_runtime(300);

    require_once dirname(__DIR__, 3) . '/config/cron_import.php';

    $products = is_array($state['products'] ?? null) ? $state['products'] : [];
    $index = (int) ($state['product_index'] ?? 0);
    $limit = max(1, (int) ($state['limit'] ?? ($meta['limit'] ?? 10)));
    $checkEpiesa = import_consumable_wants_epiesa($state);
    $stats = is_array($state['stats'] ?? null)
        ? $state['stats']
        : import_consumable_publish_default_stats($limit, count($products));
    $log = is_array($state['log'] ?? null) ? $state['log'] : [];
    $markupService = new AdaosComercialService();
    $publishMode = admin_cron_import_publish_mode();

    if ($index === 0 && ($stats['published'] ?? 0) === 0 && ($stats['skipped'] ?? 0) === 0) {
        import_consumable_publish_push_log($log, 'Import consumabile pornit — limită ' . $limit . ' produse (1/pas).', 'info');
    }

    if ($index >= count($products) || (int) ($stats['published'] ?? 0) >= $limit) {
        $result = import_consumable_publish_build_result($stats, $log);
        import_job_update($jobId, [
            'status' => 'done',
            'phase' => 'done',
            'progress' => 100.0,
            'message' => (string) ($result['message'] ?? 'Import finalizat.'),
            'result' => $result,
        ]);
        import_job_save_state($jobId, []);
    ...
    if (!is_array($product)) {
        ++$index;
        $state['product_index'] = $index;
        import_job_save_state($jobId, $state);

        return ['ok' => true, 'status' => import_job_public_status(import_job_load_meta($jobId) ?? $meta)];
    }

    $processed = import_consumable_publish_process_one($pdo, $product, [
        'limit' => $limit,
        'check_epiesa' => $checkEpiesa,
        'publish_mode' => $publishMode,
    ], $markupService, $stats, $log);

    $stats = $processed['stats'];
    $log = $processed['log'];
    ++$index;

    $progress = count($products) > 0
        ? min(99.0, ($index / count($products)) * 100.0)
        : 99.0;
    ...
    $name = mb_substr((string) ($product['pName'] ?? ''), 0, 48, 'UTF-8');
    $done = $index >= count($products) || (int) ($stats['published'] ?? 0) >= $limit;
    ...
    import_job_update($jobId, [
        'phase' => 'publish_products',
        'progress' => $progress,
        'message' => 'Import consumabile (' . $index . '/' . count($products) . '): ' . $name
            . ' — publicate ' . (int) ($stats['published'] ?? 0) . '/' . $limit,
    ]);
...
function import_consumable_scan_publish(PDO $pdo, array $products, array $options = []): array
{
    ...
    import_consumable_publish_push_log($log, 'Import consumabile pornit — limită ' . $limit . ' produse.', 'info');
    ...
    return import_consumable_publish_build_result($stats, $log);
}
...
function import_consumable_log_active_image_plans(?callable $log = null): void
{
    ...
    if (class_exists(\Besoiu\Core\Scraper\ScraperHooks::class)) {
        $plans = \Besoiu\Core\Scraper\ScraperHooks::activeImagePlans();
        $labels = [];
...
/** Consumabile: publică fără imagine dacă restul datelor e OK (imaginile pot fi adăugate ulterior). */
function import_consumable_blocks_auto_publish(array $row): bool
{
    if (!function_exists('besoiu_import_row_critical_flags')) {
        require_once dirname(__DIR__, 4) . '/system/import-queue-critical.php';
    }

    foreach (besoiu_import_row_critical_flags($row) as $flag) {
 ...
 * @param array<string, mixed> $product
 * @param callable(string,string):void|null $log
 * @param array<string, mixed> $resolveOpts force_scraper, budget_sec
 * @return array<string, mixed>
 */
function import_consumable_resolve_image(
    array $product,
    ?callable $log = null,
    ...
    import_consumable_log_active_image_plans($logFn);

    $categories = import_consumable_detect_categories($product);
    $extraQueries = import_consumable_emag_queries($product);
    $forceScraper = !empty($resolveOpts['force_scraper']) || !empty($resolveOpts['force']);
    $skipSources = is_array($resolveOpts['skip_sources'] ?? null)
        ? $resolveOpts['skip_sources']
        : import_consumable_skip_scraper_sources();

    $logFn('Imagine (pipeline Scraper Plan 1→N, mod consumabil)…', 'info');
    $resolved = \ImageSearchService::resolve($product, [
        'consumable_import' => true,
        'force' => $forceScraper,
        'categories' => $categories,
        'pipeline_extra_queries' => $extraQueries,
        'skip_sources' => $skipSources,
        'budget_sec' => max(45, min(90, (int) ($resolveOpts['budget_sec'] ?? 75))),
    ...
    $logFn('Niciun plan scraper nu a returnat imagine — produsul poate fi publicat fără poză (fallback catalog).', 'warn');
...
function import_consumable_cron_publish_batch(
    ...
    if (!$skipTecdocCsv) {
        ...
        $sharedLookup = function_exists('import_build_tecdoc_lookup_for_products')
            ...
            : [];
        $log('Index TecDoc gata — ' . count($sharedLookup['lookup'] ?? $sharedLookup) . ' potriviri.', 'ok', 12);
    } else {
