...
function import_base_allowed_part_brands(): array
{
    ...
    $brands = [];
    $path = import_base_bovsoft_aux_path('lista brand piese.txt')
        ?? (dirname(__DIR__, 3) . '/data/import_base_allowed_part_brands.txt');
    if (is_file($path)) {
...
function import_base_import_data_root(): string
{
    static $root = null;
    if (is_string($root)) {
        return $root;
    ...
    if ($env !== '' && is_dir($env)) {
        return $root = rtrim(str_replace('\\', '/', $env), '/');
    }

    $syncFile = dirname(__DIR__, 3) . '/config/besoiupieseimport_sync.php';
    if (is_file($syncFile)) {
        $sync = require $syncFile;
        if (is_array($sync)) {
            $configured = rtrim(str_replace('\\', '/', (string)($sync['import_root'] ?? '')), '/');
    ...
    return $root = 'F:/laragon/www/besoiupieseimport';
}

function import_base_bovsoft_aux_path(string $filename): ?string
{
    ...
    $candidates = [
        import_base_import_data_root() . '/Prelucrare fisiere bovsoft-base/' . $filename,
        dirname(__DIR__, 3) . '/data/' . $filename,
        dirname(__DIR__, 3) . '/data/import_base_name_overrides.txt',
    ];
...
function import_base_custom_name_overrides_path(): string
{
    return dirname(__DIR__, 3) . '/storage/import/name_overrides_custom.txt';
}

function import_base_clear_name_overrides_cache(): void
{
    import_base_reset_name_overrides_cache();
}

function import_base_reset_name_overrides_cache(): void
{
    import_base_load_name_overrides(true);
}

function import_base_name_overrides_path(): string
{
    return import_base_bovsoft_aux_path('denumiri produse.txt')
        ?? (dirname(__DIR__, 3) . '/data/import_base_name_overrides.txt');
}
 ...
 */
function import_base_load_name_overrides(bool $forceReload = false): array
{
    static $cache = null;
    ...
    $cache = [];
    foreach ([import_base_name_overrides_path(), import_base_custom_name_overrides_path()] as $path) {
        if (!is_file($path)) {
            continue;
        }
        foreach (preg_split('/\r?\n/', (string) file_get_contents($path)) ?: [] as $line) {
...
function import_base_format_product_name_title(string $name): string
{
    $name = trim($name);
    if ($name === '') {
        return '';
 ...
 *   matched_key: string,
 *   cleaned: string,
 *   patterns_applied: list<string>
 * }
 */
function import_base_normalize_product_name_detail(string $rawName): array
{
    ...
    $matchedKey = $override !== '' ? import_base_normalize_name_key($rawName) : '';
    $name = import_base_cleanup_product_name($override !== '' ? $override : $rawName);
    $patternsApplied = [];
    $source = $override !== '' ? 'override' : 'patterns';
    ...
    if ($override === '') {
        foreach (import_base_tecdoc_name_patterns() as [$pattern, $replacement]) {
            ...
            if ($next !== null && $next !== $name) {
                $patternsApplied[] = $replacement;
                $name = $next;
            }
        }
        if ($patternsApplied === [] && import_base_normalize_name_key($rawName) === import_base_normalize_name_key($name)) {
            $source = 'unchanged';
        }
    }
    ...
    return [
        'raw' => $rawName,
        'final' => import_base_format_product_name_title($name),
        'source' => $source,
        'override' => $override,
...
{
    return import_base_normalize_product_name_detail($rawName)['final'];
}

/**
 * Extrage denumirea TecDoc brută (ART_NAME) dintr-un produs de coadă / staging.
 *
 * @param array<string, mixed> $row
 */
function import_extract_tecdoc_art_name_from_product(array $row): string
{
    ...
    if (isset($raw['tecdoc_audit']) && is_array($raw['tecdoc_audit'])) {
        $value = trim((string) ($raw['tecdoc_audit']['name'] ?? ''));
        if ($value !== '') {
            return $value;
        }
    }
...
/**
 * Aplică maparea denumiri produse.txt (și pattern-uri) la pSubcategory — ca la Import Base.
 *
 * @param array<string, mixed> $row
 * @return array<string, mixed>
 */
function import_apply_tecdoc_name_normalization(array $row): array
{
    $rawArtName = import_extract_tecdoc_art_name_from_product($row);
    if ($rawArtName === '') {
        return $row;
    }

    $normalized = import_base_normalize_product_name($rawArtName);
    if ($normalized === '') {
        return $row;
    }
    ...
    $detail = import_base_normalize_product_name_detail($rawArtName);
    $raw['__tecdoc_art_name'] = $rawArtName;
    $raw['__name_normalize_source'] = (string) ($detail['source'] ?? '');
...
/**
 * True dacă textul arată a tip motor auto (nu notă TecDoc / specificații).
 */
function import_is_plausible_engine_label(string $value): bool
{
    $value = trim(html_entity_decode($value, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'));
    if ($value === '' || mb_strlen($value, 'UTF-8') > 48) {
        return false;
    }
    ...
    foreach ([
        'specificat', 'atentie', 'atenție', 'informatii', 'informații', 'service',
        'tecdoc', 'compatibil', 'descriere', 'producator', 'producător', 'garantie',
        'garanție', 'numar articol', 'număr articol', 'parts_info', 'terms_of',
    ] as $bad) {
        if (str_contains($lower, $bad)) {
            return false;
        }
    }
    if (preg_match('/\b\d+[.,]\d+\b/u', $value)) {
        return true;
    }
    if (preg_match('/\b\d{2,4}\s*(kw|cp|hp|ps)\b/iu', $value)) {
        return true;
    }
    if (preg_match('/\b(tdi|tsi|tfsi|cdi|hdi|gdi|mpi|dci|hybrid|benzina|benzină|diesel|gpl|electric)\b/iu', $value)) {
        return true;
    }
...
/** Curăță pMotorizare — păstrează doar tipuri motor plauzibile. */
function import_sanitize_motorizare_value(string $value): string
{
    $value = trim($value);
    if ($value === '') {
        return '';
    ...
    foreach (preg_split('/\r?\n|;/u', $value) ?: [] as $line) {
        ...
        if ($line === '' || !import_is_plausible_engine_label($line)) {
            continue;
        }
        $keep[$line] = true;
        if (count($keep) >= 12) {
...
/** Din HTML descriere Import Pro / Base.html: doar blocul „Compatibil cu…”. */
function import_compat_text_from_description_html(string $html): string
{
    $html = trim($html);
    if ($html === '') {
        return '';
    ...
    // Base.html: <p><b>Compatibil cu urmatoarele modele auto:</b></p><ul>…nested…</ul>
    // Import Pro vechi: <p><b>Compatibil cu:</b></p><ul>…
    if (preg_match(
        '/Compatibil\s+cu(?:\s+urmatoarele\s+modele\s+auto)?\s*:?\s*<\/(?:b|strong|p|span)[^>]*>\s*<ul>(.*)$/is',
        $html,
        $m
    ) || preg_match(
        '/<p[^>]*>\s*<b>\s*Compatibil\s+cu(?:\s+urmatoarele\s+modele\s+auto)?\s*:?\s*<\/b>\s*<\/p>\s*<ul>(.*)$/is',
        $html,
        $m
    )) {
        $block = (string) $m[1];
        // Taie la următoarea secțiune (OEM) dacă există.
        ...
        if (preg_match_all('/<li[^>]*>(.*?)<\/li>/is', $block, $items)) {
            foreach ($items[1] as $item) {
                ...
                $line = trim((string) preg_replace('/\s+/u', ' ', $line));
                if ($line === '' || str_starts_with($line, '…') || str_starts_with($line, 'Motorizari:')) {
                    continue;
                }
                // Evită linii-marcă goale tip „AUDI” fără model (vin din <li><b>AUDI</b><ul>).
                if (preg_match('/^[A-ZÄÖÜË\-]{2,20}$/u', $line) && !preg_match('/\d/u', $line)) {
                    continue;
                }
                $lines[] = $line;
            }
        ...
        return implode("\n", array_slice(array_values(array_unique($lines)), 0, 40));
    }

    $plain = strip_tags(str_replace(['</li>', '<br>', '<br/>', '<br />'], "\n", $html));
    if (preg_match('/Compatibil\s+cu(?:\s+urmatoarele\s+modele\s+auto)?\s*:\s*(.+)$/is', $plain, $m)) {
        $chunk = trim((string) $m[1]);
        if (preg_match('/^(.*?)(?:Coduri\s+OE)/is', $chunk, $cut)) {
            $chunk = trim((string) $cut[1]);
        }
        $lines = [];
        foreach (preg_split('/\r?\n/u', $chunk) ?: [] as $line) {
            $line = trim((string) preg_replace('/\s+/u', ' ', $line));
...
function import_base_strip_redundant_title_suffix(string $pieceName, string $brand, string $code): string
{
    ...
    $candidates = array_values(array_unique(array_filter([
        ...
        trim($canonical . ' ' . $code),
        // Brand singur la final (ex. header Base „Placute frana ATE”).
        $brandLabel,
        $canonical,
        trim($brand),
        $codeLabel,
...
/**
 * Descriere Website: doar antet + specificații (fără Compatibil nested / OE).
 * Formatul Base.html complet e rezervat marketplace (PieseAuto export).
 */
function import_base_generate_description_website(array $entries): string
{
    if ($entries === []) {
        return '';
    }
    ...
    foreach (preg_split('/\s*\|\s*/u', import_base_normalize_special_chars($specs)) ?: [] as $line) {
        ...
        if (str_contains($line, ':')) {
            ...
            $dedupeKey = function_exists('mb_strtolower')
                ...
                : strtolower($key);
            if ($dedupeKey !== '' && isset($seenSpecKeys[$dedupeKey])) {
                continue;
            }
            $rendered = htmlspecialchars($key . ': ' . $val, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        } else {
            $dedupeKey = function_exists('mb_strtolower')
            ...
            $rendered = htmlspecialchars($line, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        }
        if ($dedupeKey !== '') {
            $seenSpecKeys[$dedupeKey] = true;
        }
        $specLines[] = '<li>' . $rendered . '</li>';
    }
    if ($specLines !== []) {
...
/**
 * Din HTML Base complet → variantă website (taie Compatibil + Coduri OE).
 */
function import_base_description_website_from_marketplace(string $baseHtml): string
{
    $baseHtml = trim($baseHtml);
    if ($baseHtml === '') {
        return '';
    }
    if (preg_match('/^(.*?)(?:\s*<p>\s*<b>\s*Compatibil\s+cu)/is', $baseHtml, $m)) {
        return trim((string) $m[1]);
    }
    if (preg_match('/^(.*?)(?:\s*<p>\s*<b>\s*Coduri\s+OE)/is', $baseHtml, $m)) {
        return trim((string) $m[1]);
    }
...
/** Descriere Marketplace = generateDescription() din Procesare fisier import Base.html. */
function import_base_generate_description(array $entries): string
{
        ...
        foreach (preg_split('/\s*\|\s*/u', import_base_normalize_special_chars($specs)) ?: [] as $line) {
            ...
            if (str_contains($line, ':')) {
                ...
                $dedupeKey = function_exists('mb_strtolower')
                    ? mb_strtolower($key, 'UTF-8')
                    : strtolower($key);
                // Nu repeta aceeași caracteristică dacă e deja în specificații.
                if ($dedupeKey !== '' && isset($seenSpecKeys[$dedupeKey])) {
                    continue;
                }
                $rendered = htmlspecialchars($key . ': ' . $val, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    ...
    // Ierarhie Base/htmlviewer: Marcă (bold) → Model (bold) → Motorizare (listă).
    // Peste prag: sumar „Motorizari:” (ca în Base.html când sunt foarte multe variante).
    $motorizariDetailedMax = 12;
    $compHtml = '';
    ksort($compatMap);
    foreach ($compatMap as $carBrand => $models) {
        ...
        foreach ($models as $modelKey => $data) {
            ...
            } elseif ($n <= $motorizariDetailedMax) {
...
function import_base_entries_with_car_brand(array $entries): array
{
    return array_values(array_filter($entries, static function (array $entry): bool {
        return trim((string) ($entry['CAR_BRAND'] ?? '')) !== '';
    }));
}
...
function import_base_extract_vehicle_summary(array $entries): array
{
    ...
    $working = $allowed !== [] ? $allowed : import_base_entries_with_car_brand($entries);
    $marca = [];
    $model = [];
    $motorizare = [];
...
function import_base_apply_without_vehicle(array &$base, array $rows): void
{
    ...
    $firstEntry = $entries[0];
    $rawArtName = trim((string) ($firstEntry['ART_NAME'] ?? ''));
    if ($rawArtName !== '') {
        $base['__tecdoc_art_name'] = $rawArtName;
    }
    $normalizedName = import_base_normalize_product_name($rawArtName);
...
function import_base_apply_to_product(array &$base, array $group, array $priceIndex = []): bool
{
    ...
    if ($allowedEntries === [] && import_base_has_vehicle_data($rows)) {
        $allowedEntries = import_base_entries_with_car_brand($entries);
        if ($allowedEntries === []) {
            return false;
        }
    }
