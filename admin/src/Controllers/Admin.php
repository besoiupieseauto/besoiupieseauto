...
use Config\Database;
use Besoiu\Controllers\Redirector;
...
class Admin
{
    ...
    public function workspace(string $dir = ''): void
    {
        ...
        $file = $this->workspaceTemplatePath();
        if (!is_file($file)) {
    ...
    private function workspaceTemplatePath(): string
    {
        if (defined('BESOIU_ADMIN')) {
            return BESOIU_ADMIN . '/Templates/admin/pages/workspace/workspace.php';
        }

        return dirname(__DIR__, 4) . '/admin/Templates/admin/pages/workspace/workspace.php';
    }
    ...
    public function workspaceSet(string $dir = ''): void
    {
        ...
        $allowed = \Besoiu\Core\Auth\AdminWorkspace::allowedWorkspacesForSession();
        ...
        \Besoiu\Core\Auth\AdminWorkspace::setCurrent($workspaceId);
        $redirect = \Besoiu\Core\Auth\AdminWorkspaceCatalog::dashboardPath($workspaceId);
    ...
    public function workspaceSwitch(string $dir = ''): void
    {
        ...
        \Besoiu\Core\Auth\AdminWorkspace::setCurrent($workspaceId);
        $fallback = \Besoiu\Core\Auth\AdminWorkspaceCatalog::dashboardPath($workspaceId);
        $redirect = self::sanitizeWorkspaceRedirect(
    ...
    /** Redirect intern sigur după schimbare departament (?redirect=/admin/...). */
    private static function sanitizeWorkspaceRedirect(string $redirect, string $fallback): string
    {
        if ($redirect === '') {
            return $fallback;
        }
        ...
        $path = AdminUrl::normalizeRequestPath((string) $path);
    ...
    {
        // Ținte stub — doar pagini care există în proiectul nou.
        // Module neportate (orders/facturi/bots/blog/…) → dashboard, nu 404.
        $stubPairs = [
            ['addalerts', 'alerts'],
            ...
            ['addsearch-logs', 'dashboard'],
            ['profilesearch-logs', 'dashboard'],
            ['addscan', 'dashboard'],
            ['profilescan', 'dashboard'],
            ['addcron', 'dashboard'],
            ['profilecron', 'dashboard'],
            ['addreport', 'dashboard'],
            ['profilereport', 'dashboard'],
            ['addcross-reference', 'dashboard'],
            ['profilecross-reference', 'dashboard'],
            ['addfacturi', 'dashboard'],
            ['profilefacturi', 'dashboard'],
            ['addmessages', 'dashboard'],
            ['profilemessages', 'dashboard'],
            ['addbots', 'dashboard'],
            ['profilebots', 'dashboard'],
            ['addmarketplace', 'dashboard'],
            ['profilemarketplace', 'dashboard'],
            ['addblog', 'dashboard'],
            ['profileblog', 'dashboard'],
            ['orders', 'dashboard'],
            ['order-create', 'dashboard'],
            ['abandoned-carts', 'dashboard'],
            ['facturi', 'dashboard'],
            ['livrare', 'dashboard'],
            ['caietcomenzi', 'dashboard'],
            ['caiet-de-comenzi', 'dashboard'],
            ['comenzi-tm', 'dashboard'],
            ['comenzi-utvin', 'dashboard'],
            ['comenzi-externe', 'dashboard'],
        ];
        ...
        // Centru AI unificat — /admin/ai-agent → /admin/ai-rag?tab=chat
        if ($slug === 'ai-agent' || $path === AdminUrl::path('ai-agent')) {
            header('Location: ' . AdminUrl::path('ai-rag') . '?tab=chat', true, 302);
            exit;
        }
