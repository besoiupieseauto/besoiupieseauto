    ...
    <div class="mt-5 box p-5">
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 md:col-span-6">
                ...
                <label class="mb-1 block text-sm">Selectează furnizori</label>
...
(function () {
    ...
    function fetchErrorMessage(error, response) {
        if (error && (error.name === 'AbortError' || error.name === 'TimeoutError')) {
            return 'Căutarea a expirat (' + Math.round(SEARCH_TIMEOUT_MS / 1000) + 's). Reduce furnizorii sau reîncearcă.';
        }
        if (error instanceof TypeError && String(error.message || '').toLowerCase().includes('fetch')) {
            return 'Serverul nu a răspuns (conexiune întreruptă sau request PHP blocat). Reîncarcă pagina.';
        }
        if (response && response.status === 503) {
            return 'API oprit — activează din header sau verifică setările API.';
        }
        return (error && error.message) ? error.message : 'Eroare de rețea.';
    }
    ...
    function renderSupplierCheckboxes(suppliers) {
        ...
        checkboxWrap.innerHTML = supplierCatalog.map((row) => {
            ...
            const title = row.connected
                ? 'Conectat — căutare live disponibilă'
                : escapeHtml(row.reason || 'Neconectat — poți bifa, dar căutarea poate returna 0 rezultate.');

            const stateClass = row.connected
                ? 'border-primary/40 bg-primary/5 cursor-pointer'
                : 'border-warning/30 bg-warning/5 cursor-pointer';

            const dot = row.connected
                ? '<span class="inline-block h-1.5 w-1.5 rounded-full bg-primary" title="conectat"></span>'
                : '<span class="inline-block h-1.5 w-1.5 rounded-full bg-warning" title="fără credențiale search"></span>';

            return `<label class="inline-flex items-center gap-1.5 rounded border px-2 py-1 ${stateClass}" title="${title}">
        ...
        if (meta) {
            ...
            meta.textContent = connectedCount + ' conectat(i) din ' + supplierCatalog.length
                + ' — poți bifa orice furnizor; cei cu punct galben necesită credențiale Supplier Search în .env.';
    ...
    async function runSearch() {
        ...
        try {
            ...
            results.innerHTML = `<tr><td colspan="6" class="px-3 py-6 text-center text-danger">${escapeHtml(message)}</td></tr>`;
