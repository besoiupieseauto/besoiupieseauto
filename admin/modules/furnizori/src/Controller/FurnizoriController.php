...
namespace Besoiu\Modules\Furnizori\Controller;
...
class FurnizoriController
{
    ...
    {
        $service = new \Besoiu\Modules\Furnizori\Service\PriceFormationLogicService();

        return [
            'config' => $service->getConfig(),
            'suppliers' => $service->listAvailableSuppliers(),
            'blocked_suppliers' => $service->listBlockedSuppliers(),
        ];
    }
    ...
    private function buildPayload(array $rawInput, bool $isUpdate): array
    {
        ...
        if (isset($payload['scan_schedule_mode'])) {
            $payload['scan_schedule_mode'] = $this->normalizeChoice(
                ...
                \Besoiu\Modules\Furnizori\Service\SupplierScanScheduleService::allowedModes(),
