...
use Besoiu\Modules\Furnizori\Model\PriceFormationLogicRepository;
...
class PriceFormationLogicService
{
    ...
    public function __construct(
        private readonly PriceFormationLogicRepository $model = new PriceFormationLogicRepository()
    ) {
    }
    ...
    {
        $rulesLib = (defined('BESOIU_BACKEND') ? BESOIU_BACKEND : dirname(__DIR__, 4) . '/app/Backend') . '/src/Controllers/Produse/import_supplier_stock_zero_lib.php';
        if (is_file($rulesLib)) {
    ...
    public function listBlockedSuppliers(): array
    {
        $catalog = import_furnizori_catalog();
        $items = [];
        foreach (import_furnizori_blocked_codes() as $code) {
            $items[] = [
                'code' => $code,
                'name' => (string) ($catalog[$code]['name'] ?? $code),
            ];
    ...
    /** Elimină furnizorii blocați din logica salvată (freeze global). */
    public function syncBlockedFreeze(): array
    {
        return $this->saveConfig($this->getConfig());
    }
