 ...
 */
class SupplierImportFilesBrowser
{
    ...
    public function browse(array|string $furnizorOrCode, string $requestedPath = '', array $options = []): array
    {
        ...
        // Folder local = sursă de adevăr; staging doar pentru fișiere încă necopiate.
        $entries = [];
        $seenNames = [];
        foreach ($localEntries as $entry) {
            $nameKey = $this->entryNameKey($entry);
        ...
        foreach ($this->listEntries($supplierCode) as $entry) {
            $nameKey = $this->entryNameKey($entry);
            if ($nameKey === '' || isset($seenNames[$nameKey])) {
                continue;
            }
        ...
        if ($isRemoteConnection && $includeRemote) {
            ...
            $path = $feedFolderRelative;
            $mode = 'Fisiere din folder furnizor (admin/storage/supplier_feeds/)';
        }
    ...
    private function entryNameKey(array $entry): string
    {
    ...
    private function listEntries(string $supplierCode): array
    {
        ...
        foreach (list_uploaded_import_files() as $meta) {
            ...
            $diskPath = import_temp_file_path($fileId);
