...
use Besoiu\Services\AdaosComercial\PriceFormationTraceService;
use Besoiu\Services\Products\ProductFacetsService;
use Config\Database;
use Besoiu\Core\Produse\ProduseModel;

$acProCss = ModuleAssets::url('adaos_comercial', 'css/module.css');
if ($acProCss !== '') {
    echo '<link rel="stylesheet" href="' . htmlspecialchars($acProCss . '?v=20260717-ac1', ENT_QUOTES, 'UTF-8') . '">';
}
...
<div class="adaos-comercial-page ac-pro-ui">
    <div>
        <header class="ac-hero">
            <div class="ac-hero__main">
                <div class="ac-hero__badge">Adaos comercial · Besoiu Admin</div>
                <h1 class="ac-hero__title">Formare preț magazin</h1>
                <p class="ac-hero__lead">
                    Singurul loc pentru adaosul comercial, TVA magazin și rotunjire globală.
                    Compensatorul pre-import (0% / 5% / 10%) rămâne pe fiecare furnizor — tab Formare preț.
                </p>
            </div>
            <div class="ac-hero__steps">
                <div class="ac-hero-step">
                    <span class="ac-hero-step__num">1</span>
                    <span><strong>Preț furnizor (CSV)</strong> + compensator pre-import % → preț achiziție / preț bază (ex. 100 + 10% = 110 lei)</span>
                </div>
                <div class="ac-hero-step">
                    <span class="ac-hero-step__num">2</span>
                    <span><strong>Preț bază</strong> + TVA magazin + adaos comercial (global / reguli) → preț final. Nu configura adaos comercial pe profilul furnizorului.</span>
                </div>
            </div>
        </header>

        <div class="adaos-page-tabs" role="tablist" aria-label="Secțiuni adaos comercial">
            <button type="button" class="adaos-page-tab<?= $activeAdaosTab === 'rules' ? ' active' : '' ?>" data-adaos-page-tab="rules" aria-selected="<?= $activeAdaosTab === 'rules' ? 'true' : 'false' ?>">
                <span class="adaos-page-tab__icon">%</span>
                Reguli adaos
            </button>
            <button type="button" class="adaos-page-tab<?= $activeAdaosTab === 'price-log' ? ' active' : '' ?>" data-adaos-page-tab="price-log" aria-selected="<?= $activeAdaosTab === 'price-log' ? 'true' : 'false' ?>">
                <span class="adaos-page-tab__icon">↗</span>
                Log formare preț
            </button>
        </div>
        ...
        <div class="ac-config-grid">
            <div class="ac-config-card ac-config-card--vat markup-card">
                <div class="ac-config-card__head">
                    <div class="ac-config-card__icon">TVA</div>
                    <div>
                        <h3 class="ac-config-card__title">TVA preț final</h3>
                        <p class="ac-config-card__desc">Se aplică pe prețul de achiziție, înainte de adaosul comercial.</p>
                    </div>
                </div>
                <div class="ac-config-card__actions">
                    <label class="ac-field">
                        <span>TVA %</span>
                        <input id="commercial_vat_percent" class="ac-input w-28" type="number" min="0" max="100" step="0.01" value="<?= h_ac((string)$commercialVatPercent) ?>">
                    </label>
                    <button type="button" id="saveCommercialVatBtn" class="ac-btn ac-btn--primary">
                        Salvează TVA
                    </button>
                </div>
                <p class="ac-config-card__hint">
                    Exemplu: baza 100 lei + TVA <?= h_ac((string)$commercialVatPercent) ?>% = <?= h_ac(rtrim(rtrim(number_format(100 * (1 + $commercialVatPercent / 100), 2, '.', ''), '0'), '.')) ?> lei → + adaos comercial pe suma cu TVA.
                </p>
            </div>

            <div class="ac-config-card ac-config-card--markup markup-card">
                <div class="ac-config-card__head">
                    <div class="ac-config-card__icon">+</div>
                    <div>
                        <h3 class="ac-config-card__title">Adaos comercial global</h3>
                        <p class="ac-config-card__desc">Se aplică după TVA magazin, pe prețul cu TVA inclus.</p>
                    </div>
                </div>
                <div class="ac-config-card__actions">
                    <label class="ac-field">
                        <span>Adaos global %</span>
                        <input id="global_commercial_markup_percent" class="ac-input w-28" type="number" min="0" max="1000" step="0.01" value="<?= h_ac(rtrim(rtrim(number_format($globalCommercialMarkupPercent, 2, '.', ''), '0'), '.')) ?>" placeholder="ex: 30">
                    </label>
                    <button type="button" id="saveGlobalCommercialMarkupBtn" class="ac-btn ac-btn--primary">
                        Salvează adaos
                    </button>
                    <button type="button" id="reapplyGlobalMarkupAllBtn" class="ac-btn ac-btn--success">
                        Reaplică pe toate
                    </button>
                </div>
                <p class="ac-config-card__hint">
                    Exemplu: achiziție 100 lei + TVA → <?= h_ac(rtrim(rtrim(number_format(100 * (1 + $commercialVatPercent / 100), 2, '.', ''), '0'), '.')) ?> lei + <?= h_ac(rtrim(rtrim(number_format($globalCommercialMarkupPercent, 2, '.', ''), '0'), '.')) ?>% adaos global.
                </p>
            </div>

            <div class="ac-config-card ac-config-card--round markup-card">
                <div class="ac-config-card__head">
                    <div class="ac-config-card__icon">≈</div>
                    <div>
                        <h3 class="ac-config-card__title">Rotunjire preț magazin</h3>
                        <p class="ac-config-card__desc">Setare globală aplicată după TVA. Pe site, prețurile se afișează fără zecimale când rotunjirea este activă.</p>
                    </div>
                </div>
                <div class="ac-config-card__actions">
                    <label class="ac-field">
                        <span>Mod rotunjire</span>
                        <select id="global_price_round_mode" class="ac-select min-w-[220px]">
                    ...
                    <label class="ac-field" id="global_price_round_value_wrap" style="<?= $globalPriceRoundMode === 'round_to' ? '' : 'display:none;' ?>">
                        <span>Rotunjire la (lei)</span>
                        <input id="global_price_round_value" class="ac-input w-28" type="number" min="0.01" step="0.01" value="<?= h_ac(rtrim(rtrim(number_format($globalPriceRoundValue, 2, '.', ''), '0'), '.')) ?>" placeholder="ex: 5">
                    </label>
                    <button type="button" id="saveGlobalPriceRoundBtn" class="ac-btn ac-btn--primary">
                        Salvează rotunjirea
                    </button>
                </div>
                <p class="ac-config-card__hint">
        ...
        <div class="mt-6 grid grid-cols-12 gap-6">
            <div class="col-span-12 xl:col-span-4">
                <div class="ac-panel markup-card">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h3 id="markupFormTitle" class="ac-panel__title">Regulă nouă</h3>
                            <p class="ac-panel__desc">Definește filtrele și adaosul comercial (pasul 2 din formarea prețului).</p>
                        </div>
                        <button type="button" onclick="resetMarkupForm()" class="ac-btn ac-btn--ghost px-3">Resetează</button>
                    </div>
            ...
            <div class="col-span-12 xl:col-span-8">
                <div class="ac-panel markup-card">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h3 class="ac-panel__title">Reguli salvate</h3>
                            <p class="ac-panel__desc">Previzualizezi întâi impactul, apoi aplici regula doar când ești sigur.</p>
                        </div>
                ...
                <div class="ac-panel mt-6 markup-card">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h3 class="ac-panel__title">Previzualizare impact</h3>
                            <p class="ac-panel__desc">Aici vezi primele produse afectate și diferența de preț calculată.</p>
                        </div>
        ...
        <div id="adaos-page-pane-price-log" class="adaos-page-pane mt-4<?= $activeAdaosTab === 'price-log' ? ' active' : '' ?>" data-adaos-page-pane="price-log" style="display:<?= $activeAdaosTab === 'price-log' ? 'block' : 'none' ?>">
