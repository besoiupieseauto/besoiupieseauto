        ...
        <div class="col-span-11 2xl:col-span-9">
            ...
            <div class="box relative mt-8 p-5 before:absolute before:inset-0 before:mx-3 before:-mb-3 before:border before:border-foreground/10 before:bg-background/30 before:shadow-[0px_3px_5px_#0000000b] before:z-[-1] before:rounded-xl after:absolute after:inset-0 after:border after:border-foreground/10 after:bg-background after:shadow-[0px_3px_5px_#0000000b] after:rounded-xl after:z-[-1] after:backdrop-blur-md">
                <div class="rounded-lg border p-5">
                    ...
                    <div class="mt-5 flex flex-col gap-5">
                        <?php row_start('Titlu site (web)', 'Titlul afișat pe magazinul online.', true); ?><input name="pName" required class="h-10 w-full rounded-md border bg-background px-3 py-2" placeholder="Ex: Plăcuțe frână Bosch 0 986 424 268 pentru Audi A3"><?php row_end(); ?>
                        <?php row_start('Titlu marketplace', 'Titlul folosit la export PieseAuto / BaseLinker.'); ?><input name="pNameMarketplace" class="h-10 w-full rounded-md border bg-background px-3 py-2" placeholder="Ex: Plăcuțe frână Bosch 0 986 424 268 pentru AUDI/VW"><?php row_end(); ?>
...
(function () {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const result = await response.json();
        alert(result.message || (result.success ? 'Produs adăugat.' : 'Eroare la salvare.'));
        if (result.success) {
            // When opened inside the /admin/product iframe modal, navigate the parent.
            const target = '/admin/product';
