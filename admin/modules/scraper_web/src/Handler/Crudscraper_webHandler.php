...
use Besoiu\Modules\ScraperWeb\Controller\ScraperWebController;
use JsonException;
use Throwable;

final class Crudscraper_webHandler
{
    public static function handle(): void
    {
        ...
        try {
            ...
            $controller = new ScraperWebController();
            $action = (string) $payload['type_product'];

            $response = match ($action) {
                'list' => [
                    'success' => true,
                    'message' => 'Scraper Web — listă încărcată.',
                    'data' => $controller->list($payload),
                ],
                'add' => [
                    'success' => true,
                    'message' => 'Scraper Web — înregistrare adăugată.',
