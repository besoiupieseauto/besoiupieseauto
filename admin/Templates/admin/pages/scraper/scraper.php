...
$scraperCatalogCount = (new ScraperService())->epiesaProductCount();
    ...
    <div id="sc-view-detail" class="hidden">
        ...
        <div class="sc-tab-panel" data-sc-tab-panel="testare">
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12 lg:col-span-4">
                    <div class="sc-box">
                        ...
                        <label class="sc-field">
                            <span id="sc-test-query-label">Query / termen căutare</span>
                            <input type="text" id="sc-test-query" class="box h-10 w-full rounded-md border px-3 text-sm" placeholder="Cod OEM, VIN sau VW Golf 1.6 TDI">
                        </label>
                        <p id="sc-test-api-hint" class="hidden text-xs text-teal-800 bg-teal-50 border border-teal-200 rounded px-2 py-2 mb-2">
                            <strong>TecDoc API</strong> — exemple:
                            <code class="text-xs">34116761244</code> (OEM),
                            <code class="text-xs">BOSCH 0986424590</code> (brand + cod),
                            <code class="text-xs">vin:WVWZZZ1KZAW123456</code>,
                            <code class="text-xs">VW Golf 1.6 TDI</code> (marcă model motor).
                        </p>
                        <label class="sc-field">
                        ...
                        <div id="sc-test-html-only" class="sc-fetch-opts mb-3 space-y-2 text-sm hidden">
                            <label class="flex items-center gap-2 opacity-60"><input type="checkbox" id="sc-fetch-super" disabled> super (doar fallback scrape.do)</label>
                            <label class="flex items-center gap-2 opacity-60"><input type="checkbox" id="sc-fetch-render" disabled> render JS (doar fallback scrape.do)</label>
                        </div>
                        <p id="sc-test-autodoc-hint" class="text-xs text-emerald-800 bg-emerald-50 border border-emerald-200 rounded px-2 py-1 mb-2">
                            Autodoc: Chrome <strong>pe ecran</strong> la test. Dacă vezi
                            <strong>turnstileBotCheck</strong> — click manual pe bifa Cloudflare (max 2 min).
                            Prima dată: <code>php admin/tools/bootstrap_autodoc_turnstile.php VAT2318HP</code>
                            Blocat? <code>php admin/tools/kill_stealth_chrome.php</code>
                        </p>
                        ...
                        <p id="sc-test-timing-hint" class="mt-2 text-xs opacity-60">Test complet ~30–90s via stealth browser. «Analizează HTML» folosește fișierul salvat la Pas 1.</p>
                    </div>
                </div>
                <div class="col-span-12 lg:col-span-8">
                    ...
                    <div id="sc-test-vehicle-tree-box" class="sc-box mb-4 hidden">
                        <h4 class="sc-box-title">Arbore vehicule ePiesa</h4>
                        <div id="sc-test-vehicle-tree" class="text-sm opacity-90">—</div>
                    </div>
                    <div id="sc-test-howto-html" class="sc-box mb-4">
                        <h4 class="sc-box-title">Cum funcționează (pe scurt)</h4>
                        <ol class="text-sm space-y-2 m-0 pl-4 opacity-90">
                            <li><strong>Pas 1</strong> — descarcă pagina (HTML). <span id="sc-howto-p1">Rulează testul.</span></li>
                            <li><strong>Pas 2</strong> — extrage produse din HTML (selectori CSS). <span id="sc-howto-p2">Rulează testul.</span></li>
                    ...
                    <div id="sc-test-howto-api" class="sc-box mb-4 hidden">
                        <h4 class="sc-box-title">Cum funcționează TecDoc API</h4>
                        <ol class="text-sm space-y-2 m-0 pl-4 opacity-90">
                            <li><strong>Cod OEM</strong> — caută articol + imagine oficială (ex: <code>34116761244</code>, <code>BOSCH 0986424590</code>)</li>
                            <li><strong>VIN</strong> — decodează vehiculul și afișează <code>car_id</code> TecDoc</li>
                            <li><strong>Marcă model motor</strong> — rezolvă motorizări (ex: <code>VW Golf 1.6 TDI</code>)</li>
                        </ol>
                        <p class="text-sm mt-3 mb-0 opacity-70">Nu folosește scrape.do — interogare directă RapidAPI (cheie în admin/.env).</p>
                    </div>
                    <div id="sc-test-diagnostics-box" class="sc-box mb-4">
        ...
        <div class="sc-tab-panel" data-sc-tab-panel="agent">
            <div class="sc-box mb-4">
                ...
                <p class="text-xs opacity-60 mt-2">Prioritate: recunoaștere heuristică, apoi <code>GROQ_KEY</code> / <code>OPENAI_KEY</code> / Metro LLM (Ollama).</p>
            </div>
            <div class="sc-box mb-4">
...
<div id="sc-modal-create" class="sc-modal hidden" role="dialog" aria-modal="true">
    ...
    <div class="sc-modal-box">
        ...
        <p class="sc-subtitle mb-2">Creezi un card nou — apoi configurezi pașii 1, 2, 3 și testezi.</p>
        <p class="sc-subtitle mb-4 text-xs" style="padding:0.5rem 0.75rem;border-radius:0.375rem;border:1px solid #a7f3d0;background:#ecfdf5;color:#065f46;">
            Motor implicit: <strong>stealth-browser-mcp</strong> (Chrome + profil persistent). La Cloudflare așteaptă până la 120s; cookie-urile rămân pentru următoarele teste.
        </p>
