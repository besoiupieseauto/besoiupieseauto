...
$scraperBuiltinCatalog = (new ScraperService())->getStepCatalog();
unset($scraperBuiltinCatalog['extraction_goals']);
?>
<script>
...
window.ScraperStepBuilder = (function () {
    ...
    function defaultParams(type) {
        ...
        if (type === 'vehicle_tree') {
            return { resume: true, delay_ms: 120, max_brands: 1, max_models_per_brand: 2, marca_id: 0 };
        }
        return {};
    }
    ...
    function renderStepParams(step) {
        ...
        if (t === 'vehicle_tree') {
            return `
                <div class="sc-veh-panel full">
                    <p class="text-sm opacity-80 m-0 mb-3">
                        API ePiesa <code>/api/selector.php</code>: <strong>marcă → model → generație → motor</strong>.
                        Salvează în <code>storage/scraper/epiesa/vehicle_tree.json</code>.
                        Bifează <strong>Activ pas</strong> și rulează testul din tab Testare, sau folosește butoanele de mai jos.
                    </p>
                    <div class="sc-veh-grid">
                        <label class="sc-field"><span>Max mărci (0 = toate)</span>
                            <input type="number" min="0" data-param="max_brands" class="box h-9 w-full rounded-md border px-2 text-sm" value="${esc(String(p.max_brands ?? 0))}"></label>
                        <label class="sc-field"><span>Max modele / marcă (0 = toate)</span>
                            <input type="number" min="0" data-param="max_models_per_brand" class="box h-9 w-full rounded-md border px-2 text-sm" value="${esc(String(p.max_models_per_brand ?? 0))}"></label>
                        <label class="sc-field"><span>Pauză API (ms)</span>
                            <input type="number" min="0" max="2000" data-param="delay_ms" class="box h-9 w-full rounded-md border px-2 text-sm" value="${esc(String(p.delay_ms ?? 120))}"></label>
                        <label class="sc-field"><span>ID marcă preview (0 = prima)</span>
                            <input type="number" min="0" data-param="marca_id" class="box h-9 w-full rounded-md border px-2 text-sm" value="${esc(String(p.marca_id ?? 0))}"></label>
                        <label class="sc-field full flex-row items-center gap-2 pb-1">
                            <input type="checkbox" data-param="resume" ${p.resume !== false ? 'checked' : ''}>
                            <span>Resume — sari mărcile deja salvate în JSON</span>
                        </label>
                    </div>
                    <div class="sc-veh-actions">
                        <button type="button" class="sc-btn-primary sc-btn-sm" data-action="veh-step-start-bg" data-step-id="${esc(step.id)}">▶ Lansează bot complet (fundal)</button>
                        <button type="button" class="sc-btn-outline sc-btn-sm" data-action="veh-step-stop" data-step-id="${esc(step.id)}">Oprește bot</button>
                        <button type="button" class="sc-btn-outline sc-btn-sm" data-action="veh-step-preview" data-step-id="${esc(step.id)}">Test lanț (1 marcă)</button>
                        <button type="button" class="sc-btn-outline sc-btn-sm" data-action="veh-step-status" data-step-id="${esc(step.id)}">Reîmprospătează status</button>
                    </div>
                    <div class="sc-veh-progress full hidden" data-veh-progress="${esc(step.id)}">
                        <div class="sc-veh-progress-head">
                            <span class="sc-veh-progress-label">Bot oprit</span>
                            <span class="sc-veh-progress-pct">0%</span>
                        </div>
                        <div class="sc-pipeline-bar-track"><div class="sc-pipeline-bar-fill" style="width:0%"></div></div>
                    </div>
                    <p class="text-xs opacity-60 m-0 mb-2 full">
                        <strong>Bot fundal</strong> parcurge toate mărcile și salvează progresiv în
                        <code>vehicle_tree.json</code> + export plat <code>vehicle_combos_flat.json</code> (toate linkurile catalog).
                        Poți închide pagina — verifică progresul cu «Reîmprospătează status».
                    </p>
                    <pre class="sc-veh-out" data-veh-out="${esc(step.id)}">Apasă Status sau Test lanț…</pre>
                </div>`;
        }
        return `<p class="text-sm opacity-60 full">Tip necunoscut: ${esc(t)}</p>`;
    }
